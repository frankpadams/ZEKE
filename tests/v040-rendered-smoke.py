...
with sync_playwright() as p:
    ...
    for route,viewport in [('health/dashboard',{'width':1400,'height':1000}),('fitness',{'width':1400,'height':1000}),('data-integrity',{'width':1400,'height':1000}),('health/dashboard',{'width':390,'height':844})]:
        ...
        if route=='health/dashboard':
            rec.update({'dashboard_v3':page.locator('.dashboard-v3').count()==1,'story_cards':page.locator('.story-card').count(),'weekly_plan':page.locator('.weekly-plan-card').count()==1,'review_card':page.locator('.review-status-card').count()==1,'mobile_log_label':page.locator('#mobileLogButton').inner_text() if page.locator('#mobileLogButton').count() else ''})
        if route=='fitness':
