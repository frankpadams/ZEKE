...
must(index.includes('ZEKE v0.40.0')&&index.includes('2026.08.03.1'),'version/build missing from index');
...
must(version.includes("version: '0.40.0'")&&sw.includes('project-zeke-v0.40.0-20260803.1'),'version/cache mismatch');
...
must(app.includes('<b>Log</b>')&&!app.includes('<b>Gym</b>'),'mobile center action must be Log, not a separate Gym mode');
...
must(app.includes('Distance (mi, optional)')&&app.includes('Stair steps')&&app.includes('Average HR (only if measured)'),'activity-specific fields missing');
