...
def stale_supporting_continuity(d):
    p=d/'DEVELOPMENT_SYSTEM/STATUS_LANGUAGE.md'; j=json.loads((d/'DEVELOPMENT_MEMORY/PROJECT_STATE.json').read_text()); p.write_text(p.read_text().replace(j['current_build'],'2026.07.25.2'))
def stale_authority_header_governance(d):
    p=d/'DESIGN_AUTHORITY.md'; j=json.loads((d/'DEVELOPMENT_SYSTEM/GOVERNANCE_RULES.json').read_text()); p.write_text(p.read_text().replace('governance '+j['governance_revision'],'governance 2026.07.25.2',1))
