...
must(/^0(?:\.\d+){2,}$/.test(v)&&/^2026\.\d{2}\.\d{2}\.\d+$/.test(b),'project state metadata mismatch');
...
const cacheToken=b.replace(/\./g,'').replace(/^([0-9]{8})([0-9])$/,'$1.$2')+'v'+v.replace(/\./g,'');
must(index.includes(`v=${cacheToken}`),'current cache token absent');
