(function(){
  'use strict';
  const STORAGE_KEY = 'ZEKE_AI_ROUTER_CONFIG_V064';
  const CHAT_KEY = 'ZEKE_AI_ROUTER_CHAT_V064';
  ...
  const PROVIDERS = {
    ...
    groq: {
      label:'Groq Free', tier:'Free developer tier / rate limited', kind:'openai', priority:9,
  ...
  const DEFAULTS = {
    privacy:'minimal', freeFirst:true,
  ...
  function createPanel(){
    ...
    root.innerHTML=`<button class="zair-fab" title="AI Router status">AI</button>
      <section class="zair-panel zair-panel-v6" hidden>
        <div class="zair-head"><div><strong>AI Connections</strong><span>Connect services; ZEKE chooses the best available route</span></div><button class="zair-close" aria-label="Close">×</button></div>
        <div class="zair-tabs"><button data-tab="status" class="active">Status</button><button data-tab="setup">Connections</button><button data-tab="chat">Connection test</button></div>
        <div class="zair-body">
          <div class="zair-tab" data-pane="status"><div class="zair-router-summary"></div><p class="zair-note">ZEKE features ask the router for a capability. The router selects among connected services based on task fit, privacy, availability, limits, fallback status, and free-first rules.</p></div>
          <div class="zair-tab" data-pane="setup" hidden>
            <div class="zair-quickstart">
              <div><strong>Fast free setup</strong><span>Connect one or more services. You do not choose an active AI; ZEKE routes each task automatically and can fall back when needed.</span></div>
              <div class="zair-quickstart-actions">
                <button type="button" data-quick-provider="groq">Groq Free</button>
                <button type="button" data-quick-provider="gemini">Gemini Free</button>
                <button type="button" data-quick-provider="openrouter">OpenRouter Free</button>
              </div>
            </div>
            <div class="zair-global-grid"><label>Privacy <select class="zair-privacy"><option value="minimal">Minimal sharing</option><option value="balanced">Balanced context</option></select></label><label><input type="checkbox" class="zair-free-first"> Prefer free / rate-limited free providers</label></div><div class="zair-manual-packet"><strong>Manual AI packet</strong><span>Export a privacy-reviewed packet for use with an external AI, then import its response without treating it as fact automatically.</span><a href="./AI_PACKET_WORKFLOW.md" target="_blank" rel="noopener">View manual packet workflow ↗</a></div>
  ...
  function hydrate(root){
    const cfg=loadConfig(); $('.zair-privacy',root).value=cfg.privacy; $('.zair-free-first',root).checked=cfg.freeFirst;
  ...
  function saveFromForm(root){
    const cfg=loadConfig();cfg.privacy=$('.zair-privacy',root).value;cfg.freeFirst=$('.zair-free-first',root).checked;
  ...
  function bind(root){
    ...
    $$('.zair-quickstart-actions button',root).forEach(btn=>btn.onclick=()=>{
      const id=btn.dataset.quickProvider,row=$(`.zair-provider-v6[data-provider="${id}"]`,root);
      if(!row)return; const enabled=$('[data-field="enabled"]',row); if(enabled)enabled.checked=true;
      row.scrollIntoView({behavior:'smooth',block:'center'});
      row.classList.add('zair-highlight'); setTimeout(()=>row.classList.remove('zair-highlight'),1800);
    });
