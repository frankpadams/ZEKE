(() => {
  ...
  function openWorkoutEntryModal(repeatLast=false){
    ...
    const renderSummary=()=>{
      ...
      screen.innerHTML=`<section class="zeke-today-card"><div class="zeke-today-heading"><div><span class="tile-kicker">TODAY'S WORKOUT</span><h3>${draft.items.length?`${draft.items.length} exercise${draft.items.length===1?'':'s'}`:'Ready when you are'}</h3></div>${draft.items.length?showSaveState('Draft preserved','saved'):''}</div><div class="zeke-workout-items">${cards||'<p class="zeke-empty-workout">Add an exercise when you reach a machine. You can do them in any order.</p>'}</div><button type="button" class="primary zeke-add-exercise" id="addExerciseBtn">+ Add Exercise</button>${draft.items.length?'<button type="button" class="secondary zeke-finish-workout" id="finishWorkoutBtn">Finish Workout</button>':''}</section><section class="zeke-exercise-picker" id="exercisePicker" hidden><label for="exerciseChoice">Choose an exercise</label><select id="exerciseChoice"><option value="">Select exercise</option>${options}<option value="__custom">Other / custom</option></select><input id="customExerciseName" placeholder="Exercise name" hidden><div class="zeke-picker-actions"><button type="button" class="secondary" id="cancelExercisePicker">Cancel</button><button type="button" class="primary" id="openExerciseBtn">Open Exercise</button></div></section>`;
    ...
    const renderExercise=index=>{
      ...
      const readiness=arr.length>=3?'High':arr.length?'Moderate':'Learning';
      ...
      $('#saveExerciseBtn').onclick=async()=>{const btn=$('#saveExerciseBtn'),status=$('#exerciseSaveState'),error=$('#exerciseSaveError');error.hidden=true;btn.disabled=true;btn.firstChild.textContent='Saving…';try{item.notes=$('#exerciseNotes')?.value.trim()||'';item.rpe=$('#exerciseRpe')?.value===''?null:Number($('#exerciseRpe')?.value);item.rest_sec=$('#exerciseRest')?.value===''?null:Number($('#exerciseRest')?.value);if(profile==='strength'){item.weight=$('#exerciseWeight').value;item.setReps=$$('.set-reps').map(x=>x.value).filter(Boolean);item.reps=item.setReps.length?Math.round(item.setReps.reduce((a,b)=>a+Number(b),0)/item.setReps.length):null;item.sets=item.setReps.length;}else{item.duration_min=$('#exerciseDuration')?.value||'';item.steps=$('#exerciseSteps')?.value||'';item.distance_mi=$('#exerciseDistance')?.value||'';item.average_hr=$('#exerciseHr')?.value||'';}const transactionId=(globalThis.crypto?.randomUUID?.()||`tx-${Date.now()}`);await ZekeData.addEvent({category:'workout',timestamp:`${draft.date}T12:00:00`,raw_text:item.notes,structured:{exercise:item.exercise,canonical_activity_id:activityKey(item.exercise).replace(/ /g,'_'),activity_profile:profile,workout_id:`workout-${draft.date}`,weight:item.weight===''?null:Number(item.weight),reps:item.reps??null,sets:item.sets??null,set_reps:item.setReps?.map(Number)||null,duration_min:item.duration_min===''?null:Number(item.duration_min),steps:item.steps===''?null:Number(item.steps),distance_mi:item.distance_mi===''?null:Number(item.distance_mi),average_hr:item.average_hr===''?null:Number(item.average_hr),rpe:item.rpe,rest_sec:item.rest_sec,notes:item.notes,completed:true,interpretation_status:'confirmed'},provenance:{source:'mockup-fidelity-exercise-screen',entry_mode:'structured-form',transaction_id:transactionId}});item.saved=true;persist();status.innerHTML='<strong>✓ Saved '+new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})+'</strong><small>Synced to Drive ☁</small>';btn.disabled=false;btn.innerHTML=`✓ &nbsp; Save Exercise<small>${esc(item.exercise)} saved to today’s workout</small>`;await refreshData();}catch(err){btn.disabled=false;btn.innerHTML='Try Save Again';status.innerHTML='<strong>Save failed</strong><small>Nothing was marked saved</small>';error.hidden=false;error.textContent=`Exercise was not saved: ${err?.message||'Unknown error'}`;}};
    };
    function guideMediaFor(exercise){ return null; }
    const openFormGuide=exercise=>{document.body.classList.add('zeke-sheet-open');document.body.insertAdjacentHTML('beforeend',`<div class="zeke-sheet-scrim" id="formGuideSheet"><section class="zeke-form-sheet mock-form-sheet" role="dialog" aria-modal="true" aria-label="Form guide for ${esc(exercise)}"><div class="zeke-sheet-handle"></div><header><h2>${esc(exercise)} – Form Guide</h2><button type="button" id="closeFormGuide" aria-label="Close">×</button></header><nav class="zeke-guide-tabs"><button class="active" data-guide-tab="overview">▣<small>Overview</small></button><button data-guide-tab="setup">♙<small>Setup</small></button><button data-guide-tab="movement">♨<small>Movement</small></button><button data-guide-tab="mistakes">⚠<small>Common Mistakes</small></button><button data-guide-tab="tips">♧<small>Tips</small></button></nav><div id="guideContent" class="mock-guide-overview"><div class="mock-guide-visual"><strong>${esc(exercise)}</strong><small>Reviewed adult guide image pending</small></div><ul class="mock-guide-facts"><li><b>Targets:</b> Primary muscles for this exercise</li><li><b>Equipment:</b> ${esc(exercise)} machine</li><li><b>Type:</b> Strength</li><li><b>Level:</b> Beginner to Advanced</li></ul><section><h3>KEY POINTS</h3><article><i>♙</i><div><strong>Set up</strong><p>Adjust the machine so the working joint and handles align correctly. Plant your feet and brace against the pad.</p></div></article><article><i>♨</i><div><strong>Movement</strong><p>Move through a controlled path. Keep the target muscles engaged and avoid using momentum.</p></div></article><article><i>⚠</i><div><strong>Tempo &amp; Breathing</strong><p>Exhale through the effort. Inhale during the controlled return. Avoid locking out hard.</p></div></article><article><i>♧</i><div><strong>Tips</strong><p>Use a load that allows stable form and a repeatable range of motion.</p></div></article></section></div><button type="button" class="mock-got-it" id="gotItGuide">Got it</button></section></div>`);const close=()=>{$('#formGuideSheet')?.remove();document.body.classList.remove('zeke-sheet-open')};$('#closeFormGuide').onclick=close;$('#gotItGuide').onclick=close;$('#formGuideSheet').onclick=e=>{if(e.target.id==='formGuideSheet')close()};$$('[data-guide-tab]').forEach(btn=>btn.onclick=()=>{$$('[data-guide-tab]').forEach(x=>x.classList.toggle('active',x===btn));const key=btn.dataset.guideTab;const copy={overview:'Overview combines the exercise image, equipment facts, and key points.',setup:'Adjust the seat, pads, and handles before loading the movement.',movement:'Use a controlled range and keep your body stable against the support.',mistakes:'Avoid momentum, shortened range, poor alignment, and loads that change your technique.',tips:'Use repeatable form, controlled breathing, and stop for sharp or increasing pain.'};$('#guideContent').innerHTML=`<section class="mock-tab-copy"><h3>${esc(btn.textContent.trim())}</h3><p>${esc(copy[key])}</p></section>`;});};
