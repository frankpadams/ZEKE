(() => {
  ...
  function familyVariationChart(sessions=[],category='strength',family=''){
    ...
    const grouped=new Map();for(const row of sessions){if(!Number.isFinite(Number(row.weight))||!row.date)continue;const v=row.variation_name||family;if(!grouped.has(v))grouped.set(v,[]);grouped.get(v).push(row)}
    const series=[...grouped.entries()].filter(([,rows])=>rows.length>=1);if(!series.length)return '<div class="activity-chart-status"><strong>Load trend unavailable</strong><span>No comparable load values are recorded yet.</span></div>';
    const all=series.flatMap(([,rows])=>rows),dates=all.map(r=>new Date(r.date).getTime()),weights=all.map(r=>Number(r.weight)),minD=Math.min(...dates),maxD=Math.max(...dates),minW=Math.min(...weights),maxW=Math.max(...weights),w=360,h=120,pl=34,pr=12,pt=10,pb=26,x=t=>pl+(w-pl-pr)*((t-minD)/(maxD-minD||1)),y=v=>pt+(h-pt-pb)*(1-((v-minW)/(maxW-minW||1)));
    const paths=series.map(([name,rows],si)=>{const pts=rows.map(r=>[x(new Date(r.date).getTime()),y(Number(r.weight)),r]).sort((a,b)=>a[0]-b[0]),d=pts.map((p,i)=>(i?'L':'M')+p[0].toFixed(1)+' '+p[1].toFixed(1)).join(' ');return `<g class="variation-series variation-series-${si%6}"><path d="${d}"/>${pts.map(p=>`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="3.5"><title>${esc(name)} · ${fmtDate(p[2].date,{month:'short',day:'numeric'})}: ${p[2].weight} lb</title></circle>`).join('')}</g>`}).join('');
    const legend=series.map(([name],si)=>`<span class="variation-legend-item variation-series-${si%6}"><i></i>${esc(name)}</span>`).join('');
    return `<div class="activity-chart-block family-variation-chart"><span class="activity-chart-label">Load by variation · shared axes</span><svg viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(family)} load history with each variation shown as a separate series on shared axes"><line class="family-axis" x1="${pl}" y1="${h-pb}" x2="${w-pr}" y2="${h-pb}"/><line class="family-axis" x1="${pl}" y1="${pt}" x2="${pl}" y2="${h-pb}"/><text class="axis-label" x="2" y="${pt+5}">${Math.round(maxW)} lb</text><text class="axis-label" x="2" y="${h-pb}">${Math.round(minW)} lb</text>${paths}</svg><div class="variation-legend">${legend}</div></div>`;
  }
  ...
  function fitnessPageHTML() {
    ...
    const cards=entries.map(({name,arr,profile,category})=>{const first=arr[0],last=arr.at(-1),latestVariation=last?.variation_name||name,exactSessions=workoutGroups().get(latestVariation)||arr,rec=activityRecommendation(latestVariation,exactSessions),spark=familyVariationChart(arr,category,name),facts=activitySummaryFacts(first,last,category,name),expanded=state.expandedActivity===name;
