(() => {
  ...
  const BUILD = window.ZEKE_BUILD || window.ZEKE_VERSION || { version: '0.48.0.2', build: '2026.08.30.1', label: 'ZEKE v0.48.0.2 Longitudinal Intelligence + Product Coherence' };
  const state = {
    route:'dashboard', range:localStorage.getItem('zeke-fitness-range')||'month', dashboardHealthRange:localStorage.getItem('zeke-dashboard-health-range')||'month', dashboardTrendRange:localStorage.getItem('zeke-dashboard-trend-range')||'quarter', timelineScale:localStorage.getItem('zeke.timeline.scale.v1')||'week', selectedMetric:'weight',
  ...
  function v47DashboardMetricCard(id){
    ...
    let pts=id==='blood_pressure'?bloodPressureSeries(false,state.dashboardHealthRange).sys:metricSeries(id,state.dashboardHealthRange);if(pts.length<2)pts=id==='blood_pressure'?bloodPressureSeries(true).sys:allMetricSeries(id);if(pts.length>24)pts=pts.slice(-24);
  ...
  function v47StatusHTML(){
    const hr=latestMetric('resting_hr'),sleep=latestMetric('sleep_duration'),steps=latestMetric('steps');
    const questions=reviewTasks().length,injury=state.events.filter(e=>recordIsActive(e)&&semanticCategory(e)==='injury'&&(e.structured?.ongoing||/ongoing|active/i.test(String(e.structured?.status||'')))).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0))[0];
    const summary=questions?`${questions} item${questions===1?'':'s'} need your attention.`:injury?`Current injury context is available when ZEKE plans training.`:'Your current record is ready for today’s decisions.';
    const items=[];if(hr)items.push([v47Icon('heart',18),'Resting HR',`${hr.value} ${hr.unit||'bpm'}`]);if(sleep)items.push([v47Icon('moon',18),'Sleep',durationLabel(sleep.value)]);if(steps)items.push([v47Icon('activity',18),'Active',`${Number(steps.value).toLocaleString()} ${steps.unit||'steps'}`]);
    return `<section class="v47-card v47-status"><div class="v47-card-title"><span class="v47-title-icon">${v47Icon('activity',25)}</span><h2>Today’s Status</h2><span class="v47-pill good">${questions?'Needs attention':'On Track'}</span></div><p>${esc(summary)}</p><div class="v47-status-metrics">${items.map(([i,l,v])=>`<div>${i}<span>${esc(l)}</span><strong>${esc(v)}</strong></div>`).join('')||'<div class="v47-compact-empty">No current summary metrics yet.</div>'}</div></section>`;
  }
  ...
  function v47InsightsHTML(){
    ...
    const discovery=state.discoveries.find(d=>!['superseded','dismissed','resolved','stale'].includes(String(d.status||'').toLowerCase()));if(discovery)items.push({icon:'insight',title:discovery.title||'New observation',text:discovery.summary||discovery.text||'Open the evidence for details.',key:discovery.id||discovery.title});
    const sleep=latestMetric('sleep_duration');if(sleep&&!items.some(x=>/sleep/i.test(x.title)))items.push({icon:'moon',title:'Sleep context is available',text:`Latest recorded sleep: ${durationLabel(sleep.value)}. Open Health for the dated trend.`,action:'health'});
    const weight=latestMetric('weight');if(weight&&items.length<3)items.push({icon:'heart',title:'Weight trend is current',text:`Latest verified weight: ${weight.value} ${weight.unit||'lb'}. ZEKE keeps the full history in Health.`,action:'health'});
    return `<section class="v47-card v47-insights"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('insight',24)}<h2>ZEKE Insights</h2></div><button class="text-action" data-route="insights">View All</button></div><div class="v47-insight-list">${items.slice(0,3).map((x,i)=>`<button type="button" class="v47-insight-row tone-${i}" ${x.key?`data-insight-key="${esc(x.key)}" data-insight-action="pattern"`:`data-route="${x.action||'insights'}"`}><span class="v47-row-icon">${v47Icon(x.icon,20)}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.text)}</small></div>${v47Icon('chevron',18)}</button>`).join('')||'<div class="v47-compact-empty">No supported new insight needs attention.</div>'}</div></section>`;
  }
  function v47HealthHTML(){
    ...
    return `<section class="v47-card v47-health"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('heart',24)}<h2>Health at a Glance</h2></div><button class="text-action" data-route="health">View Health</button></div><div class="v47-health-grid">${chosen.map(v47DashboardMetricCard).join('')||'<div class="v47-compact-empty">Log a health value to begin your snapshot.</div>'}</div></section>`;
  }
  function v47TimelineHTML(){
    const recent=dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e))).sort((a,b)=>new Date(b.timestamp||b.recorded_at)-new Date(a.timestamp||a.recorded_at)).slice(0,5).reverse();
    const label=e=>isWorkoutEvent(e)?workoutStructured(e).exercise||'Workout':humanEvent(e),kind=e=>isWorkoutEvent(e)?'dumbbell':semanticCategory(e)==='sleep'?'moon':semanticCategory(e)==='medication'?'pill':semanticCategory(e)==='injury'?'heart':'activity';
    return `<section class="v47-card v47-timeline"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('calendar',24)}<h2>Timeline Snapshot</h2></div><button class="text-action" data-route="calendar">Open timeline</button></div>${recent.length?`<div class="v47-timeline-track">${recent.map((e,i)=>`<button type="button" data-route="${isWorkoutEvent(e)?'fitness':'health'}"><span>${v47Icon(kind(e),18)}</span><strong>${esc(label(e))}</strong><small>${esc(fmtDate(e.timestamp||e.recorded_at,{month:'short',day:'numeric'}))}</small><i></i></button>`).join('')}</div>`:'<div class="v47-compact-empty">No recorded timeline events yet.</div>'}</section>`;
  }
  function v47GoalsHTML(){
    const goals=state.factors.filter(f=>f.type==='goal'&&!['dismissed','resolved','inactive'].includes(String(f.status||'').toLowerCase())).slice(0,3);
    return `<section class="v47-card v47-goals"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('target',24)}<h2>Goals</h2></div><button class="text-action" data-route="fitness">Review</button></div>${goals.length?`<div class="v47-goal-list">${goals.map(g=>`<button type="button" data-route="fitness"><strong>${esc(g.goal_statement||g.summary||'Goal')}</strong><small>${esc(g.target?`Target: ${g.target}${g.unit?` ${g.unit}`:''}`:'Open to review progress')}</small><span class="v47-progress"><i style="width:${Math.max(12,Math.min(82,Number(g.progress_pct)||35))}%"></i></span></button>`).join('')}</div>`:'<div class="v47-goals-empty">No active goals recorded.<button type="button" class="text-action" data-route="fitness">Add or review goals</button></div>'}</section>`;
  }
