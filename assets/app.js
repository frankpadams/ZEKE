(() => {
  ...
  function openWorkoutEntryModal(){
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay zeke-workout-shell" id="directWorkoutModal"><div class="zeke-workout-app" role="dialog" aria-modal="true" aria-label="Gym Mode"><header class="zeke-workout-header"><button type="button" class="zeke-back-button" id="workoutBackBtn" aria-label="Back"><span aria-hidden="true">‹</span><span class="zeke-back-label">Today’s Workout</span></button><h2 id="workoutScreenTitle">Today’s Workout</h2><button type="button" class="icon-btn zeke-workout-more" id="closeDirectWorkout" aria-label="Exit Gym Mode">×</button><label class="zeke-workout-date">Date <input id="gymWorkoutDate" type="date" value="${escAttr(date)}"></label></header><main id="workoutScreen" class="zeke-workout-screen"></main></div></div>`);
    ...
    const renderSummary=()=>{currentIndex=null;screen.className='zeke-workout-screen is-workout-summary';back.hidden=true;title.textContent='Today’s Workout';const cards=draft.items.map((item,i)=>{const [icon,label]=statusLabel(item);return`<article class="zeke-workout-item-wrap" draggable="true" data-drag-index="${i}"><button type="button" class="zeke-workout-item" data-edit-workout-item="${i}"><span class="zeke-item-check state-${item.status}">${icon}</span><span><strong>${esc(item.exercise)}</strong><small>${esc(summaryText(item))}</small><em>${esc(label)}</em></span><span aria-hidden="true">›</span></button><div class="zeke-reorder-actions"><button type="button" data-move-up="${i}" aria-label="Move ${esc(item.exercise)} up">↑</button><button type="button" data-move-down="${i}" aria-label="Move ${esc(item.exercise)} down">↓</button><button type="button" data-remove-item="${i}" aria-label="Remove ${esc(item.exercise)}">×</button></div></article>`}).join('');screen.innerHTML=`<section class="zeke-today-card"><div class="zeke-today-heading"><div><span class="tile-kicker">GYM MODE</span><h3>${draft.items.length?`${draft.items.length} exercise${draft.items.length===1?'':'s'}`:'Choose how to begin'}</h3></div></div>${cards?`<div class="zeke-workout-items">${cards}</div>`:`<div class="zeke-empty-actions"><button type="button" class="primary" id="startRoutineBtn">Start from Routine</button><button type="button" class="secondary" id="addExerciseBtn">Enter Exercises</button></div>`}${draft.items.length?`<div class="zeke-summary-actions"><button type="button" class="secondary" id="addExerciseBtn">+ Add Exercise</button><button type="button" class="primary" id="endWorkoutBtn">End Workout</button></div>`:''}</section><section class="zeke-exercise-picker" id="exercisePicker" hidden><h3>Add an exercise</h3><div class="zeke-common-exercises">${exerciseNames.slice(0,8).map(n=>`<button type="button" data-common-exercise="${escAttr(n)}">${esc(n)}</button>`).join('')}</div><details><summary>Search exercise library</summary><input id="exerciseSearch" type="search" placeholder="Search exercises"><div id="exerciseSearchResults" class="zeke-search-results"></div></details><button type="button" class="secondary" id="customExerciseBtn">Create custom exercise</button><div class="zeke-picker-actions"><button type="button" class="secondary" id="cancelExercisePicker">Cancel</button></div></section>`;
      const openPicker=()=>{$('#exercisePicker').hidden=false;};$('#addExerciseBtn')?.addEventListener('click',openPicker);$('#startRoutineBtn')?.addEventListener('click',()=>{const choices=['Full Body Starter','Upper Body','Lower Body','Shoulder-Friendly'];document.body.insertAdjacentHTML('beforeend',`<div class="zeke-mini-dialog" id="routinePicker"><section><h3>Start from Routine</h3>${choices.map(x=>`<button type="button" data-routine="${x}">${x}</button>`).join('')}<button type="button" id="cancelRoutine">Cancel</button></section></div>`);const templates={'Full Body Starter':['Leg Press','Chest Press','Lat Pulldown','Seated Leg Curl','Seated Row'],'Upper Body':['Chest Press','Lat Pulldown','Seated Row','Bicep Curl','Tricep Press'],'Lower Body':['Leg Press','Leg Extension','Seated Leg Curl','Stair Climber'],'Shoulder-Friendly':['Seated Row','Lat Pulldown','Bicep Curl','Tricep Press']};$$('[data-routine]').forEach(b=>b.onclick=()=>{for(const n of templates[b.dataset.routine]||[]){const x=blankItem(n);x.status='suggested';x.source='routine';draft.items.push(x)}persist();$('#routinePicker').remove();renderSummary()});$('#cancelRoutine').onclick=()=>$('#routinePicker').remove();});
    ...
    $('#gymWorkoutDate').addEventListener('change',e=>{const next=e.target.value;if(!next||next===date)return;if(draft.items.some(x=>x.status==='in-progress')&&!confirm('Change the workout date? Unsaved entries will move to the selected date.')){e.target.value=date;return;}persist();date=next;draft=readDraft()||{date,items:[]};draft.date=date;persist();renderSummary();});back.onclick=renderSummary;$('#closeDirectWorkout').onclick=()=>{if(draft.items.some(x=>x.status==='in-progress')&&!confirm('Exit Gym Mode with unsaved changes? They will remain available on this device.'))return;persist();$('#directWorkoutModal')?.remove();};renderSummary();
  }
