(() => {
  ...
  function canonicalActivityCategory(name,profile){
    ...
    if(p==='sport'||/ski|basketball|tennis|sport|recreation|hike|kayak|canoe|paddle|rowing|surf/.test(n))return 'sport';
  ...
  function fitnessPageHTML() {
    ...
    const customActivities=customActivityLibrary(),favorites=new Set(JSON.parse(localStorage.getItem('zeke.fitness.activityFavorites.v1')||localStorage.getItem('zeke-activity-favorites')||'[]')),customNames=new Set(customActivities.map(a=>normalizedActivityName(a.name).toLowerCase()));
  ...
  function activityProfile(name, explicit=''){
    ...
    if(/stair|treadmill|elliptical|bike|bicycl|cycle|walk|run|swim|rower|cardio/.test(n))return 'cardio';
    if(/kayak|canoe|paddle|rowing|surf|ski|tennis|basketball|pickleball/.test(n))return 'sport';
    return 'strength';
  }
  ...
  function profileFields(profile, latest={}){
    ...
    if(profile==='cardio')return `<label>Duration (min)<input id="activityDuration" type="number" min="0.1" step="0.1"></label><label>Distance (mi, optional)<input id="activityDistance" type="number" min="0" step="0.01"></label><label>Steps (optional)<input id="activitySteps" type="number" min="0" step="1"></label><label>Level / incline (optional)<input id="activityLevel" type="number" min="0" step="0.1"></label><label>Average HR (optional)<input id="activityHr" type="number" min="1"></label>`;
    if(profile==='sport')return `<label>Duration (min)<input id="activityDuration" type="number" min="0.1" step="0.1"></label><label>Distance (mi, optional)<input id="activityDistance" type="number" min="0" step="0.01"></label><label>Effort / RPE (optional)<input id="activityRpe" type="number" min="1" max="10" step="0.5"></label><label>Route / location (optional)<input id="activityArea" type="text"></label>`;
  ...
  function bind() {
    ...
    $('#mobileGymButton')?.addEventListener('click',()=>openWorkoutEntryModal());
    $('#manageRoutinesBtn')?.addEventListener('click',openRoutineManager);
