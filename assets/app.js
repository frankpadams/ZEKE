(() => {
  ...
  const BUILD = window.ZEKE_BUILD || window.ZEKE_VERSION || { version: '0.26.0', build: '2026.07.22.1', label: 'Daily Briefing & Health Architecture' };
  const state = {
    ...
    coachExpanded:false, coachCardExpanded:false, coachFocus:'', coachAlertDismissed:{}, activityTab:localStorage.getItem('zeke.fitness.activityTab.v1')||localStorage.getItem('zeke-activity-tab')||'favorites', expandedActivity:'', healthTab:localStorage.getItem('zeke.health.libraryTab.v1')||localStorage.getItem('zeke-health-tab')||'frequent', expandedHealthMetric:'', customizeOpen:false, metricMenuOpen:false, quickLogOpen:false, expandedReviewTasks:new Set(),
  ...
  function coachHTML() {
    ...
    if(timely&&timely.score>=55)recommendations.push({title:timely.title,text:timely.suggestion,meta:`${timely.confidence} confidence · ${timely.recent.length} recent session${timely.recent.length===1?'':'s'}`,action:`<button class="text-action" data-coach-exercise="${esc(timely.name)}">Open activity</button>`});
  ...
  function openEvidenceReview(key){
    ...
    const discovery=state.discoveries.find(d=>String(d.id||'')===String(key)||String(d.title||'').toLowerCase()===normalized||normalized.includes(String(d.title||'').toLowerCase()));
    const patterns=patternCandidates();
    const pattern=patterns.find(p=>normalized.includes(String(prettyVar(p.a)).toLowerCase())||normalized.includes(String(prettyVar(p.b)).toLowerCase()))||patterns[0];
    const title=discovery?.title|| (pattern?`${prettyVar(pattern.a)} and ${prettyVar(pattern.b)}`:String(key||'Evidence review'));
    const summary=discovery?.summary|| (pattern?(pattern.r>0?'These values moved in the same direction on paired dates.':'These values moved in opposite directions on paired dates.'):'ZEKE does not yet have enough structured evidence to show a specific relationship.');
    const limitations=discovery?.limitations||'This is an exploratory observation. Timing, missing records, measurement differences, and third variables may explain the pattern. It does not establish causation.';
    const support=pattern?`Correlation: r = ${pattern.r.toFixed(2)} · ${pattern.n} paired day${pattern.n===1?'':'s'}`:'No calculated paired-data result is attached to this observation.';
    const related=dedupeDisplayEvents(state.events.filter(recordIsActive)).filter(e=>{const text=(humanEvent(e)+' '+(e.raw_text||'')).toLowerCase();return title.toLowerCase().split(/\W+/).filter(x=>x.length>4).some(x=>text.includes(x));}).slice(0,8);
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="evidenceReviewModal"><div class="direct-entry-card evidence-review-card"><div class="section-head"><div><span class="tile-kicker">SUPPORTING EVIDENCE</span><h2>${esc(title)}</h2><p>${esc(summary)}</p></div><button class="icon-btn" id="closeEvidenceReview" aria-label="Close evidence review">×</button></div><div class="evidence-review-stat"><strong>${esc(support)}</strong><span>Screening result, not a diagnosis or causal conclusion</span></div><section><h3>What ZEKE observed</h3><p>${esc(summary)}</p></section><section><h3>Limitations</h3><p>${esc(String(limitations))}</p></section><section><h3>Related dated records</h3>${related.length?`<div class="evidence-review-list">${related.map(e=>`<div><time>${esc(fmtDate(e.timestamp||e.recorded_at,{month:'short',day:'numeric',year:'numeric'}))}</time><span>${esc(humanEvent(e))}</span><small>${esc(e.provenance?.source||e.provenance?.sheet||'ZEKE record')}</small></div>`).join('')}</div>`:'<p>No directly matched dated records were found. This observation should not be acted on without reviewing the underlying data.</p>'}</section><section><h3>Research connections</h3><p>Published evidence is separate from your personal pattern. Review current literature before treating an association as guidance.</p><a class="secondary compact external-evidence-link" href="https://pubmed.ncbi.nlm.nih.gov/?term=${encodeURIComponent(title)}" target="_blank" rel="noopener noreferrer">Search PubMed for this topic</a></section><div class="direct-entry-actions"><button class="secondary" id="closeEvidenceReviewBottom">Close</button><button class="primary" id="openFullPatternLab">Open Pattern Lab</button></div></div></div>`);
    const close=()=>document.querySelector('#evidenceReviewModal')?.remove();
    document.querySelector('#closeEvidenceReview')?.addEventListener('click',close);document.querySelector('#closeEvidenceReviewBottom')?.addEventListener('click',close);document.querySelector('#evidenceReviewModal')?.addEventListener('click',e=>{if(e.target.id==='evidenceReviewModal')close()});document.querySelector('#openFullPatternLab')?.addEventListener('click',()=>{close();openPatternLab(title)});
  }
  ...
  function fitnessPageHTML() {
    ...
    if(state.activityTab==='favorites'){const favoriteEntries=entries.filter(x=>favorites.has(x.name));entries=favoriteEntries.length?favoriteEntries:entries.sort((a,b)=>{const ad=a.arr.at(-1)?.date?new Date(a.arr.at(-1).date).getTime():0,bd=b.arr.at(-1)?.date?new Date(b.arr.at(-1).date).getTime():0;return (b.arr.length*3+bd/1e12)-(a.arr.length*3+ad/1e12)}).slice(0,10);}else if(ACTIVITY_TAXONOMY.some(x=>x.id===state.activityTab))entries=entries.filter(x=>x.category===state.activityTab);else entries=entries.sort((a,b)=>{const ad=a.arr.at(-1)?.date?new Date(a.arr.at(-1).date).getTime():0,bd=b.arr.at(-1)?.date?new Date(b.arr.at(-1).date).getTime():0;return (b.arr.length*3+bd/1e12)-(a.arr.length*3+ad/1e12)}).slice(0,10);
  ...
  function openWorkoutEntryModal(repeatLast=false){
    ...
    const previous=repeatLast?dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e)&&isWorkoutEvent(e))).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0)):[];
    const lastDate=previous[0]?.timestamp?String(previous[0].timestamp).slice(0,10):'';
    const repeatEntries=lastDate?previous.filter(e=>String(e.timestamp||'').slice(0,10)===lastDate).reverse().map(e=>workoutStructured(e)):[];
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="directWorkoutModal"><div class="direct-entry-card workout-entry-card"><div class="section-head"><div><h2>${repeatLast?'Repeat last workout':'Gym logging'}</h2><p>${repeatLast?'Last session is preloaded. Change only what is different, then save.':'Large, activity-specific fields keep logging fast. Optional effort, pain, technique, and injury context remain available.'}</p></div><button type="button" class="icon-btn" id="closeDirectWorkout" aria-label="Close workout editor" title="Close">×</button></div><form id="directWorkoutForm"><div class="workout-session-fields"><label>Workout date<input id="directWorkoutDate" type="date" value="${esc(activeDay())}" required></label><label>Session notes<input id="directWorkoutSessionNotes"></label></div><div id="workoutEntryRows">${repeatEntries.length?repeatEntries.map(()=>row()).join(''):row()}</div><button type="button" class="secondary compact" id="addWorkoutRow">+ Add activity</button><p class="form-error" id="directWorkoutError" hidden></p><div class="direct-entry-actions"><button type="button" class="secondary" id="cancelDirectWorkout">Cancel</button><button type="button" class="primary" id="saveDirectWorkout">Save workout</button></div></form></div></div>`);
    const compactFields=p=>p==='strength'?`<label>Weight<input class="workout-weight" type="number" step="0.1"></label><label>Reps<input class="workout-reps" type="number"></label><label>Sets<input class="workout-sets" type="number"></label>`:p==='cardio'?`<label>Minutes<input class="workout-duration" type="number"></label><label class="workout-step-label">Steps<input class="workout-steps" type="number"></label><label>Distance (mi)<input class="workout-distance" type="number" step="0.01"></label><label>Level / intensity<input class="workout-level"></label><label>Average HR<input class="workout-average-hr" type="number"></label>`:p==='rehab'?`<label>Minutes<input class="workout-duration" type="number"></label><label>Reps<input class="workout-reps" type="number"></label><label>Sets<input class="workout-sets" type="number"></label>`:`<label>Minutes<input class="workout-duration" type="number"></label><label>Reps<input class="workout-reps" type="number"></label><label>Area / focus<input class="workout-area"></label>`;
    const bindRow=el=>{const select=el.querySelector('.workout-exercise'),profile=el.querySelector('.workout-profile'),fields=el.querySelector('.workout-profile-fields');const sync=()=>{const name=select.value==='__custom'?el.querySelector('.workout-custom-exercise').value:select.value,savedProfile=saved.find(x=>x.name===name)?.profile;profile.value=savedProfile||activityProfile(name,profile.value);fields.innerHTML=compactFields(profile.value);const stepLabel=el.querySelector('.workout-step-label');if(stepLabel)stepLabel.childNodes[0].textContent=/stair|climbmill/i.test(name)?'Stair steps':'Walking / activity steps';el.querySelector('.custom-exercise-wrap').hidden=select.value!=='__custom'};select.onchange=sync;profile.onchange=()=>fields.innerHTML=compactFields(profile.value);sync();el.querySelector('.remove-workout-row').onclick=()=>{$$('[data-workout-row]').length>1?el.remove():null}};
    $$('[data-workout-row]').forEach((el,index)=>{bindRow(el);const prev=repeatEntries[index];if(!prev)return;const select=el.querySelector('.workout-exercise');if([...select.options].some(o=>o.value===prev.exercise))select.value=prev.exercise;else{select.value='__custom';el.querySelector('.workout-custom-exercise').value=prev.exercise||'';}select.dispatchEvent(new Event('change'));const set=(sel,val)=>{const input=el.querySelector(sel);if(input&&val!=null)input.value=val};set('.workout-weight',prev.weight);set('.workout-reps',prev.reps);set('.workout-sets',prev.sets);set('.workout-duration',prev.duration_min);set('.workout-steps',prev.steps);set('.workout-distance',prev.distance_mi);set('.workout-level',prev.level);set('.workout-average-hr',prev.average_hr);set('.workout-rpe',prev.rpe);set('.workout-pain-before',prev.pain_before);set('.workout-pain-during',prev.pain_during);set('.workout-pain-after',prev.pain_after);});$('#addWorkoutRow').onclick=()=>{const h=$('#workoutEntryRows');h.insertAdjacentHTML('beforeend',row());bindRow(h.lastElementChild)};const close=()=>$('#directWorkoutModal')?.remove();$('#closeDirectWorkout').onclick=close;$('#cancelDirectWorkout').onclick=close;
    const saveWorkoutForm=async()=>{const form=$('#directWorkoutForm'),saveBtn=$('#saveDirectWorkout'),errorEl=$('#directWorkoutError');if(!form||!saveBtn||form.dataset.saving==='true')return;errorEl.hidden=true;errorEl.textContent='';form.dataset.saving='true';saveBtn.disabled=true;saveBtn.textContent='Checking…';try{const date=$('#directWorkoutDate')?.value||activeDay(),sessionNotes=$('#directWorkoutSessionNotes')?.value.trim()||'',num=(el,sel)=>{const input=el.querySelector(sel);if(!input||String(input.value).trim()==='')return null;const v=Number(input.value);return Number.isFinite(v)&&v>=0?v:null};const val=(el,sel)=>el.querySelector(sel)?.value.trim()||null;const entries=$$('[data-workout-row]').map(el=>{const select=el.querySelector('.workout-exercise'),sel=select?.value||'',custom=el.querySelector('.workout-custom-exercise')?.value.trim()||'',exercise=activityDisplayName(sel==='__custom'?custom:sel),profile=el.querySelector('.workout-profile')?.value||activityProfile(exercise);return {exercise,profile,weight:num(el,'.workout-weight'),reps:num(el,'.workout-reps'),sets:num(el,'.workout-sets'),duration_min:num(el,'.workout-duration'),steps:num(el,'.workout-steps'),stair_steps:/stair|climbmill/i.test(exercise)?num(el,'.workout-steps'):null,ambulatory_steps:!/stair|climbmill/i.test(exercise)?num(el,'.workout-steps'):null,distance_mi:num(el,'.workout-distance'),average_hr:num(el,'.workout-average-hr'),level:val(el,'.workout-level'),body_area:val(el,'.workout-area'),rpe:num(el,'.workout-rpe'),pain_before:num(el,'.workout-pain-before'),pain_during:num(el,'.workout-pain-during'),pain_after:num(el,'.workout-pain-after'),technique:val(el,'.workout-technique'),injury_context:val(el,'.workout-injury-context'),notes:val(el,'.workout-notes')||''}}).filter(x=>x.exercise);if(!entries.length)throw new Error('Choose at least one activity before saving.');const candidates=entries.map(item=>({category:'workout',timestamp:`${date}T12:00:00`,structured:{exercise:item.exercise,activity_profile:item.profile,weight:item.weight,reps:item.reps,sets:item.sets,duration_min:item.duration_min,steps:item.steps,stair_steps:item.stair_steps,ambulatory_steps:item.ambulatory_steps,distance_mi:item.distance_mi,average_hr:item.average_hr,level:item.level}}));let duplicateCount=0;for(const candidate of candidates){try{if((await ZekeData.findLikelyDuplicates(candidate,0.94)).length)duplicateCount++;}catch(scanError){console.warn('Workout duplicate check skipped:',scanError);}}if(duplicateCount&&!window.confirm(`${duplicateCount} activit${duplicateCount===1?'y looks':'ies look'} identical to an existing entry for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}. Save anyway?`)){form.dataset.saving='false';saveBtn.disabled=false;saveBtn.textContent='Save workout';return;}saveBtn.textContent='Saving…';const transactionId=(globalThis.crypto?.randomUUID?.()||`tx-${Date.now()}-${Math.random().toString(36).slice(2)}`),created=[];for(const item of entries)created.push(await ZekeData.addEvent({category:'workout',timestamp:`${date}T12:00:00`,raw_text:item.notes||sessionNotes,structured:{exercise:item.exercise,canonical_activity_id:activityKey(item.exercise).replace(/ /g,'_'),activity_profile:item.profile,workout_id:`workout-${date}-${transactionId}`,...item,notes:item.notes||sessionNotes,completed:true,interpretation_status:'confirmed'},provenance:{source:'direct-workout-entry',entry_mode:'structured-form',transaction_id:transactionId}}));state.lastSave={ids:created.map(x=>x.id),route:'fitness',label:'workout'};close();await refreshData();render();showToast(`${entries.length} activities logged for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}.`);}catch(err){console.error('Workout save failed:',err);form.dataset.saving='false';saveBtn.disabled=false;saveBtn.textContent='Save workout';errorEl.hidden=false;errorEl.textContent=`Workout was not saved: ${err?.message||'Unknown error'}`;errorEl.scrollIntoView?.({behavior:'smooth',block:'nearest'});}};$('#directWorkoutForm').onsubmit=e=>{e.preventDefault();saveWorkoutForm()};$('#saveDirectWorkout').onclick=e=>{e.preventDefault();saveWorkoutForm()};
  }
