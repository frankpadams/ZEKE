(() => {
  ...
  function coachHTML() {
    ...
    const pattern=patternCandidates?.()?.[0]||null;
    if(!selected && !timely) return `<section class="panel coach-panel"><div class="section-head"><div><div class="coach-badge">🏋 Coach's Eye</div><h2>No specific coaching yet</h2><p>Repeated sessions and recorded effort, pain, or recovery will unlock prioritized guidance.</p></div></div></section>`;
    const next=selected||timely;
    const timelyHTML=timely?`<article class="coach-lane coach-now"><span>NOW</span><div><strong>${esc(timely.title)}</strong><p>${esc(timely.suggestion)}</p></div><button class="text-action" data-coach-exercise="${esc(timely.name)}">Review</button></article>`:`<article class="coach-lane"><span>NOW</span><div><strong>Nothing time-sensitive</strong><p>No immediate fitness warning or pre-workout reminder is supported by the current record.</p></div></article>`;
    const nextHTML=next?`<article class="coach-lane"><span>NEXT SESSION</span><div><strong>${esc(next.name)}</strong><p>${esc(next.suggestion)}</p><small>${next.recent.length} recent session${next.recent.length===1?'':'s'} · ${next.score>=80?'Moderate':'Low'} confidence</small></div><button class="text-action" data-coach-exercise="${esc(next.name)}">View exercise</button></article>`:'';
    const patternHTML=`<article class="coach-lane"><span>PATTERNS</span><div><strong>${pattern?esc(pattern.title||pattern.name||'A broader pattern is ready to explore'):'Broader patterns need more paired evidence'}</strong><p>${pattern?esc(pattern.summary||pattern.description||'Open Pattern Lab for the evidence and limitations.'):'ZEKE will surface cross-activity progress, balance, recovery, or missing-data patterns here.'}</p></div><button class="text-action" data-route="pattern-lab">Pattern Lab</button></article>`;
    return `<section class="panel coach-panel coach-dashboard-structured"><div class="section-head"><div><div class="coach-badge">🏋 Coach's Eye</div><h2>Prioritized coaching</h2><p>Abbreviated guidance here; full evidence and history remain with the exercise tile.</p></div></div><div class="coach-lanes">${timelyHTML}${nextHTML}${patternHTML}</div></section>`;
  }
  ...
  function healthPageHTML() {
    ...
    const cards=shown.map(id=>`<div class="health-library-item ${state.expandedHealthMetric===id?'is-expanded':''}" data-health-metric="${esc(id)}" tabindex="0" role="button" aria-expanded="${state.expandedHealthMetric===id}"><button class="favorite-button health-favorite ${healthFavorites.has(id)?'active':''}" data-favorite-health="${esc(id)}" aria-label="${healthFavorites.has(id)?'Remove from':'Add to'} health favorites">★</button>${metricCard(id)}${state.expandedHealthMetric===id?`<div class="health-detail-extension"><p><strong>Detailed view</strong></p><p>Review the dated trend, source, reference context, and related insights above. ZEKE never treats an old result as current without showing its date.</p><div class="detail-actions"><button class="text-action" data-log-metric="${esc(id)}">+ Log new value</button><button class="text-action" data-pattern-focus="${esc(METRICS[id]?.label||id)}">Explore in Pattern Lab</button></div></div>`:''}</div>`).join('');
    const tabs=[['frequent','Frequent'],['favorites','Favorites'],['measurements','Measurements'],['labs','Labs']];
  ...
  function fitnessPageHTML() {
    ...
    const favorites=new Set(JSON.parse(localStorage.getItem('zeke.fitness.activityFavorites.v1')||localStorage.getItem('zeke-activity-favorites')||'[]'));
    ...
    let entries=[...groups.entries()].map(([name,arr])=>({name,arr,profile:arr.at(-1).activity_profile||activityProfile(name),category:canonicalActivityCategory(name,arr.at(-1).activity_profile||activityProfile(name))}));
    if(state.activityTab==='favorites')entries=entries.filter(x=>favorites.has(x.name));
    else if(ACTIVITY_TAXONOMY.some(x=>x.id===state.activityTab))entries=entries.filter(x=>x.category===state.activityTab);
    ...
    const cards=entries.map(({name,arr,profile,category})=>{const first=arr[0],last=arr.at(-1);const loadChange=first.weight&&last.weight?last.weight-first.weight:null;let recommendation='More repeated sessions are needed before recommending a change.';let confidence='low';if(arr.length>=2&&last.pain>=4){recommendation='Hold progression and review pain, technique, recovery, and clinician/PT guidance.';confidence='high';}else if(arr.length>=2&&last.weight&&last.reps>=12&&last.sets>=2){recommendation=`Consider ${last.weight+5} lb next session for 8–12 controlled reps, provided form and pain remain stable.`;confidence=arr.length>=3?'moderate':'low';}else if(arr.length>=2&&last.weight){recommendation=`Repeat ${last.weight} lb and aim to add 1–2 controlled reps before increasing load.`;confidence='moderate';}
      const chartMetric=category==='strength'?'weight':category==='cardio'?'steps':'duration_min';
      const pts=arr.filter(x=>Number(x[chartMetric])>0);const spark=pts.length>1?miniSpark(pts.map((x,i)=>({value:x[chartMetric],date:x.date,unit:chartMetric==='weight'?'lb':chartMetric==='duration_min'?'min':'steps',id:`${name}-${i}`})),chartMetric):'';
      const facts=activitySummaryFacts(first,last,category);
    ...
    return `<div class="page-head"><div><h1>Fitness</h1><p>Frequent activities, exercise-specific coaching, and evidence-linked next-session guidance.</p></div><div class="page-head-actions"><button class="secondary" id="addActivityBtn">+ Add activity</button><button class="primary" id="logWorkoutBtn">+ Log workout</button></div></div>${fitnessRangeHTML()}<div class="fitness-workspace"><div class="fitness-primary">${coachHTML()}<section class="panel fitness-insight-card compact-empty"><div class="section-head"><div><span class="tile-kicker">FITNESS INSIGHTS</span><h2>Patterns beyond one exercise</h2><p>Cross-activity progress, recovery questions, and longer-term hypotheses live in Pattern Lab.</p></div></div><button class="secondary compact" data-pattern-focus="Fitness overview">Open Pattern Lab</button></section></div><section class="panel fitness-library-panel"><div class="section-head"><div><h2>Activity library</h2><p>Frequently used activities appear first. Choose a category or favorite, then open a tile for granular detail.</p></div></div><div class="library-tabs" role="tablist">${tabs.map(([id,label])=>`<button class="library-tab ${state.activityTab===id?'active':''}" data-activity-tab="${id}" role="tab" aria-selected="${state.activityTab===id}">${label}</button>`).join('')}</div><div class="fitness-progress-grid">${cards||'<div class="empty-inline">No activities in this view yet.</div>'}</div></section><section class="panel fitness-history-panel"><div class="section-head"><div><h2>Workout history</h2><p>Entries are grouped into one workout per calendar day by default. Empty draft shells are excluded.</p></div></div>${history}</section></div>`;
  }
  ...
  function openPatternLab(focus=''){
    state.patternFocus=String(focus||'').trim();
    try{state.patternFocus?sessionStorage.setItem('zeke-pattern-focus',state.patternFocus):sessionStorage.removeItem('zeke-pattern-focus')}catch(_){}
    state.route='pattern-lab';location.hash='#/pattern-lab';render();
  }
  function patternLabPageHTML(){const data=pairedDailyData(), patterns=patternCandidates(),focus=state.patternFocus||sessionStorage.getItem('zeke-pattern-focus')||'';return `${focus?`<section class="panel pattern-focus-banner"><span class="tile-kicker">FOCUSED CONTEXT</span><h2>${esc(focus)}</h2><p>Pattern Lab opened from this activity. Broader associations remain clearly separated from exercise-specific evidence.</p></section>`:''}<div class="page-head"><div><h1>Pattern Lab</h1><p>Deterministic statistical analysis of your longitudinal records. AI may explain results, but it does not calculate them.</p></div><button class="secondary" id="runPatternLab">Run analysis</button></div><section class="pattern-summary"><div><b>${data.length}</b><span>days with structured data</span></div><div><b>${patterns.length}</b><span>exploratory associations</span></div><div><b>Guarded</b><span>minimum-data gate active</span></div></section><section class="panel"><div class="section-head"><div><h2>Exploratory associations</h2><p>These are screening results, not causal conclusions. Multiple-regression models unlock after enough overlapping observations exist.</p></div></div>${patterns.length?`<div class="pattern-grid">${patterns.map(p=>`<article class="pattern-card"><span class="confidence ${Math.abs(p.r)>.6?'high':'moderate'}">${Math.abs(p.r)>.6?'Stronger':'Possible'} association</span><h3>${esc(prettyVar(p.a))} ↔ ${esc(prettyVar(p.b))}</h3><p>${p.r>0?'They tended to rise together.':'When one was higher, the other tended to be lower.'}</p><div class="pattern-stats"><b>r = ${p.r.toFixed(2)}</b><span>n = ${p.n} paired days</span></div><small>Exploratory only; timing, missing data, and third variables may explain this pattern.</small></article>`).join('')}</div>`:'<div class="empty-inline">Not enough overlapping structured observations yet. Track repeated symptoms, exposures, sleep, and context to unlock analysis.</div>'}</section><section class="panel"><h2>Regression readiness</h2><p>ZEKE will use linear, logistic, count, lagged, and regularized models only when sample size, variation, and missingness checks pass. It will show included variables, exclusions, confidence intervals, sensitivity checks, and model limitations.</p></section>`}
  ...
  function navHTML() {
    ...
    return `<aside class="sidebar" id="sidebar"><div class="brand"><button class="brand-home" data-route="dashboard" title="Return to Dashboard"><img class="brand-logo" src="./assets/branding/zeke-mark-provisional.png" alt="Project ZEKE"></button><div><strong>PROJECT ZEKE</strong><span>One thread. Everything connected.</span></div><button class="sidebar-close" id="sidebarClose" aria-label="Close navigation">×</button></div><nav>${navItems}</nav><div class="sidebar-spacer"></div><div class="privacy-note">Private by design. Your records stay with your chosen storage provider.</div><div class="build-label">v${esc(BUILD.version)} · ${esc(BUILD.build)}</div></aside><div class="sidebar-scrim" id="sidebarScrim"></div><nav class="mobile-bottom-nav" aria-label="Primary navigation"><button class="mobile-nav-item ${state.route==='dashboard'?'active':''}" data-route="dashboard"><span>⌂</span><b>Dashboard</b></button><button class="mobile-nav-item ${state.route==='health'?'active':''}" data-route="health"><span>♡</span><b>Health</b></button><button class="mobile-nav-item ${state.route==='fitness'?'active':''}" data-route="fitness"><span>⌁</span><b>Fitness</b></button><button class="mobile-nav-item ${state.route==='pattern-lab'?'active':''}" data-route="pattern-lab"><span>▥</span><b>Pattern</b></button><button class="mobile-nav-item" id="mobileMoreButton"><span>•••</span><b>More</b></button></nav>`;
  }
