(() => {
  ...
  function activityDetailColumns(category){
    const common=[['date','Date',x=>x.date?fmtDate(x.date,{month:'short',day:'numeric'}):'—']];
    const defs={
      strength:[...common,['load','Load',x=>x.weight!=null?`${x.weight} lb`:'—'],['reps','Reps × sets',x=>x.reps!=null||x.sets!=null?`${x.reps??'—'} × ${x.sets??'—'}`:'—'],['rpe','RPE',x=>x.rpe??'—'],['pain','Pain',x=>x.pain_after??x.pain_during??x.pain??'—']],
      cardio:[...common,['duration','Duration',x=>x.duration_min!=null?`${x.duration_min} min`:'—'],['steps','Steps',x=>x.steps??'—'],['distance','Distance',x=>x.distance_mi!=null?`${x.distance_mi} mi`:'—'],['level','Level / intensity',x=>x.level??x.rpe??'—'],['pain','Pain',x=>x.pain_after??x.pain??'—']],
      ...
      recovery:[...common,['duration','Duration',x=>x.duration_min!=null?`${x.duration_min} min`:'—'],['notes','Notes',x=>x.notes||'—']],
      sport:[...common,['duration','Duration',x=>x.duration_min!=null?`${x.duration_min} min`:'—'],['steps','Steps',x=>x.steps??'—'],['distance','Distance',x=>x.distance_mi!=null?`${x.distance_mi} mi`:'—'],['rpe','RPE',x=>x.rpe??'—']],
      functional:[...common,['duration','Duration',x=>x.duration_min!=null?`${x.duration_min} min`:'—'],['steps','Steps',x=>x.steps??'—'],['rpe','RPE',x=>x.rpe??'—']]
    };
    return defs[category]||defs.strength;
  }
  function activityDetailTable(arr,category){
    const columns=activityDetailColumns(category),rows=arr.slice(-6).reverse();
  ...
  function fitnessPageHTML() {
    ...
    return `<div class="page-head"><div><h1>Fitness</h1><p>Activity-specific history, consistent graph rules, and one cautious, evidence-linked consideration per activity.</p></div><div class="page-head-actions"><button class="secondary" id="addActivityBtn">+ Add activity</button><button class="secondary" id="repeatLastWorkoutBtn">Repeat last workout</button><button class="primary" id="logWorkoutBtn">Start gym logging</button></div></div>${fitnessRangeHTML()}<div class="fitness-workspace"><div class="fitness-primary">${coachHTML()}<section class="panel fitness-insight-card compact-empty"><div class="section-head"><div><span class="tile-kicker">FITNESS INSIGHTS</span><h2>${esc((patternCandidates?.()?.[0])?`${prettyVar(patternCandidates()[0].a)} and ${prettyVar(patternCandidates()[0].b)}`:'Recovery relationships need more data')}</h2><p>${(patternCandidates?.()?.[0])?esc(`${patternCandidates()[0].r>0?'They moved in the same direction':'They moved in opposite directions'} across ${patternCandidates()[0].n} paired days. Open the dated evidence before changing training.`):'Record repeated activity plus sleep, effort, or pain before ZEKE can show a specific relationship.'}</p></div></div><button class="secondary compact" data-pattern-focus="Fitness overview">${(patternCandidates?.()?.[0])?'Review this evidence':'Open Pattern Lab'}</button></section></div><section class="panel fitness-library-panel"><div class="section-head"><div><h2>Activity library</h2><p>Every graph names its metric. When a graph cannot be drawn, the tile explains why.</p></div></div><div class="library-tabs" role="tablist">${tabs.map(([id,label])=>`<button class="library-tab ${state.activityTab===id?'active':''}" data-activity-tab="${id}" role="tab" aria-selected="${state.activityTab===id}">${label}</button>`).join('')}</div><div class="fitness-progress-grid">${cards||'<div class="empty-inline">No activities in this view yet.</div>'}</div></section><section class="panel fitness-history-panel"><div class="section-head"><div><h2>Workout history</h2><p>Edit any record to review the same optional effort, pain, technique, and injury-context fields available during initial entry.</p></div></div>${history}</section></div>`;
  }

  function providerPageHTML(){
    const focus=state.providerFocus||'pt',events=dedupeDisplayEvents(state.events.filter(recordIsActive));
    const workouts=events.filter(isWorkoutEvent).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0));
    const recent=workouts.filter(e=>new Date(e.timestamp||0)>=new Date(Date.now()-30*86400000));
    const painRows=recent.map(e=>workoutStructured(e)).filter(w=>w.pain_before!=null||w.pain_during!=null||w.pain_after!=null);
    const avg=vals=>vals.length?(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1):'Not enough data';
    const before=painRows.map(x=>Number(x.pain_before)).filter(Number.isFinite),after=painRows.map(x=>Number(x.pain_after)).filter(Number.isFinite);
    const exercises=[...new Set(recent.map(e=>workoutStructured(e).exercise).filter(Boolean))];
    const questions=(state.factors||[]).filter(f=>f.type==='clarification_question'&&!['resolved','dismissed'].includes(f.status)).slice(0,5);
    const focusCopy={pt:['Physical Therapy','Pain, function, exercise tolerance, adherence, and injury context'],primary:['Primary Care','Major changes, medications, labs, activity, symptoms, and questions'],orthopedic:['Orthopedics','Injury timeline, pain, function, imaging context, and return-to-activity progression']}[focus]||['Provider','Relevant changes and questions'];
    return `<div class="page-head provider-page-head"><div><span class="tile-kicker">VISIT VIEW</span><h1>${focusCopy[0]} summary</h1><p>${focusCopy[1]}. This view summarizes recorded data; it does not diagnose or establish causation.</p></div><div class="page-head-actions"><button class="secondary" id="printProviderSummary">Print / save PDF</button></div></div>
      <div class="provider-focus-tabs">${[['pt','PT'],['primary','Primary care'],['orthopedic','Orthopedics']].map(([id,label])=>`<button class="library-tab ${focus===id?'active':''}" data-provider-focus="${id}">${label}</button>`).join('')}</div>
      <section class="provider-summary-grid"><article class="panel provider-hero"><span class="tile-kicker">SINCE THE LAST 30 DAYS</span><h2>${recent.length} recorded workout activities</h2><p>${exercises.length} distinct activities. Pain was recorded for ${painRows.length} entries.</p><div class="provider-stat-row"><div><b>${avg(before)}</b><span>Average pain before</span></div><div><b>${avg(after)}</b><span>Average pain after</span></div><div><b>${exercises.length}</b><span>Activities represented</span></div></div></article>
      <article class="panel"><div class="section-head"><div><h2>Current activity snapshot</h2><p>Most recently recorded load, repetitions, duration, effort, and pain by activity.</p></div></div>${recent.length?`<div class="provider-activity-list">${[...new Map(recent.map(e=>[activityKey(workoutStructured(e).exercise||'Workout'),e])).values()].slice(0,8).map(e=>{const w=workoutStructured(e);return `<div><strong>${esc(w.exercise||'Workout')}</strong><span>${esc(w.weight!=null?`${w.weight} lb · ${w.reps??'—'} × ${w.sets??'—'}`:w.duration_min!=null?`${w.duration_min} min${w.steps!=null?` · ${w.steps} steps`:''}`:'Details not recorded')}</span><small>${esc(fmtDate(e.timestamp,{month:'short',day:'numeric'}))}${w.pain_after!=null?` · pain after ${esc(w.pain_after)}/10`:''}</small></div>`}).join('')}</div>`:'<div class="empty-inline">No recent workout data is available.</div>'}</article>
      <article class="panel"><div class="section-head"><div><h2>Questions for this visit</h2><p>Keep the conversation focused on decisions that need a clinician’s judgment.</p></div></div>${questions.length?questions.map(q=>`<div class="provider-question">□ ${esc(q.question||q.summary||'Review this item')}</div>`).join(''):'<div class="empty-inline">No visit questions are currently saved. Add them through Talk to ZEKE.</div>'}</article>
      <article class="panel provider-disclaimer"><h2>How to use this view</h2><p>Show it on screen or print it. Verify important values against source records. ZEKE labels observations and considerations separately and should not substitute for a clinician’s assessment.</p></article></section>`;
  }
  ...
  function navHTML() {
    const items=[['dashboard','⌂','Dashboard'],['health','♡','Health'],['fitness','⌁','Fitness'],['life-events','◎','Life & Symptoms'],['insights','💡','Insights'],['provider','▤','Provider View'],['medications','✚','Medications'],['calendar','▣','Calendar'],['questions','💬','Questions'],['settings','⚙','Settings']];
  ...
  function connectedAppHTML() {
    ...
    else if(state.route==='provider') content=providerPageHTML();
  ...
  function bind() {
    ...
    $('#profileForm')?.addEventListener('submit',e=>{e.preventDefault();const value=$('#preferredNameInput')?.value.trim()||'';const existing=userProfile();localStorage.setItem('zeke-user-profile',JSON.stringify({...existing,preferred_name:value,pronouns:$('#pronounsInput')?.value.trim()||'',gender_identity:$('#genderIdentityInput')?.value.trim()||'',sex_assigned_at_birth:$('#sexAssignedInput')?.value||'',clinical_context:$('#clinicalContextInput')?.value.trim()||''}));showToast(value?'Profile saved.':'Profile saved with a neutral greeting.');render();});
    ...
    $$('[data-provider-focus]').forEach(el=>el.onclick=()=>{state.providerFocus=el.dataset.providerFocus;localStorage.setItem('zeke-provider-focus',state.providerFocus);render()});
    $('#printProviderSummary')?.addEventListener('click',()=>window.print());
