(() => {
  ...
  function healthPageHTML() {
    ...
    return `<div class="page-head"><div><h1>Health</h1><p>Your health overview, including sleep, measurements, labs, medications, history, and context.</p></div></div>
      <section class="panel"><div class="section-head"><div><h2>Health overview</h2><p>Use + Log on any tile. Prior values are never carried forward.</p></div></div><div class="metrics-row">${cards||'<div class="empty-inline">No verified health metrics yet.</div>'}</div></section><section class="panel"><div class="section-head"><div><h2>Body measurements</h2><p>Waist, chest, hips, neck, arms, thighs, body-fat percentage, and other dated measurements.</p></div><button class="secondary" data-log-metric="waist_circumference">+ Add measurement</button></div><div class="body-measure-grid">${['waist_circumference','body_fat_pct'].map(id=>metricCard(id)).join('')}</div></section>
  ...
  function openWorkoutEntryModal(){
    ...
    const common=[...new Set([...workoutGroups().keys(),...saved.map(x=>x.name),'Stair Climber','Treadmill','Stationary Bike','Lat Pulldown','Seated Row','Chest Press','Leg Press','Leg Extension','Seated Leg Curl','Bicep Curl','Abdominal','Massage Chair','HydroMassage','Shoulder PT','Hamstring Stretch','Shoulder Mobility'])].sort((a,b)=>a.localeCompare(b));
    ...
    $('#directWorkoutForm').onsubmit=async e=>{e.preventDefault();const date=$('#directWorkoutDate').value,sessionNotes=$('#directWorkoutSessionNotes').value.trim(),num=(el,sel)=>{const v=Number(el.querySelector(sel)?.value);return Number.isFinite(v)&&v>=0?v:null};const entries=$$('[data-workout-row]').map(el=>{const sel=el.querySelector('.workout-exercise').value,exercise=sel==='__custom'?el.querySelector('.workout-custom-exercise').value.trim():sel,profile=el.querySelector('.workout-profile').value;return {exercise,profile,weight:num(el,'.workout-weight'),reps:num(el,'.workout-reps'),sets:num(el,'.workout-sets'),duration_min:num(el,'.workout-duration'),steps:num(el,'.workout-steps'),level:el.querySelector('.workout-level')?.value||null,body_area:el.querySelector('.workout-area')?.value||null,pain_before:num(el,'.workout-pain-before'),pain_after:num(el,'.workout-pain-after'),notes:el.querySelector('.workout-notes').value.trim()}}).filter(x=>x.exercise);if(!entries.length){$('#directWorkoutError').hidden=false;$('#directWorkoutError').textContent='Choose at least one activity.';return;}for(const item of entries)await ZekeData.addEvent({category:'workout',timestamp:`${date}T12:00:00`,raw_text:item.notes||sessionNotes,structured:{exercise:item.exercise,activity_profile:item.profile,workout_id:`workout-${date}`,...item,notes:item.notes||sessionNotes,completed:true,interpretation_status:'confirmed'},provenance:{source:'direct-workout-entry',entry_mode:'structured-form'}});close();await refreshData();render();showToast(`${entries.length} activities logged for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}.`)};
  }
