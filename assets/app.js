(() => {
  ...
  const BUILD = window.ZEKE_BUILD || window.ZEKE_VERSION || { version: '0.47.0', build: '2026.08.24.1', label: 'ZEKE Visual System + Preserved Functionality' };
  const state = {
  ...
  function bind() {
    ...
    $$('[data-direct-question-choice]').forEach(el=>el.onclick=async()=>{const q=state.factors.find(f=>f.id===state.activeReviewId);if(!q)return;const actions=el.closest('.review-decision-actions');actions?.querySelectorAll('button').forEach(b=>b.disabled=true);el.classList.add('is-selected','is-working');const original=el.textContent;el.textContent='Saving…';const feedback=document.createElement('div');feedback.className='review-live-feedback';feedback.setAttribute('role','status');feedback.textContent='ZEKE is applying your decision…';actions?.insertAdjacentElement('afterend',feedback);const wf=beginWorkflow(q.question||'Resolve duplicate review',{goal:'Resolve possible duplicate record',target:{question_id:q.id,question_key:q.question_key}});state.pending={type:'question',question:q,workflowId:wf?.id||state.workflowId};try{await handleQuestionChoice(el.dataset.directQuestionChoice);}catch(err){actions?.querySelectorAll('button').forEach(b=>b.disabled=false);el.classList.remove('is-working');el.textContent=original;feedback.textContent='That did not save. Nothing was changed; please try again.';throw err;}});
