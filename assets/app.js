(() => {
  ...
  function fitnessPageHTML() {
    ...
    const userCustomActivities=customActivityLibrary(),knowledgeActivities=(window.ZekeKnowledgeBase?.catalog||[]).map(a=>({name:a.name,profile:a.profile,equipment:a.equipment,movement:a.movement,knowledge:true})),customActivities=[...knowledgeActivities,...userCustomActivities],favorites=new Set(JSON.parse(localStorage.getItem('zeke.fitness.activityFavorites.v1')||localStorage.getItem('zeke-activity-favorites')||'[]')),customNames=new Set(userCustomActivities.map(a=>normalizedActivityName(a.name).toLowerCase()));
  ...
  function profileFields(profile, latest={}, name=''){
    const n=String(name||'').toLowerCase();
    if(profile==='strength')return `<label>Weight / resistance<input id="activityWeight" type="number" step="0.1" min="0" placeholder="${latest.weight??''}"></label><label>Unit<select id="activityWeightUnit"><option value="lb">lb</option><option value="kg">kg</option><option value="bowflex-unit">Bowflex setting</option><option value="band">Band / qualitative</option></select></label><label>Reps<input id="activityReps" type="number" min="1" placeholder="${latest.reps??''}"></label><label>Sets<input id="activitySets" type="number" min="1" placeholder="${latest.sets??''}"></label><label>RPE (optional)<input id="activityRpe" class="workout-rpe" type="number" min="1" max="10"></label><label>Rest seconds (optional)<input id="activityRest" type="number" min="0"></label><label class="wide">Technique notes (optional)<input id="activityTechnique" class="workout-technique" type="text" placeholder="e.g., neutral grip felt better; keep elbows low"></label><label class="wide">Injury / limitation context (optional)<input id="activityInjuryContext" class="workout-injury-context" type="text" placeholder="e.g., right shoulder comfortable; stopped before pain"></label>`;
    if(profile==='cardio'){
      const stair=/stair|climbmill|step mill/.test(n),walking=/walk|treadmill/.test(n);
      return `<label>Duration (min)<input id="activityDuration" type="number" min="1"></label>${stair?'<label>Stair steps<input id="activitySteps" type="number" min="0"></label>':'<label>Distance (mi, optional)<input id="activityDistance" type="number" min="0" step="0.01"></label>'}${walking?'<label>Steps (optional)<input id="activitySteps" type="number" min="0"></label>':''}<label>Level / resistance<input id="activityLevel" type="number" min="0" step="0.1"></label><label>Incline % (optional)<input id="activityIncline" type="number" min="0" step="0.1"></label><label>Average HR (only if measured)<input id="activityHr" type="number" min="1"></label><label>RPE (optional)<input id="activityRpe" class="workout-rpe" type="number" min="1" max="10"></label>`;
    }
    if(profile==='sport')return `<label>Duration (min)<input id="activityDuration" type="number" min="1"></label><label>Distance (mi, optional)<input id="activityDistance" type="number" min="0" step="0.01"></label><label>Effort / RPE (optional)<input id="activityRpe" class="workout-rpe" type="number" min="1" max="10"></label><label>Location / waterway (optional)<input id="activityLocation" type="text"></label><label class="wide">Conditions (optional)<input id="activityConditions" type="text" placeholder="weather, water, terrain"></label>`;
    if(profile==='rehab')return `<label>Duration (min, optional)<input id="activityDuration" type="number" min="1"></label><label>Pain before (0–10)<input id="activityPainBefore" class="workout-pain-before" type="number" min="0" max="10"></label><label>Pain during (0–10)<input id="activityPainDuring" class="workout-pain-during" type="number" min="0" max="10"></label><label>Pain after (0–10)<input id="activityPainAfter" class="workout-pain-after" type="number" min="0" max="10"></label><div class="direct-entry-hint wide">Follow the exact activity, range, and resistance provided by your clinician or PT.</div>`;
    ...
    return `<label>Duration (min)<input id="activityDuration" type="number" min="1"></label><label>Body area / focus (optional)<input id="activityArea" type="text"></label><label>RPE (optional)<input id="activityRpe" class="workout-rpe" type="number" min="1" max="10"></label><label>Pain after (0–10)<input id="activityPainAfter" class="workout-pain-after" type="number" min="0" max="10"></label>`;
  }
  ...
  function openActivityEntryModal(name, profileOverride=''){
    ...
    $('#directActivityProfile')?.addEventListener('change',e=>{$('#activityProfileFields').innerHTML=profileFields(e.target.value,latest,name)});
    $('#directExerciseForm')?.addEventListener('submit',async e=>{e.preventDefault();const date=$('#directExerciseDate').value,profile=$('#directActivityProfile').value,notes=$('#directExerciseNotes').value.trim();const num=id=>{const el=$(id);if(!el)return null;const v=Number(el.value);return Number.isFinite(v)&&v>=0?v:null};const structured={exercise:name,canonical_activity_id:window.ZekeKnowledgeBase?.get?.(name)?.id||activityKey(name).replace(/ /g,'_'),equipment:window.ZekeKnowledgeBase?.get?.(name)?.equipment||null,movement_pattern:window.ZekeKnowledgeBase?.get?.(name)?.movement||null,activity_profile:profile,workout_id:`workout-${date}`,weight:num('#activityWeight'),weight_unit:$('#activityWeight')?($('#activityWeightUnit')?.value||'lb'):'',reps:num('#activityReps'),sets:num('#activitySets'),rpe:num('#activityRpe'),rest_sec:num('#activityRest'),duration_min:num('#activityDuration'),steps:num('#activitySteps'),distance_mi:num('#activityDistance'),level:$('#activityLevel')?.value||null,incline_pct:num('#activityIncline'),average_hr:num('#activityHr'),location:$('#activityLocation')?.value||null,conditions:$('#activityConditions')?.value||null,body_area:$('#activityArea')?.value||null,pain_before:num('#activityPainBefore'),pain_during:num('#activityPainDuring'),pain_after:num('#activityPainAfter'),technique_notes:$('#activityTechnique')?.value.trim()||null,injury_context:$('#activityInjuryContext')?.value.trim()||null,completed:true,notes,interpretation_status:'confirmed'};const meaningful=Object.entries(structured).some(([k,v])=>!['exercise','activity_profile','workout_id','completed','interpretation_status','notes','weight_unit'].includes(k)&&v!=null&&v!=='')||notes||['rehab'].includes(profile);if(!meaningful){const err=$('#directExerciseError');err.hidden=false;err.textContent='Enter at least one relevant detail.';return;}await ZekeData.addEvent({category:'workout',timestamp:`${date}T12:00:00`,raw_text:notes,structured,provenance:{source:'direct-activity-entry',entry_mode:'structured-form'}});close();await refreshData();render();showToast(`${name} logged for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}.`)});
  }
  ...
  function openWorkoutEntryModal(options={}){
    ...
    const common=['Stair Climber','Treadmill','Stationary Bike','Lat Pulldown','Seated Row','Chest Press','Leg Press','Leg Extension','Seated Leg Curl','Bicep Curl','Abdominal','Shoulder Press','Tricep Press','Massage Chair','Shoulder PT','Hamstring Stretch','Shoulder Mobility'];
    ...
    const exerciseNames=[...new Map([...recentNames,...common].map(n=>[activityKey(n),activityDisplayName(n)])).values()];
    let date=activeDay();
    ...
    const historyFor=name=>workoutEvents().map(e=>({...workoutStructured(e),event:e})).filter(w=>activityKey(w.exercise)===activityKey(name)).sort((a,b)=>new Date(a.event.timestamp)-new Date(b.event.timestamp));
    ...
    const blankItem=name=>{const profile=activityProfile(name);return{exercise:activityDisplayName(name),profile,status:'not-started',source:'manual',...primaryFromLast(name,profile),rpe:'',pain:'',rest_sec:'',notes:'',event_id:null};};
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay zeke-workout-shell" id="directWorkoutModal"><div class="zeke-workout-app" role="dialog" aria-modal="true" aria-label="Mobile workout entry"><header class="zeke-workout-header"><button type="button" class="zeke-back-button" id="workoutBackBtn" aria-label="Back"><span aria-hidden="true">‹</span><span class="zeke-back-label">Workout</span></button><h2 id="workoutScreenTitle">Workout</h2><button type="button" class="icon-btn zeke-workout-more" id="closeDirectWorkout" aria-label="Exit workout entry">×</button><label class="zeke-workout-date">Date <input id="gymWorkoutDate" type="date" value="${escAttr(date)}"></label></header><main id="workoutScreen" class="zeke-workout-screen"></main></div></div>`);
    ...
    const readinessFor=(item,arr)=>{const confirmed=arr.filter(x=>x.weight!=null&&x.reps!=null&&x.sets!=null);if(confirmed.length<3)return{category:'Not enough evidence',position:'unknown',text:'ZEKE needs at least three comparable confirmed entries before suggesting an increase.',apply:null};const last3=confirmed.slice(-3),same=last3.every(x=>Number(x.weight)===Number(last3[0].weight)),stable=same&&last3.every(x=>Number(x.reps)>=Number(last3[0].reps)-1);if(stable){const next=Math.max(0,Number(last3.at(-1).weight)+5);return{category:'Progression may be reasonable',position:'progress',text:`You completed three comparable entries at about ${last3.at(-1).weight} lb with stable repetitions.`,apply:{weight:next,reps:Math.max(1,Number(last3.at(-1).reps)||10),sets:Math.max(1,Number(last3.at(-1).sets)||3)}};}return{category:'Maintain current level',position:'maintain',text:'Recent entries are not yet consistent enough to support increasing difficulty.',apply:null};};
