(() => {
  ...
  const BUILD = window.ZEKE_BUILD || window.ZEKE_VERSION || { version: '0.26.1', build: '2026.07.22.2', label: 'Fitness Navigation & Evidence Hotfix' };
  const state = {
  ...
  /* v0.26 mobile-save regression contract retained conceptually in Gym Entry Mode:
  ...
  function workoutPrograms(){
    const builtIns=[
      {id:'builtin-full-body',name:'Beginner Full Body',source:'built-in',goal:'General fitness',injury_context:[],exercises:['Leg Press','Chest Press','Lat Pulldown','Seated Leg Curl','Seated Row','Abdominal']},
      {id:'builtin-push',name:'Push Day',source:'built-in',goal:'Strength and muscle',injury_context:[],exercises:['Chest Press','Shoulder Press','Tricep Press','Pec Fly','Abdominal']},
      {id:'builtin-pull',name:'Pull Day',source:'built-in',goal:'Strength and muscle',injury_context:[],exercises:['Lat Pulldown','Seated Row','Bicep Curl','Back Extension','Abdominal']},
      {id:'builtin-legs',name:'Leg Day',source:'built-in',goal:'Lower-body strength',injury_context:[],exercises:['Leg Press','Leg Extension','Seated Leg Curl','Glute Machine','Calf Raise']},
      {id:'builtin-shoulder-aware',name:'Shoulder-Friendly Upper Body',source:'built-in',goal:'Maintain upper-body training while limiting provocative overhead work',injury_context:['shoulder'],caution:'Not physical therapy or medical clearance. Stop for sharp or increasing pain and follow clinician restrictions.',exercises:['Seated Row','Lat Pulldown','Bicep Curl','Tricep Press','Shoulder PT']},
      {id:'builtin-knee-aware',name:'Knee-Friendly Lower Body',source:'built-in',goal:'Lower-body training with conservative knee loading',injury_context:['knee'],caution:'Exercise choices still require individual tolerance and clinician guidance when injured.',exercises:['Seated Leg Curl','Glute Machine','Hip Abduction','Calf Raise','Stationary Bike']}
    ];
    const custom=Array.isArray(state.preferences?.workout_programs)?state.preferences.workout_programs:[];
    return [...builtIns,...custom];
  }
  async function saveWorkoutPrograms(programs){
    state.preferences={...(state.preferences||{}),workout_programs:programs,workout_programs_schema_version:1};
    await ZekeData.savePreferences(state.preferences);
  }
  function readinessForExercise(name,sessions=[]){
    const recent=sessions.filter(x=>!x.placeholder).slice(-3);
    if(!recent.length)return {score:20,label:'Establish baseline',reasons:['No confirmed comparable sessions yet']};
    let score=45,reasons=[];
    const pain=recent.map(x=>Number(x.pain)).filter(Number.isFinite);
    const rpe=recent.map(x=>Number(x.rpe)).filter(Number.isFinite);
    const loads=recent.map(x=>Number(x.weight)).filter(Number.isFinite);
    if(pain.length&&Math.max(...pain)>=5){score-=30;reasons.push('Recent pain was 5/10 or higher');}
    else if(pain.length&&Math.max(...pain)<=2){score+=10;reasons.push('Recent pain stayed low');}
    else reasons.push('Pain was not consistently recorded');
    if(rpe.length&&rpe.every(v=>v<=8)){score+=15;reasons.push('Recent effort remained manageable');}
    else if(rpe.some(v=>v>=9)){score-=15;reasons.push('Recent effort was near maximal');}
    else reasons.push('RPE history is incomplete');
    if(loads.length>=2&&loads.at(-1)>=loads[0]){score+=15;reasons.push('Load was stable or progressing');}
    if(recent.length>=3)score+=10;
    score=Math.max(0,Math.min(100,score));
    return {score,label:score>=75?'Ready to consider progression':score>=50?'Hold and reassess':'Do not increase yet',reasons};
  }
  ...
  function openWorkoutEntryModal(repeatLast=false){
    ...
    const draftKey=`zeke.workout.draft.${activeDay()}`;
    let draft={date:activeDay(),items:[],...(JSON.parse(sessionStorage.getItem(draftKey)||'{}'))};
    if(repeatLast&&!draft.items.length){
      const previous=dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e)&&isWorkoutEvent(e))).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0));
      const lastDate=String(previous[0]?.timestamp||'').slice(0,10);
      draft.items=lastDate?previous.filter(e=>String(e.timestamp||'').slice(0,10)===lastDate).reverse().map(e=>{const w=workoutStructured(e);return {exercise:w.exercise,profile:w.activity_profile||activityProfile(w.exercise),weight:w.weight??'',setReps:Array.isArray(w.set_reps)?w.set_reps:[w.reps??'',w.reps??'',w.reps??''].slice(0,w.sets||3),duration_min:w.duration_min??'',steps:w.steps??'',distance_mi:w.distance_mi??'',average_hr:w.average_hr??'',level:w.level??'',notes:w.notes||'',saved:true};}):[];
    }
    const persist=()=>sessionStorage.setItem(draftKey,JSON.stringify(draft));
    const options=common.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join('');
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay zeke-workout-shell" id="directWorkoutModal"><div class="zeke-workout-app" role="dialog" aria-modal="true" aria-label="Today's workout"><header class="zeke-workout-header"><button type="button" class="zeke-back-button" id="workoutBackBtn" aria-label="Back to today's workout"><span aria-hidden="true">‹</span><span class="zeke-back-label">Today's Workout</span></button><h2 id="workoutScreenTitle">Today's Workout</h2><button type="button" class="icon-btn zeke-workout-more" id="closeDirectWorkout" aria-label="Workout options">•••</button><p id="workoutScreenSubtitle" class="zeke-workout-subtitle">${esc(fmtDate(draft.date+'T12:00:00',{weekday:'long',month:'short',day:'numeric'}))}</p></header><main id="workoutScreen" class="zeke-workout-screen"></main></div></div>`);
    const screen=$('#workoutScreen'),title=$('#workoutScreenTitle'),subtitle=$('#workoutScreenSubtitle'),back=$('#workoutBackBtn');
    let currentIndex=null;
    const showSaveState=(text,kind='saved')=>`<span class="zeke-save-state ${kind}" aria-live="polite">${kind==='saved'?'✓ ':kind==='syncing'?'↻ ':'! '}${esc(text)}</span>`;
    const renderSummary=()=>{
      currentIndex=null; screen.classList.remove('is-exercise-view'); screen.classList.add('is-workout-summary'); back.hidden=true; title.textContent="Today's Workout";subtitle.textContent=fmtDate(draft.date+'T12:00:00',{weekday:'long',month:'short',day:'numeric'});
      const cards=draft.items.map((item,i)=>`<button type="button" class="zeke-workout-item" data-edit-workout-item="${i}"><span class="zeke-item-check ${item.saved?'is-saved':(item.dirty?'is-draft':'is-empty')}">${item.saved?'✓':(item.dirty?'◐':'○')}</span><span><strong>${esc(item.exercise)}</strong><small>${esc(item.profile==='strength'?[item.weight?`${item.weight} lb`:'',item.setReps?.filter(String).join(' · ')].filter(Boolean).join(' · '):[item.duration_min?`${item.duration_min} min`:'',item.steps?`${item.steps} steps`:''].filter(Boolean).join(' · ')||'Tap to enter details')}</small></span><span aria-hidden="true">›</span></button>`).join('');
      screen.innerHTML=`<section class="zeke-today-card"><div class="zeke-today-heading"><div><span class="tile-kicker">TODAY'S WORKOUT</span><h3>${draft.items.length?`${draft.items.length} exercise${draft.items.length===1?'':'s'}`:'Ready when you are'}</h3></div>${draft.items.length?showSaveState('Workout in progress','syncing'):''}</div><div class="zeke-workout-items">${cards||'<p class="zeke-empty-workout">Add an exercise when you reach a machine. You can do them in any order.</p>'}</div><div class="zeke-workout-summary-actions"><button type="button" class="primary zeke-add-exercise" id="addExerciseBtn">+ Add Exercise</button><button type="button" class="secondary" id="chooseProgramBtn">Choose Program</button><button type="button" class="secondary" id="buildProgramBtn">Build Program</button></div>${draft.items.length?'<div class="zeke-workout-final-actions"><button type="button" class="secondary danger-outline" id="cancelWorkoutBtn">Cancel Workout</button><button type="button" class="primary zeke-finish-workout" id="finishWorkoutBtn">Finish Workout</button></div>':''}</section><section class="zeke-exercise-picker" id="exercisePicker" hidden><label for="exerciseChoice">Choose an exercise</label><select id="exerciseChoice"><option value="">Select exercise</option>${options}<option value="__custom">Other / custom</option></select><input id="customExerciseName" placeholder="Exercise name" hidden><div class="zeke-picker-actions"><button type="button" class="secondary" id="cancelExercisePicker">Cancel</button><button type="button" class="primary" id="openExerciseBtn">Open Exercise</button></div></section>`;
      $('#addExerciseBtn').onclick=()=>{$('#exercisePicker').hidden=false;$('#exerciseChoice').focus()};
      $('#cancelExercisePicker').onclick=()=>{$('#exercisePicker').hidden=true};
      $('#exerciseChoice').onchange=e=>{$('#customExerciseName').hidden=e.target.value!=='__custom';if(e.target.value==='__custom')$('#customExerciseName').focus()};
      $('#openExerciseBtn').onclick=()=>{const select=$('#exerciseChoice'),name=select.value==='__custom'?$('#customExerciseName').value.trim():select.value;if(!name)return;draft.items.push({exercise:activityDisplayName(name),profile:activityProfile(name),weight:'',setReps:['','',''],duration_min:'',steps:'',distance_mi:'',average_hr:'',level:'',notes:'',saved:false,dirty:false});persist();renderExercise(draft.items.length-1)};
      $$('[data-edit-workout-item]').forEach(el=>el.onclick=()=>renderExercise(Number(el.dataset.editWorkoutItem)));
      $('#chooseProgramBtn')?.addEventListener('click',()=>{
        const programs=workoutPrograms();
        const choice=prompt('Choose a workout program by number:\n\n'+programs.map((p,i)=>`${i+1}. ${p.name}${p.injury_context?.length?' · injury-aware':''}`).join('\n'));
        const program=programs[Number(choice)-1];if(!program)return;
        if(program.caution&&!confirm(`${program.name}\n\n${program.caution}\n\nUse this program?`))return;
        draft.program_id=program.id;draft.program_name=program.name;
        draft.items=program.exercises.map(exercise=>({exercise,profile:activityProfile(exercise),weight:'',setReps:['','',''],duration_min:'',steps:'',distance_mi:'',average_hr:'',level:'',notes:'',saved:false,dirty:false}));
        persist();renderSummary();
      });
      $('#buildProgramBtn')?.addEventListener('click',async()=>{
        const name=prompt('Program name (for example, Chest Day)')?.trim();if(!name)return;
        const raw=prompt('Exercises, separated by commas')||'';const exercises=raw.split(',').map(x=>activityDisplayName(x.trim())).filter(Boolean);if(!exercises.length)return;
        const goal=prompt('Goal or notes for this program (optional)')?.trim()||'';
        const custom=(Array.isArray(state.preferences?.workout_programs)?state.preferences.workout_programs:[]).slice();
        custom.push({id:`program-${Date.now()}`,name,source:'user',goal,injury_context:[],exercises,created_at:new Date().toISOString()});
        try{await saveWorkoutPrograms(custom);showToast(`${name} saved to your connected ZEKE workspace.`);}catch(err){showToast(`Program was not saved: ${err.message}`,'error');}
      });
      $('#cancelWorkoutBtn')?.addEventListener('click',()=>{if(!confirm('Cancel this workout? Unsaved entries and the workout draft will be discarded. Saved exercise records will remain in your history.'))return;sessionStorage.removeItem(draftKey);$('#directWorkoutModal')?.remove();showToast('Workout draft canceled.');});
      $('#finishWorkoutBtn')?.addEventListener('click',async()=>{const savedItems=draft.items.filter(x=>x.saved);if(!savedItems.length&&!confirm('No exercises have been saved. Finish and discard this empty workout draft?'))return;try{if(savedItems.length)await ZekeData.addEvent({category:'workout_session',timestamp:`${draft.date}T12:00:00`,raw_text:draft.program_name||'Workout session',structured:{workout_id:`workout-${draft.date}-${Date.now()}`,program_id:draft.program_id||null,program_name:draft.program_name||null,exercise_count:savedItems.length,exercise_names:savedItems.map(x=>x.exercise),completed:true,interpretation_status:'confirmed'},provenance:{source:'gym-entry-mode',entry_mode:'structured-form',transaction_id:(globalThis.crypto?.randomUUID?.()||`tx-${Date.now()}`)}});sessionStorage.removeItem(draftKey);$('#directWorkoutModal')?.remove();await refreshData();showToast(`Workout finished with ${savedItems.length} saved exercise${savedItems.length===1?'':'s'}.`);}catch(err){showToast(`Workout was not finished: ${err.message}`,'error');}});
    };
    const renderExercise=index=>{
      currentIndex=index; screen.classList.remove('is-workout-summary'); screen.classList.add('is-exercise-view'); const item=draft.items[index];const arr=(workoutGroups().get(item.exercise)||[]).slice().sort((a,b)=>String(a.date||'').localeCompare(String(b.date||'')));const last=arr.at(-1)||{};const rec=activityRecommendation(item.exercise,arr);const profile=item.profile||activityProfile(item.exercise);
      back.hidden=false; title.textContent=item.exercise; subtitle.textContent='';
      const nums=arr.slice(-7).map(x=>Number(x.weight??x.duration_min??0)).filter(Number.isFinite), min=Math.min(...nums,0), max=Math.max(...nums,1), range=Math.max(max-min,1);
      const pts=nums.map((v,i)=>`${8+i*(84/Math.max(nums.length-1,1))},${48-((v-min)/range)*30}`).join(' ');
      const dateLabels=arr.slice(-7).map(x=>x.date?fmtDate(x.date+'T12:00:00',{month:'numeric',day:'numeric'}):'');
      const lastSets=Array.isArray(last.set_reps)&&last.set_reps.length?last.set_reps:[last.reps,last.reps,last.reps].filter(v=>v!=null);
      const recommendation=rec.suggestion||((last.weight!=null)?`You’ve been strong at ${last.weight} lb!`:'Build from a comfortable starting point.');
      const rationale=rec.rationale||((last.weight!=null)?`Try ${Number(last.weight)+5} lb today. If your final set drops below your target reps, keep ${Number(last.weight)+5} lb next session.`:'ZEKE needs more completed sessions before making a progression recommendation.');
      const readiness=readinessForExercise(item.exercise,arr);
      const entry=profile==='strength'?`<div class="mock-entry-top"><div class="mock-weight-control"><label>Weight (lbs)</label><div><button type="button" id="weightDown" aria-label="Decrease weight">−</button><input id="exerciseWeight" inputmode="decimal" type="number" step="0.1" value="${esc(item.weight||last.weight||'')}"><button type="button" id="weightUp" aria-label="Increase weight">+</button></div></div><button type="button" class="mock-form-guide-trigger" id="formGuideBtn"><span class="mock-guide-person">♙</span><span><strong>View form guide</strong><small>Setup, technique &amp;<br>common mistakes</small></span><b>›</b></button></div><div class="mock-reps-label">Reps – Enter each set</div><div class="mock-set-row first"><label>Set 1</label><input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[0]??lastSets[0]??'')}"><button type="button" id="copySetOne">Copy Set 1 to all <span>▣</span></button></div><div class="mock-set-row split"><label>Set 2</label><input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[1]??lastSets[1]??'')}"><label>Set 3</label><input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[2]??lastSets[2]??'')}"><button type="button" class="mock-remove-set" aria-label="Remove set">−</button></div><button type="button" class="mock-add-set" id="addSetBtn">＋ Add Set</button><div class="mock-entry-footer"><label>RPE (optional)<input id="exerciseRpe" type="number" min="0" max="10" step="0.5" value="${esc(item.rpe??'')}"></label><label>Rest (sec)<input id="exerciseRest" type="number" min="0" step="5" value="${esc(item.rest_sec??90)}"></label><button type="button" id="notesToggle">▢ Notes</button></div><label class="mock-notes" id="notesWrap" hidden>Notes<input id="exerciseNotes" value="${esc(item.notes||'')}"></label>`:`<div class="mock-cardio-entry"><label>Minutes<input id="exerciseDuration" type="number" value="${esc(item.duration_min)}"></label><label>${/stair|climbmill/i.test(item.exercise)?'Stair steps':'Steps'}<input id="exerciseSteps" type="number" value="${esc(item.steps)}"></label><label>Distance (mi)<input id="exerciseDistance" type="number" step="0.01" value="${esc(item.distance_mi)}"></label><label>Average HR<input id="exerciseHr" type="number" value="${esc(item.average_hr)}"></label><button type="button" class="mock-form-guide-trigger" id="formGuideBtn"><span><strong>View form guide</strong><small>Setup, technique &amp; common mistakes</small></span><b>›</b></button></div><label class="mock-notes">Notes<input id="exerciseNotes" value="${esc(item.notes||'')}"></label>`;
      screen.innerHTML=`<div class="mock-exercise-page"><section class="mock-coach-card"><div class="mock-card-title"><span class="mock-z-icon">Z</span><strong>COACH'S EYE</strong><button type="button" id="coachEvidenceBtn" aria-label="Coach evidence">ⓘ</button></div><div class="mock-coach-body"><div><h3>${esc(recommendation)}</h3><p>${esc(rationale)}</p></div><div class="mock-readiness"><div class="mock-gauge"><span></span></div><small>Readiness to progress</small><strong>${readiness.score}% · ${esc(readiness.label)}</strong><em>${esc(readiness.reasons.join(' · '))}</em></div></div><div class="mock-coach-tags"><span>◎ Goal: Strength</span><span>▥ Trend: ${arr.length>1?'Improving':'Learning'}</span><span>♢ Shoulder: Good</span></div></section><section class="mock-progress-card" id="progressionCard" role="button" tabindex="0" aria-label="Open progression history"><div class="mock-section-head"><strong>PROGRESSION</strong><button type="button" id="openProgressionBtn">View history ›</button></div><div class="mock-chart"><div class="mock-ylabels"><span>${Math.ceil(max+10)}</span><span>${Math.round((max+min)/2)}</span><span>${Math.floor(min)}</span></div><svg viewBox="0 0 100 56" preserveAspectRatio="none"><line x1="0" y1="12" x2="100" y2="12"/><line x1="0" y1="30" x2="100" y2="30"/><line x1="0" y1="48" x2="100" y2="48"/><polyline points="${pts}"/><circle cx="${nums.length?8+(nums.length-1)*(84/Math.max(nums.length-1,1)):8}" cy="${nums.length?48-((nums.at(-1)-min)/range)*30:48}" r="2"/></svg></div><div class="mock-xlabels">${dateLabels.map(x=>`<span>${esc(x)}</span>`).join('')}<span>Today</span></div><div class="mock-chart-summary"><span>Best set: ${max?`${max} lb × ${last.reps||'—'} reps`:'No prior sets'}</span><span>${arr.length} session${arr.length===1?'':'s'}${max?` at ${max} lb`:''}</span></div></section><section class="mock-last-card"><div class="mock-section-head"><strong>LAST TIME <small>${last.date?'('+fmtDate(last.date+'T12:00:00',{month:'short',day:'numeric'})+')':''}</small></strong></div><div class="mock-last-grid"><span class="mock-pill">${last.weight!=null?last.weight+' lb':'—'}</span>${lastSets.slice(0,3).map(v=>`<span class="mock-pill">${esc(v)}</span>`).join('')}<div class="mock-last-notes"><strong>▤ Notes</strong><small>${esc(last.notes||'No notes recorded.')}</small></div></div></section><section class="mock-today-card"><div class="mock-section-head"><strong>TODAY'S ENTRY</strong></div>${entry}<p class="form-error" id="exerciseSaveError" hidden></p></section><div class="mock-save-row"><button type="button" class="secondary" id="cancelExerciseBtn">Cancel</button><button type="button" class="mock-save-exercise" id="saveExerciseBtn">Save</button><div id="exerciseSaveState" class="mock-save-status">${item.saved?'<strong>✓ Saved</strong><small>Synced to Drive ☁</small>':'<strong>Not saved</strong><small>Blank fields remain unknown—not zero</small>'}</div></div></div>`;
      $('#weightDown')?.addEventListener('click',()=>{$('#exerciseWeight').value=Math.max(0,Number($('#exerciseWeight').value||0)-5)});$('#weightUp')?.addEventListener('click',()=>{$('#exerciseWeight').value=Number($('#exerciseWeight').value||0)+5});
      $('#copySetOne')?.addEventListener('click',()=>{const sets=$$('.set-reps'),v=sets[0]?.value||'';sets.slice(1).forEach(x=>x.value=v)});
      $('#addSetBtn')?.addEventListener('click',()=>{const row=document.querySelector('.mock-set-row.split');const input=document.createElement('input');input.className='set-reps';input.type='number';input.inputMode='numeric';input.placeholder='Set '+($$('.set-reps').length+1);row.parentNode.insertBefore(input,$('#addSetBtn'));});
      $('#notesToggle')?.addEventListener('click',()=>{$('#notesWrap').hidden=!$('#notesWrap').hidden});
      $('#formGuideBtn')?.addEventListener('click',()=>openFormGuide(item.exercise));$('#coachEvidenceBtn')?.addEventListener('click',()=>openEvidenceReview(`${item.exercise} coaching evidence`));
      $('#cancelExerciseBtn')?.addEventListener('click',()=>renderSummary());
      const openProgression=()=>{state.expandedActivity=item.exercise;$('#directWorkoutModal')?.remove();go('fitness');setTimeout(()=>document.querySelector(`[data-activity-name="${CSS.escape(item.exercise)}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}),100);};$('#openProgressionBtn')?.addEventListener('click',e=>{e.stopPropagation();openProgression()});$('#progressionCard')?.addEventListener('click',openProgression);$('#progressionCard')?.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openProgression()}});
      $$('.mock-today-card input,.mock-today-card textarea').forEach(el=>el.addEventListener('input',()=>{item.dirty=true;item.saved=false;persist();if(el.classList.contains('set-reps')&&el.value==='0'){el.remove();item.dirty=true;persist();}}));
      $('#saveExerciseBtn').onclick=async()=>{const btn=$('#saveExerciseBtn'),status=$('#exerciseSaveState'),error=$('#exerciseSaveError');error.hidden=true;btn.disabled=true;btn.textContent='Saving…';try{item.notes=$('#exerciseNotes')?.value.trim()||'';item.rpe=$('#exerciseRpe')?.value===''?null:Number($('#exerciseRpe')?.value);item.rest_sec=$('#exerciseRest')?.value===''?null:Number($('#exerciseRest')?.value);if(profile==='strength'){item.weight=$('#exerciseWeight')?.value??'';item.setReps=$$('.set-reps').map(x=>x.value).filter(v=>v!==''&&v!=='0');item.reps=item.setReps.length?Math.round(item.setReps.reduce((a,b)=>a+Number(b),0)/item.setReps.length):null;item.sets=item.setReps.length||null;}else{item.duration_min=$('#exerciseDuration')?.value??'';item.steps=$('#exerciseSteps')?.value??'';item.distance_mi=$('#exerciseDistance')?.value??'';item.average_hr=$('#exerciseHr')?.value??'';}const transactionId=(globalThis.crypto?.randomUUID?.()||`tx-${Date.now()}`);await ZekeData.addEvent({category:'workout',timestamp:`${draft.date}T12:00:00`,raw_text:item.notes,structured:{exercise:item.exercise,canonical_activity_id:activityKey(item.exercise).replace(/ /g,'_'),activity_profile:profile,workout_id:`workout-${draft.date}`,program_id:draft.program_id||null,program_name:draft.program_name||null,weight:item.weight===''?null:Number(item.weight),reps:item.reps??null,sets:item.sets??null,set_reps:item.setReps?.length?item.setReps.map(Number):null,duration_min:item.duration_min===''?null:Number(item.duration_min),steps:item.steps===''?null:Number(item.steps),distance_mi:item.distance_mi===''?null:Number(item.distance_mi),average_hr:item.average_hr===''?null:Number(item.average_hr),rpe:item.rpe,rest_sec:item.rest_sec,notes:item.notes,completed:true,interpretation_status:'confirmed'},provenance:{source:'gym-entry-mode',entry_mode:'structured-form',transaction_id:transactionId,field_semantics:'blank-is-unknown-zero-is-explicit'}});item.saved=true;item.dirty=false;persist();status.innerHTML='<strong>✓ Saved</strong><small>Syncing to connected storage…</small>';btn.textContent='Saved';await refreshData();status.innerHTML='<strong>✓ Saved</strong><small>Synced to Drive ☁</small>';setTimeout(()=>renderSummary(),650);}catch(err){btn.disabled=false;btn.textContent='Try Save Again';status.innerHTML='<strong>Save failed</strong><small>Nothing was marked saved</small>';error.hidden=false;error.textContent=`Exercise was not saved: ${err?.message||'Unknown error'}`;}};
    };
    function guideMediaFor(exercise){
      const library=globalThis.ZekeExerciseGuides;
      if(library&&typeof library.get==='function')return library.get(exercise);
      return {name:exercise||'Exercise',profile:'strength',level:'Varies',targets:['Guide unavailable'],equipment:'Confirm the exact exercise',photo:null,setup:['Confirm the exact machine and settings before beginning.'],movement:['Use a controlled, comfortable range of motion.'],mistakes:['Do not assume a different machine variation is identical.'],tips:['Stop for sharp, catching, or increasing pain.']};
    }
    const guideBullets=(items=[])=>`<ul class="mock-guide-detail-list">${items.map(item=>`<li>${esc(item)}</li>`).join('')}</ul>`;
    const guideSection=(title,icon,items=[])=>`<section class="mock-guide-detail"><h3><span aria-hidden="true">${icon}</span>${esc(title)}</h3>${guideBullets(items)}</section>`;
    const openFormGuide=exercise=>{
      const guide=guideMediaFor(exercise);
      const photo=guide.photo;
      const photoMarkup=photo?`<figure class="mock-guide-photo"><img id="formGuidePhoto" src="${esc(photo.src)}" alt="${esc(photo.alt||`${guide.name} demonstration`)}" loading="lazy" referrerpolicy="no-referrer"><div class="mock-guide-photo-fallback" id="formGuidePhotoFallback" hidden><strong>Photo unavailable</strong><span>This guide photo requires an internet connection.</span><a href="${esc(photo.source)}" target="_blank" rel="noopener noreferrer">Open the licensed source</a></div><figcaption>Photo: <a href="${esc(photo.source)}" target="_blank" rel="noopener noreferrer">${esc(photo.credit)}</a> · <a href="${esc(photo.license?.url||photo.source)}" target="_blank" rel="noopener noreferrer">${esc(photo.license?.name||'Source license')}</a></figcaption></figure>`:`<div class="mock-guide-photo mock-guide-photo-missing"><strong>No reviewed photo is available for this exact exercise.</strong><span>Confirm the exercise or machine variation before relying on the guide.</span></div>`;
      const overview=()=>`<div class="mock-guide-overview mock-guide-rich-overview">${photoMarkup}<ul class="mock-guide-facts"><li><b>Targets:</b> ${esc((guide.targets||[]).join(', '))}</li><li><b>Equipment:</b> ${esc(guide.equipment||'Varies')}</li><li><b>Type:</b> ${esc(guide.profile==='cardio'?'Cardio':'Strength')}</li><li><b>Level:</b> ${esc(guide.level||'Varies')}</li></ul><section class="mock-guide-key-points"><h3>KEY POINTS</h3><article><i aria-hidden="true">1</i><div><strong>Set up</strong><p>${esc((guide.setup||[])[0]||'Confirm the machine setup and starting position.')}</p></div></article><article><i aria-hidden="true">2</i><div><strong>Movement</strong><p>${esc((guide.movement||[])[0]||'Use a controlled range of motion.')}</p></div></article><article><i aria-hidden="true">!</i><div><strong>Watch for</strong><p>${esc((guide.mistakes||[])[0]||'Avoid momentum and poor alignment.')}</p></div></article><article><i aria-hidden="true">✓</i><div><strong>Useful cue</strong><p>${esc((guide.tips||[])[0]||'Choose a load that preserves repeatable form.')}</p></div></article></section><p class="mock-guide-safety"><strong>Safety:</strong> Educational guidance only. It is not diagnosis, medical clearance, or physical therapy. Follow clinician restrictions and stop for sharp, catching, or increasing pain.</p></div>`;
      const tabs={
        overview,
        setup:()=>guideSection('Setup','1',guide.setup),
        movement:()=>guideSection('Movement','2',guide.movement),
        mistakes:()=>guideSection('Common mistakes','!',guide.mistakes),
        tips:()=>guideSection('Tips','✓',guide.tips)
      };
      document.body.classList.add('zeke-sheet-open');
      document.body.insertAdjacentHTML('beforeend',`<div class="zeke-sheet-scrim" id="formGuideSheet"><section class="zeke-form-sheet mock-form-sheet" role="dialog" aria-modal="true" aria-label="Form guide for ${esc(guide.name||exercise)}"><div class="zeke-sheet-handle"></div><header><h2>${esc(guide.name||exercise)} – Form Guide</h2><button type="button" id="closeFormGuide" aria-label="Close">×</button></header><nav class="zeke-guide-tabs" aria-label="Form guide sections"><button class="active" data-guide-tab="overview"><span aria-hidden="true">▣</span><small>Overview</small></button><button data-guide-tab="setup"><span aria-hidden="true">1</span><small>Setup</small></button><button data-guide-tab="movement"><span aria-hidden="true">2</span><small>Movement</small></button><button data-guide-tab="mistakes"><span aria-hidden="true">!</span><small>Common Mistakes</small></button><button data-guide-tab="tips"><span aria-hidden="true">✓</span><small>Tips</small></button></nav><div id="guideContent" aria-live="polite">${overview()}</div><button type="button" class="mock-got-it" id="gotItGuide">Got it</button></section></div>`);
      const close=()=>{$('#formGuideSheet')?.remove();document.body.classList.remove('zeke-sheet-open')};
      const attachPhotoFallback=()=>{const image=$('#formGuidePhoto'),fallback=$('#formGuidePhotoFallback');if(image&&fallback)image.addEventListener('error',()=>{image.hidden=true;fallback.hidden=false;},{once:true});};
      attachPhotoFallback();
      $('#closeFormGuide').onclick=close;$('#gotItGuide').onclick=close;$('#formGuideSheet').onclick=e=>{if(e.target.id==='formGuideSheet')close()};
      $$('[data-guide-tab]').forEach(btn=>btn.onclick=()=>{$$('[data-guide-tab]').forEach(x=>x.classList.toggle('active',x===btn));const render=tabs[btn.dataset.guideTab]||overview;$('#guideContent').innerHTML=render();attachPhotoFallback();});
    };
    back.onclick=renderSummary;$('#closeDirectWorkout').onclick=()=>{$('#directWorkoutModal')?.remove()};persist();renderSummary();
  }
