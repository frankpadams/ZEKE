(() => {
  ...
  function truthfulRecentActivityHTML(){
    ...
    const iconFor=(cat,w)=>dashboardIcon(w?'workout':cat==='sleep'?'moon':cat==='medication'?'med':cat==='immunotherapy'||cat==='vaccination'?'med':cat==='lab'?'doc':'heart');
  ...
  function dashboardIcon(name, cls=''){
    const p={status:'<path d="M4 19V9l4-4 4 4 4-4 4 4v10"/><path d="M7 16l3-3 3 2 4-5"/>',workout:'<path d="M6 9v6M3 10v4M18 9v6M21 10v4M6 12h12"/>',heart:'<path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z"/>',moon:'<path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8Z"/>',scale:'<path d="M6 19h12a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2Z"/><path d="M9 9a3 3 0 0 1 6 0M12 9l2-2"/>',calendar:'<path d="M6 2v4M18 2v4M3 9h18M5 4h14a2 2 0 0 1 2 2v14H3V6a2 2 0 0 1 2-2Z"/>',chat:'<path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4Z"/><path d="M8 10h8M8 14h5"/>',doc:'<path d="M6 2h8l4 4v16H6Z"/><path d="M14 2v5h5M9 12h6M9 16h6"/>',med:'<path d="m10 4 10 10-6 6L4 10Z"/><path d="m7 13 6-6"/>',activity:'<path d="M3 12h4l2-6 4 12 2-6h6"/>',insight:'<path d="M9 18h6M10 22h4M8.5 15.5A7 7 0 1 1 15.5 15.5C14.5 16.4 14 17 14 18h-4c0-1-.5-1.6-1.5-2.5Z"/>',target:'<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/>',arrow:'<path d="m9 18 6-6-6-6"/>'};
    return `<svg class="zeke-ui-icon ${cls}" viewBox="0 0 24 24" aria-hidden="true">${p[name]||p.activity}</svg>`;
  }
  function dashboardQuickActionsHTML(){return `<section class="panel mock-quick"><div class="mock-section-title"><span class="mock-title-icon">${dashboardIcon('activity')}</span><h2>Quick Actions</h2></div><div class="mock-quick-grid"><button data-route="fitness"><span>${dashboardIcon('workout')}</span><strong>Start<br>Workout</strong></button><button data-health-log="measurement"><span>${dashboardIcon('heart')}</span><strong>Log<br>Health</strong></button><button data-route="calendar"><span>${dashboardIcon('calendar')}</span><strong>View<br>Calendar</strong></button><button data-global-talk-open><span>${dashboardIcon('chat')}</span><strong>Talk to<br>ZEKE</strong></button></div></section>`;}
  function dashboardStatusHTML(){const weight=latestMetric('weight'),sleep=latestMetric('sleep_duration'),hr=latestMetric('resting_hr');const pain=dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e)&&['injury','symptom'].includes(semanticCategory(e)))).sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp))[0];const stat=(icon,label,value)=>`<div class="mock-stat"><span>${dashboardIcon(icon)}</span><div><small>${label}</small><strong>${esc(value||'—')}</strong></div></div>`;return `<section class="panel mock-status"><div class="mock-section-title"><span class="mock-title-icon">${dashboardIcon('status')}</span><h2>Today’s Status</h2><span class="mock-good">On Track</span></div><p class="mock-status-copy">${pain?'Current injury and symptom context is being considered.':'Your current record is ready for today’s decisions.'}</p><div class="mock-stat-row">${stat('heart','Resting HR',hr?`${hr.value} ${hr.unit||'bpm'}`:'—')}${stat('moon','Sleep',sleep?`${sleep.value} ${sleep.unit||'h'}`:'—')}${stat('scale','Weight',weight?`${weight.value} ${weight.unit||'lb'}`:'—')}</div></section>`;}
  function dashboardInsightsCompactHTML(){const timely=coachInsight(),sleep=latestMetric('sleep_duration'),weight=latestMetric('weight'),rows=[];if(timely&&timely.score>=55)rows.push(['workout',timely.title,timely.suggestion,'fitness']);if(sleep)rows.push(['moon','Sleep context is available',`Latest recorded sleep: ${sleep.value} ${sleep.unit||'h'}.`,'health']);if(weight)rows.push(['heart','Weight trend is current',`Latest verified weight: ${weight.value} ${weight.unit||'lb'}.`,'health']);if(!rows.length)rows.push(['insight','No new cross-domain insight yet','ZEKE will surface one when the evidence supports it.','insights']);return `<section class="panel mock-insights"><div class="mock-section-title"><span class="mock-title-icon">${dashboardIcon('insight')}</span><h2>ZEKE Insights</h2><button class="text-action" data-route="insights">View All</button></div><div class="mock-insight-list">${rows.slice(0,3).map(([i,t,d,r])=>`<button class="mock-insight-row" data-route="${r}"><span>${dashboardIcon(i)}</span><div><strong>${esc(t)}</strong><small>${esc(d)}</small></div>${dashboardIcon('arrow','row-arrow')}</button>`).join('')}</div></section>`;}
  function dashboardGoalsHTML(){const goals=state.factors.filter(f=>f.status!=='dismissed'&&/goal/i.test(String(f.type||''))).slice(0,3);return `<section class="panel mock-goals"><div class="mock-section-title"><span class="mock-title-icon">${dashboardIcon('target')}</span><h2>Your Goals</h2><button class="text-action" data-route="health">View</button></div>${goals.length?`<div class="mock-goal-list">${goals.map((g,i)=>`<div><strong>${esc(g.summary||g.answer||g.value||'Goal')}</strong><span><i style="width:${[68,48,76][i]||55}%"></i></span></div>`).join('')}</div>`:'<div class="mock-empty">No active goals yet. <button class="text-action" data-route="health">Add a goal</button></div>'}</section>`;}
  function dashboardHTML(){const repairs=repairCandidates();return `<div class="dashboard-mockup-shell"><section class="mock-grid mock-top-row"><div class="span-4">${dashboardStatusHTML()}</div><div class="span-5">${upcomingHTML()}</div><div class="span-3">${dashboardQuickActionsHTML()}</div></section><section class="mock-grid mock-main-row"><div class="span-4">${truthfulRecentActivityHTML()}</div><div class="span-5">${dashboardInsightsCompactHTML()}</div><div class="span-3">${healthGlanceHTML(4)}</div></section><section class="mock-grid mock-bottom-row"><div class="span-9">${timelineSnapshotHTML()}</div><div class="span-3">${dashboardGoalsHTML()}</div></section><section class="mock-grid mock-guidance-row"><div class="span-6">${coachHTML()}</div><div class="span-6">${weeklyPlanHTML()}</div></section>${repairs.length?`<div class="mock-integrity-alert">${reviewStatusHTML(repairs)}</div>`:''}</div>`;}
