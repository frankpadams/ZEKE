(() => {
  ...
  const BUILD = window.ZEKE_BUILD || window.ZEKE_VERSION || { version: '0.27.3', build: '2026.07.22.3', label: 'Mobile Exercise Workspace' };
  const state = {
  ...
  function settingsPageHTML() {
    return `<div class="page-head"><div><h1>Settings</h1><p>Connections and preferences. ZEKE's router and provider managers handle the technical choices.</p></div></div>
      ...
      <section class="panel settings-section"><div class="section-head"><div><h2>Form Guide</h2><p>Choose the model used in licensed exercise demonstrations. Exercise accuracy takes priority when no exact match is available.</p></div></div><form id="guideModelForm" class="guide-settings-form"><label>Guide model<select id="guideModelInput"><option value="match-profile" ${guidePreferences().model==='match-profile'?'selected':''}>Match my profile</option><option value="man" ${guidePreferences().model==='man'?'selected':''}>Man</option><option value="woman" ${guidePreferences().model==='woman'?'selected':''}>Woman</option><option value="neutral" ${guidePreferences().model==='neutral'?'selected':''}>Gender-neutral</option><option value="no-preference" ${guidePreferences().model==='no-preference'?'selected':''}>No preference</option></select></label><label>Visual style<select id="guideStyleInput"><option value="photo" ${guidePreferences().style==='photo'?'selected':''}>Photograph</option><option value="illustration" ${guidePreferences().style==='illustration'?'selected':''}>Illustration</option><option value="either" ${guidePreferences().style==='either'?'selected':''}>Either</option></select></label><label class="guide-image-toggle"><input id="guideImagesInput" type="checkbox" ${guidePreferences().show_images!==false?'checked':''}> Show licensed images</label><button class="primary compact" type="submit">Save preferences</button></form><p class="safety-copy">Every image displays its creator, source, license, and license-verification date.</p></section>
  ...
  function guidePreferences(){return state.preferences?.guide_model||{model:'match-profile',style:'photo',show_images:true}}
  function inferredGuideModel(){const pref=guidePreferences().model;if(pref!=='match-profile')return pref;const t=`${userProfile().gender_identity||''} ${userProfile().pronouns||''} ${userProfile().sex_assigned_at_birth||''}`.toLowerCase();if(/male|man|he\/him/.test(t))return 'man';if(/female|woman|she\/her/.test(t))return 'woman';return 'neutral';}
  const GUIDE_LIBRARY=[{keys:['lat pulldown','lat pull down','pulldown'],model:'man',style:'photo',src:'https://upload.wikimedia.org/wikipedia/commons/thumb/3/33/Amer-Lat-Pulldown.jpg/960px-Amer-Lat-Pulldown.jpg',creator:'Abooyeah',license:'CC BY-SA 4.0',source:'https://commons.wikimedia.org/wiki/File:Amer-Lat-Pulldown.jpg',license_url:'https://creativecommons.org/licenses/by-sa/4.0/',verified:'2026-07-22'},{keys:['chest press','seated chest press'],model:'woman',style:'photo',src:'https://upload.wikimedia.org/wikipedia/commons/thumb/2/22/Girl_doing_chest_press_machine_exercise.jpg/960px-Girl_doing_chest_press_machine_exercise.jpg',creator:'Tyler Read / PTPioneer',license:'CC BY 2.0',source:'https://commons.wikimedia.org/wiki/File:Girl_doing_chest_press_machine_exercise.jpg',license_url:'https://creativecommons.org/licenses/by/2.0/',verified:'2026-07-22'}];
  function guideMediaFor(exercise){if(!guidePreferences().show_images)return null;const n=String(exercise||'').toLowerCase(),m=GUIDE_LIBRARY.filter(x=>x.keys.some(k=>n.includes(k)));if(!m.length)return null;const model=inferredGuideModel();return m.find(x=>x.model===model)||m[0];}
  ...
  function openWorkoutEntryModal(repeatLast=false){
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay zeke-workout-shell" id="directWorkoutModal"><div class="zeke-workout-app" role="dialog" aria-modal="true" aria-label="Today's workout"><header class="zeke-workout-header"><button type="button" class="zeke-back-button" id="workoutBackBtn" aria-label="Back to today's workout">‹</button><div><span class="tile-kicker">FITNESS</span><h2 id="workoutScreenTitle">Today's Workout</h2><p id="workoutScreenSubtitle">${esc(fmtDate(draft.date+'T12:00:00',{weekday:'long',month:'short',day:'numeric'}))}</p></div><button type="button" class="icon-btn" id="closeDirectWorkout" aria-label="Close workout">×</button></header><main id="workoutScreen" class="zeke-workout-screen"></main></div></div>`);
    ...
    const renderSummary=()=>{
      currentIndex=null; back.hidden=true; title.textContent="Today's Workout";subtitle.textContent=fmtDate(draft.date+'T12:00:00',{weekday:'long',month:'short',day:'numeric'});
    ...
    const renderExercise=index=>{
      currentIndex=index;const item=draft.items[index];back.hidden=false;title.textContent=item.exercise;subtitle.textContent='Exercise Workspace';
      const arr=workoutGroups().get(item.exercise)||[];const last=arr.at(-1)||{};const rec=activityRecommendation(item.exercise,arr);const profile=item.profile||activityProfile(item.exercise);const points=arr.slice(-6).map((x,i)=>({x:i,y:Number(x.weight??x.duration_min??0)})).filter(x=>x.y);const max=Math.max(...points.map(x=>x.y),1);const poly=points.map((p,i)=>`${10+(i*(80/Math.max(points.length-1,1)))},${48-(p.y/max*34)}`).join(' ');
      const entry=profile==='strength'?`<div class="zeke-weight-row"><label>Weight <span>lb</span><input id="exerciseWeight" inputmode="decimal" type="number" step="0.1" value="${esc(item.weight)}"></label></div><div class="zeke-set-heading"><strong>Sets and reps</strong><button type="button" class="text-action" id="copySetOne">Copy Set 1 to all</button></div><div class="zeke-set-grid"><label>Set 1<input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[0]??'')}"></label><label>Set 2<input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[1]??'')}"></label><label>Set 3<input class="set-reps" inputmode="numeric" type="number" value="${esc(item.setReps?.[2]??'')}"></label></div>`:`<div class="zeke-cardio-grid"><label>Minutes<input id="exerciseDuration" inputmode="numeric" type="number" value="${esc(item.duration_min)}"></label><label>${/stair|climbmill/i.test(item.exercise)?'Stair steps':'Steps'}<input id="exerciseSteps" inputmode="numeric" type="number" value="${esc(item.steps)}"></label><label>Distance (mi)<input id="exerciseDistance" inputmode="decimal" type="number" step="0.01" value="${esc(item.distance_mi)}"></label><label>Average HR<input id="exerciseHr" inputmode="numeric" type="number" value="${esc(item.average_hr)}"></label></div>`;
      screen.innerHTML=`<section class="zeke-coach-card"><div class="zeke-section-label">COACH'S EYE</div><h3>${esc(rec.suggestion||'Use your last comfortable performance as today’s starting point.')}</h3><p>${esc(rec.rationale||'This is based on the workout history currently available to ZEKE.')}</p><button type="button" class="text-action" id="coachEvidenceBtn">Why this? Evidence</button></section><section class="zeke-progress-card"><div><span class="zeke-section-label">PROGRESSION</span><h3>${esc(item.exercise)}</h3><p>${arr.length?`${arr.length} recorded session${arr.length===1?'':'s'}`:'Not enough history for a trend yet.'}</p></div><svg viewBox="0 0 100 55" role="img" aria-label="Recent progression"><polyline points="${poly}" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></polyline></svg></section><section class="zeke-last-card"><span class="zeke-section-label">LAST WORKOUT</span><div><strong>${last.date?esc(fmtDate(last.date+'T12:00:00',{month:'short',day:'numeric'})):'No prior workout'}</strong><span>${last.weight!=null?`${esc(last.weight)} lb · ${esc(last.reps??'—')} × ${esc(last.sets??'—')}`:last.duration_min!=null?`${esc(last.duration_min)} min`:''}</span></div></section><section class="zeke-entry-card"><div class="zeke-entry-title"><div><span class="zeke-section-label">TODAY'S ENTRY</span><h3>Record what you actually did</h3></div><button type="button" class="secondary compact" id="formGuideBtn">Form Guide</button></div>${entry}<label class="zeke-notes">Notes<input id="exerciseNotes" value="${esc(item.notes||'')}" placeholder="Optional"></label><p class="form-error" id="exerciseSaveError" hidden></p><button type="button" class="primary zeke-save-exercise" id="saveExerciseBtn">${item.saved?'Update Exercise':'Save Exercise'}</button><div id="exerciseSaveState">${item.saved?showSaveState('Saved to today’s workout','saved'):''}</div></section>`;
      $('#copySetOne')?.addEventListener('click',()=>{const sets=$$('.set-reps'),v=sets[0]?.value||'';sets.slice(1).forEach(x=>x.value=v)});
      $('#formGuideBtn').onclick=()=>openFormGuide(item.exercise);
      $('#coachEvidenceBtn').onclick=()=>openEvidenceReview(`${item.exercise} coaching evidence`);
      $('#saveExerciseBtn').onclick=async()=>{const btn=$('#saveExerciseBtn'),status=$('#exerciseSaveState'),error=$('#exerciseSaveError');error.hidden=true;btn.disabled=true;btn.textContent='Saving…';status.innerHTML=showSaveState('Saving locally…','syncing');try{item.notes=$('#exerciseNotes').value.trim();if(profile==='strength'){item.weight=$('#exerciseWeight').value;item.setReps=$$('.set-reps').map(x=>x.value);item.reps=item.setReps.filter(Boolean).length?Math.round(item.setReps.filter(Boolean).reduce((a,b)=>a+Number(b),0)/item.setReps.filter(Boolean).length):null;item.sets=item.setReps.filter(Boolean).length;}else{item.duration_min=$('#exerciseDuration')?.value||'';item.steps=$('#exerciseSteps')?.value||'';item.distance_mi=$('#exerciseDistance')?.value||'';item.average_hr=$('#exerciseHr')?.value||'';}const transactionId=(globalThis.crypto?.randomUUID?.()||`tx-${Date.now()}`);await ZekeData.addEvent({category:'workout',timestamp:`${draft.date}T12:00:00`,raw_text:item.notes,structured:{exercise:item.exercise,canonical_activity_id:activityKey(item.exercise).replace(/ /g,'_'),activity_profile:profile,workout_id:`workout-${draft.date}`,weight:item.weight===''?null:Number(item.weight),reps:item.reps??null,sets:item.sets??null,set_reps:item.setReps?.filter(Boolean).map(Number)||null,duration_min:item.duration_min===''?null:Number(item.duration_min),steps:item.steps===''?null:Number(item.steps),stair_steps:/stair|climbmill/i.test(item.exercise)&&item.steps!==''?Number(item.steps):null,distance_mi:item.distance_mi===''?null:Number(item.distance_mi),average_hr:item.average_hr===''?null:Number(item.average_hr),notes:item.notes,completed:true,interpretation_status:'confirmed'},provenance:{source:'mobile-exercise-workspace',entry_mode:'structured-form',transaction_id:transactionId}});item.saved=true;persist();status.innerHTML=showSaveState('Saved to today’s workout','saved');btn.textContent='Saved';setTimeout(async()=>{await refreshData();renderSummary();},550);}catch(err){btn.disabled=false;btn.textContent='Try Save Again';status.innerHTML=showSaveState('Save failed','error');error.hidden=false;error.textContent=`Exercise was not saved: ${err?.message||'Unknown error'}`;}};
    };
    const openFormGuide=exercise=>{const media=guideMediaFor(exercise);const mediaHTML=media?`<figure class="zeke-guide-media"><img src="${esc(media.src)}" alt="${esc(exercise)} demonstration" loading="lazy"><figcaption>Image: <a href="${esc(media.source)}" target="_blank" rel="noopener noreferrer">${esc(media.creator)}</a> · <a href="${esc(media.license_url)}" target="_blank" rel="noopener noreferrer">${esc(media.license)}</a> · checked ${esc(media.verified)}</figcaption></figure>`:`<div class="zeke-form-figure">${esc(exercise)}<br><span>No reviewed image available yet</span></div>`;document.body.classList.add('zeke-sheet-open');document.body.insertAdjacentHTML('beforeend',`<div class="zeke-sheet-scrim" id="formGuideSheet"><section class="zeke-form-sheet" role="dialog" aria-modal="true" aria-label="Form guide for ${esc(exercise)}"><div class="zeke-sheet-handle"></div><header><div><span class="zeke-section-label">FORM GUIDE</span><h2>${esc(exercise)}</h2></div><button type="button" class="icon-btn" id="closeFormGuide">×</button></header><nav class="zeke-guide-tabs"><button class="active" data-guide-tab="overview">Overview</button><button data-guide-tab="setup">Setup</button><button data-guide-tab="movement">Movement</button><button data-guide-tab="mistakes">Mistakes</button><button data-guide-tab="tips">Tips</button></nav><div id="guideContent" class="zeke-guide-content"><h3>Overview</h3><p>Use a controlled range of motion and a load that allows stable, repeatable technique. Stop if the movement causes sharp or increasing pain.</p>${mediaHTML}</div><button type="button" class="primary" id="gotItGuide">Got it</button></section></div>`);const close=()=>{$('#formGuideSheet')?.remove();document.body.classList.remove('zeke-sheet-open')};$('#closeFormGuide').onclick=close;$('#gotItGuide').onclick=close;$('#formGuideSheet').onclick=e=>{if(e.target.id==='formGuideSheet')close()};const content={overview:['Overview','Use a controlled range of motion and a load that allows stable, repeatable technique.'],setup:['Setup','Adjust the seat, pads, and handles so the machine aligns naturally with your joints.'],movement:['Movement','Move smoothly through the comfortable range. Pause briefly, then return under control.'],mistakes:['Common mistakes','Avoid bouncing, using momentum, rushing the return, or changing posture to finish a repetition.'],tips:['Tips','Start conservatively, keep the same setup from session to session, and record pain or unusual strain.']};$$('[data-guide-tab]').forEach(btn=>btn.onclick=()=>{$$('[data-guide-tab]').forEach(x=>x.classList.toggle('active',x===btn));const [h,p]=content[btn.dataset.guideTab];$('#guideContent').innerHTML=`<h3>${h}</h3><p>${p}</p>${mediaHTML}`;});};
  ...
  function bind() {
    ...
    $('#guideModelForm')?.addEventListener('submit',async e=>{e.preventDefault();state.preferences={...state.preferences,guide_model:{model:$('#guideModelInput')?.value||'match-profile',style:$('#guideStyleInput')?.value||'photo',show_images:!!$('#guideImagesInput')?.checked,updated_at:new Date().toISOString()}};await ZekeData.savePreferences(state.preferences);showToast('Form Guide preferences saved.');render();});
