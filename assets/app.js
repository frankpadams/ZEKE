(() => {
  ...
  async function ensureAssumedMedicationDoses(){
    const actions=(state.actions.catalog||[]).filter(a=>a.active!==false&&a.kind==='medication'&&a.adherence_mode==='assume_scheduled'&&a.assumption_effective_date&&a.schedule);
    if(!actions.length)return 0;let added=0;
    for(const action of actions){const canonical=ZekeParser.canonicalMedicationId(action.label||action.name||''),related=state.events.filter(e=>recordIsActive(e)&&semanticCategory(e)==='medication'&&ZekeParser.canonicalMedicationId(e.structured?.medication_name||e.structured?.name||'')===canonical),existingDays=new Set(related.map(e=>String(e.timestamp||e.recorded_at||'').slice(0,10))),latest=related.map(e=>String(e.timestamp||e.recorded_at||'').slice(0,10)).filter(Boolean).sort().at(-1),effective=[action.assumption_effective_date,action.start_date].filter(Boolean).sort().at(-1)||activeDay();let cursor=new Date(`${effective}T12:00:00`);if(latest&&latest>=effective){cursor=new Date(`${latest}T12:00:00`);cursor.setDate(cursor.getDate()+1);}const today=new Date(`${activeDay()}T12:00:00`);for(let guard=0;cursor<=today&&guard<180;guard++,cursor.setDate(cursor.getDate()+1)){if(!actionDateMatches(action,cursor))continue;const day=localDateISO(cursor);if(existingDays.has(day))continue;const created=await ZekeData.addEvent({category:'medication',timestamp:`${day}T12:00:00`,raw_text:'',structured:{medication_name:action.label||action.name,original_medication_name:action.label||action.name,canonical_medication_id:canonical,dose:action.dose??null,unit:action.unit||'',status:'taken',action_id:action.id,interpretation_status:'assumed',adherence_evidence:'assumed_from_schedule'},provenance:{source:'scheduled-adherence-assumption',assumption_rule:'user_opted_in',action_id:action.id}});state.events.push(created);existingDays.add(day);added++;}}
    return added;
  }
  ...
  function activityIdentity(name='',structured={},notes=''){
    ...
    else if(/cable/.test(text)){equipment='cable';confidence='high';variation=family==='Row'?'Seated Cable Row':`${family} — Cable`;}
  ...
  function workoutGroups({respectRange=true}={}) {
    ...
    for(const e of state.events.filter(isWorkoutEvent)){
      ...
      prev.variants.add(original);const num=v=>v==null||v===''?null:Number(v),max=(a,b)=>b==null?a:Math.max(a??b,b),weight=num(st.weight??st.load),reps=num(st.reps),sets=num(st.sets),setRows=Number(st.set_number||st.set_no||0)?1:0,painVals=[st.pain,st.pain_before,st.pain_during,st.pain_after].map(num).filter(Number.isFinite),pain=painVals.length?Math.max(...painVals):null;
      sessions.set(sessionKey,{...prev,event:e,date:date||prev.date,weight:max(prev.weight,weight),reps:max(prev.reps,reps),sets:prev.sets+(sets||setRows||1),rpe:max(prev.rpe,num(st.rpe)),rir:max(prev.rir,num(st.rir)),pain:max(prev.pain,pain),pain_before:max(prev.pain_before,num(st.pain_before)),pain_during:max(prev.pain_during,num(st.pain_during)),pain_after:max(prev.pain_after,num(st.pain_after)),duration_min:max(prev.duration_min,num(st.duration_min)),steps:max(prev.steps,num(st.steps)),stair_steps:max(prev.stair_steps,num(st.stair_steps)),ambulatory_steps:max(prev.ambulatory_steps,num(st.ambulatory_steps)),distance_mi:max(prev.distance_mi,num(st.distance_mi)),average_hr:max(prev.average_hr,num(st.average_hr)),pace:st.pace??prev.pace,level:st.level??prev.level,technique:st.technique_notes??st.technique??prev.technique,injury_context:st.injury_context??prev.injury_context,notes:st.notes??prev.notes,family:st.exercise_family||identity.family,equipment:st.equipment_type||st.equipment||identity.equipment,load_basis:st.load_basis||identity.load_basis});
    }
  ...
  function activityRecommendation(name,sessions=[]){
    ...
    const gap=prev?Math.round((new Date(last.date)-new Date(prev.date))/864e5):null,pain=Number(last.pain),rpe=Number(last.rpe),rir=Number(last.rir),hasPain=Number.isFinite(pain),hasRpe=Number.isFinite(rpe),hasRir=Number.isFinite(rir),reps=Number(last.reps),sets=Number(last.sets)||3,load=Number(last.weight),lower=8,upper=12;
    ...
    const highEffort=(hasRpe&&rpe>=9)||(hasRir&&rir<=1),comfortable=(hasRpe&&rpe<=8)||(hasRir&&rir>=2),topCount=recent.slice(-3).filter(x=>Number(x.reps)>=upper&&((Number.isFinite(Number(x.rpe))&&Number(x.rpe)<=8)||(Number.isFinite(Number(x.rir))&&Number(x.rir)>=2))).length;
  ...
  function dashboardStoryCardsHTML(){
    ...
    return `<section class="dashboard-story-grid"><article class="story-card celebration"><span class="story-icon">✦</span><div><small>NICE WORK</small><h2>${esc(niceTitle)}</h2><p>${esc(niceText)}</p></div></article><article class="story-card thinking"><span class="story-icon">◎</span><div><small>I'VE BEEN THINKING…</small><h2>${esc(thinkTitle)}</h2><p>${esc(thinkText)}</p>${discovery?`<button class="text-action" data-insight-key="${esc(discovery.id||discovery.title)}" data-insight-action="pattern">Review evidence</button>`:'<span class="story-limit">Personal evidence not yet sufficient</span>'}</div></article><article class="story-card learning"><span class="story-icon">▤</span><div><small>TODAY I LEARNED</small><h2>${esc(learnTitle)}</h2><p>${esc(learnText)}</p>${knowledge?`<button class="text-action" data-form-guide="${esc(knowledge.name)}">Open form guide</button>`:'<button class="text-action" data-route="fitness">Explore exercise library</button>'}</div></article></section>`;
  }
  ...
  function fitnessPageHTML() {
    ...
    const userCustomActivities=customActivityLibrary(),knowledgeActivities=(window.ZekeKnowledgeBase?.catalog||[]).map(a=>({name:a.name,profile:a.profile,equipment:a.equipment,movement:a.movement,knowledge:true})),customActivities=[...knowledgeActivities,...userCustomActivities],favorites=new Set(JSON.parse(localStorage.getItem('zeke.fitness.activityFavorites.v1')||localStorage.getItem('zeke-activity-favorites')||'[]')),customNames=new Set(userCustomActivities.map(a=>normalizedActivityName(a.name).toLowerCase())),preferences=activityPreferenceMap();
    for(const a of customActivities){if(!groups.has(a.name))groups.set(a.name,[{weight:null,reps:null,sets:null,duration_min:null,date:null,activity_profile:a.profile,placeholder:true,equipment:a.equipment,movement_pattern:a.movement}]);}
  ...
  function openActivityEntryModal(name, profileOverride=''){
    $('#directExerciseModal')?.remove();const sessions=workoutGroups({respectRange:false}).get(name)||[],latest=sessions.at(-1)||{},profile=activityProfile(name,profileOverride),custom=customActivityDefinition(name),guide=window.ZekeKnowledgeBase?.get?.(name),kb=guide&&activityKey(guide.name)===activityKey(name)?guide:null,identity=activityIdentity(name,{equipment:kb?.equipment||''},'');
    const family=identity.family||name,equipment=kb?.equipment||identity.equipment||'unknown',loadBasis=activityLoadBasis(equipment,name,'');
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="directExerciseModal"><div class="direct-entry-card"><div class="section-head"><div><h2>Log ${esc(name)}</h2><p>${esc(activityProfileLabel(profile))} · ${esc(activeDateLabel())}${custom?.fields?.length?' · custom fields':''}</p></div><button class="icon-btn" id="closeDirectExercise" aria-label="Close activity entry">×</button></div>${progressionTargetHTML(name,profile)}<section class="activity-identity-summary"><div><span>Exercise family</span><strong>${esc(family)}</strong></div><div><span>Variation</span><strong>${esc(name)}</strong></div><div><span>Equipment</span><strong>${esc(equipment==='unknown'?'Not specified':equipment)}</strong></div><div><span>Load is recorded as</span><strong>${esc(loadBasis.replaceAll('_',' '))}</strong></div>${guide?`<button type="button" class="secondary compact" id="openEntryFormGuide">Form guide</button>`:''}</section><form id="directExerciseForm" class="direct-entry-form"><label>Date<input id="directExerciseDate" type="date" value="${esc(activeDay())}" required></label><label>Activity type<select id="directActivityProfile"><option value="strength" ${profile==='strength'?'selected':''}>Strength</option><option value="cardio" ${profile==='cardio'?'selected':''}>Cardio</option><option value="mobility" ${profile==='mobility'?'selected':''}>Mobility / stretch</option><option value="rehab" ${profile==='rehab'?'selected':''}>Rehabilitation / PT</option><option value="recovery" ${profile==='recovery'?'selected':''}>Recovery</option><option value="sport" ${profile==='sport'?'selected':''}>Sport / recreation</option><option value="functional" ${profile==='functional'?'selected':''}>Chores / functional activity</option></select></label><div id="activityProfileFields" class="profile-fields wide">${profileFields(profile,latest,name)}</div><label class="wide">Notes (optional)<textarea id="directExerciseNotes" rows="2"></textarea></label><div class="direct-entry-actions wide"><button type="button" class="secondary" id="cancelDirectExercise">Cancel</button><button type="submit" class="primary">Save</button></div><p class="form-error wide" id="directExerciseError" hidden></p></form></div></div>`);
    const close=()=>$('#directExerciseModal')?.remove();$('#closeDirectExercise')?.addEventListener('click',close);$('#cancelDirectExercise')?.addEventListener('click',close);$('#directExerciseModal')?.addEventListener('click',e=>{if(e.target.id==='directExerciseModal')close()});$('#openEntryFormGuide')?.addEventListener('click',()=>openKnowledgeGuide(name));
    $('#directActivityProfile')?.addEventListener('change',e=>{$('#activityProfileFields').innerHTML=profileFields(e.target.value,latest,name)});$$('[data-coach-evidence]',$('#directExerciseModal')).forEach(el=>el.onclick=e=>{e.preventDefault();openCoachEvidence(el.dataset.coachEvidence)});
    $('#directExerciseForm')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;if(!form.reportValidity())return;const date=$('#directExerciseDate').value,chosenProfile=$('#directActivityProfile').value,notes=$('#directExerciseNotes').value.trim(),fieldValues=readActivityFields(),finalIdentity=activityIdentity(name,{equipment:kb?.equipment||equipment,notes},notes),structured={exercise:name,exercise_family:family,variation_name:name,variation_id:kb?.id||activityKey(name).replace(/ /g,'_'),canonical_activity_id:kb?.id||activityKey(name).replace(/ /g,'_'),equipment:equipment==='unknown'?null:equipment,equipment_type:equipment==='unknown'?null:equipment,load_basis:loadBasis,identity_schema_version:1,identity_confidence:'confirmed',movement_pattern:kb?.movement||null,activity_profile:chosenProfile,activity_schema_fields:activitySchema(name,chosenProfile).fields,workout_id:`workout-${date}`,...fieldValues,completed:true,notes,interpretation_status:'confirmed'};const meaningful=Object.entries(fieldValues).some(([,v])=>v!=null&&v!=='')||notes;if(!meaningful){const err=$('#directExerciseError');err.hidden=false;err.textContent='Enter at least one relevant detail or a note.';return;}await ZekeData.addEvent({category:'workout',timestamp:`${date}T12:00:00`,raw_text:notes,structured,provenance:{source:'direct-activity-entry',entry_mode:'schema-driven-form',activity_schema_version:'3',identity_preserved:true}});close();await refreshData();render();showToast(`${name} logged for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}.`)});
  }
  ...
  function historicalIdentityCandidates(){
    return state.events.filter(e=>recordIsActive(e)&&isWorkoutEvent(e)&&!(e.structured||{}).identity_schema_version).map(e=>{
      ...
      return {event:e,identity,confidence:explicit?'high':'unknown'};
    });
  }
  ...
  function openWorkoutEntryModal(options={}){
    ...
    const primaryFromLast=(name,profile)=>{const last=lastFor(name);if(!last)return profile==='strength'?{sets:[{weight:'',reps:''},{weight:'',reps:''},{weight:'',reps:''}]}:{duration_min:'',steps:'',distance_mi:'',intensity_min:'',intensity_max:''};if(profile==='strength'){const reps=Array.isArray(last.set_reps)&&last.set_reps.length?last.set_reps:Array(Math.max(1,Number(last.sets)||3)).fill(last.reps??'');const weights=Array.isArray(last.set_weights)&&last.set_weights.length?last.set_weights:Array(reps.length).fill(last.weight??'');return{sets:reps.map((r,i)=>({weight:weights[i]??last.weight??'',reps:r??''}))};}return{duration_min:last.duration_min??'',steps:last.steps??'',distance_mi:last.distance_mi??'',intensity_min:last.intensity_min??last.level??'',intensity_max:last.intensity_max??''};};
    const blankItem=name=>{const display=activityDisplayName(name),profile=activityProfile(display),guide=window.ZekeKnowledgeBase?.get?.(display),exactGuide=guide&&activityKey(guide.name)===activityKey(display)?guide:null,baseIdentity=activityIdentity(display,{equipment:exactGuide?.equipment||''},exactGuide?.equipment||'');return{exercise:display,profile,status:'not-started',source:'manual',...primaryFromLast(display,profile),exercise_family:baseIdentity.family,variation_name:exactGuide?.name||display,equipment_type:exactGuide?.equipment||'unknown',load_basis:exactGuide?activityLoadBasis(exactGuide.equipment,display,''):'unknown',rpe:'',rir:'',pain:'',rest_sec:'',notes:'',event_id:null};};
    ...
    const renderSummary=()=>{currentIndex=null;screen.className='zeke-workout-screen is-workout-summary';back.hidden=true;title.textContent='Workout';const cards=draft.items.map((item,i)=>{const [icon,label]=statusLabel(item);return`<article class="zeke-workout-item-wrap" draggable="true" data-drag-index="${i}"><button type="button" class="zeke-workout-item" data-edit-workout-item="${i}"><span class="zeke-item-check state-${item.status}">${icon}</span><span><strong>${esc(item.exercise)}</strong><small>${esc(summaryText(item))}</small><em>${esc(label)}</em></span><span aria-hidden="true">›</span></button><div class="zeke-reorder-actions"><button type="button" data-move-up="${i}" aria-label="Move ${esc(item.exercise)} up">↑</button><button type="button" data-move-down="${i}" aria-label="Move ${esc(item.exercise)} down">↓</button><button type="button" data-remove-item="${i}" aria-label="Remove ${esc(item.exercise)}">×</button></div></article>`}).join('');screen.innerHTML=`<section class="zeke-today-card"><div class="zeke-today-heading"><div><span class="tile-kicker">WORKOUT ENTRY</span><h3>${draft.items.length?`${draft.items.length} exercise${draft.items.length===1?'':'s'}`:'Choose how to begin'}</h3></div></div>${cards?`<div class="zeke-workout-items">${cards}</div>`:`<div class="zeke-empty-actions"><button type="button" class="primary" id="startRoutineBtn">Start from Routine</button><button type="button" class="secondary" id="addExerciseBtn">Enter Exercises</button></div>`}${draft.items.length?`<div class="zeke-summary-actions"><button type="button" class="secondary" id="addExerciseBtn">+ Add Exercise</button><button type="button" class="primary" id="endWorkoutBtn">End Workout</button></div>`:''}</section><section class="zeke-exercise-picker" id="exercisePicker" hidden><h3>Add an exercise</h3><div class="zeke-common-exercises">${exerciseNames.slice(0,8).map(n=>`<button type="button" data-common-exercise="${escAttr(n)}">${esc(n)}</button>`).join('')}</div><details><summary>Search exercise library</summary><input id="exerciseSearch" type="search" placeholder="Search exercises"><div id="exerciseSearchResults" class="zeke-search-results"></div></details><button type="button" class="secondary" id="customExerciseBtn">Create custom exercise</button><div class="zeke-picker-actions"><button type="button" class="secondary" id="cancelExercisePicker">Cancel</button></div></section>`;
      ...
      $('#cancelExercisePicker')?.addEventListener('click',()=>{$('#exercisePicker').hidden=true});const addName=n=>{if(!n)return;draft.items.push(blankItem(n));persist();renderExercise(draft.items.length-1)};$$('[data-common-exercise]').forEach(b=>b.onclick=()=>addName(b.dataset.commonExercise));$('#exerciseSearch')?.addEventListener('input',e=>{const q=e.target.value.trim().toLowerCase();$('#exerciseSearchResults').innerHTML=q?exerciseNames.filter(n=>n.toLowerCase().includes(q)).slice(0,12).map(n=>`<button type="button" data-search-exercise="${escAttr(n)}">${esc(n)}</button>`).join(''):'';$$('[data-search-exercise]').forEach(b=>b.onclick=()=>addName(b.dataset.searchExercise));});$('#customExerciseBtn')?.addEventListener('click',()=>{const n=prompt('Custom exercise name');if(n?.trim())addName(n.trim())});$$('[data-edit-workout-item]').forEach(b=>b.onclick=()=>renderExercise(Number(b.dataset.editWorkoutItem)));$$('[data-move-up]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.moveUp);if(i>0){[draft.items[i-1],draft.items[i]]=[draft.items[i],draft.items[i-1]];persist();renderSummary()}});$$('[data-move-down]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.moveDown);if(i<draft.items.length-1){[draft.items[i+1],draft.items[i]]=[draft.items[i],draft.items[i+1]];persist();renderSummary()}});$$('[data-remove-item]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.removeItem),item=draft.items[i];if(item.status==='saved'&&!confirm('Remove this saved exercise from active history? The correction trail will be preserved.'))return;draft.items.splice(i,1);persist();renderSummary()});$('#endWorkoutBtn')?.addEventListener('click',()=>{const unsaved=draft.items.some(x=>x.status==='in-progress');if(unsaved&&!confirm('One or more exercises have unsaved changes. End the workout and keep only saved exercises?'))return;clearDraft();$('#directWorkoutModal')?.remove();showToast('Workout ended. You can add more exercises later today.');});
    };
    const readinessFor=(item,arr)=>{const rec=activityRecommendation(item.variation_name||item.exercise,arr),last=rec.last||arr.at(-1),target=rec.target||null,targetReps=target?Number(String(target.reps||'').match(/\d+/)?.[0]||last?.reps||10):null,targetLoad=target?.load!=null?Number(target.load):null,targetSets=target?.sets!=null?Number(target.sets):null,progress=targetLoad!=null&&Number(last?.weight)>0&&targetLoad>Number(last.weight);return{category:rec.title||'ZEKE target',position:rec.clinician_priority?'maintain':progress?'progress':target?'maintain':'unknown',text:[rec.rationale,rec.suggestion].filter(Boolean).join(' '),apply:target&&targetLoad>0?{weight:targetLoad,reps:Math.max(1,targetReps||10),sets:Math.max(1,targetSets||3)}:null,evidence_ids:rec.evidence_ids||[],recommendation:rec};};
    const renderHistory=(item,returnIndex)=>{screen.className='zeke-workout-screen is-history-view';back.hidden=false;title.textContent=`${item.exercise} History`;const arr=historyFor(item.exercise);screen.innerHTML=`<section class="zeke-history-panel"><div class="mock-section-head"><strong>PROGRESSION HISTORY</strong></div>${arr.length?`<div class="zeke-history-list">${arr.slice().reverse().map(x=>`<article><strong>${fmtDate(x.event.timestamp,{month:'short',day:'numeric',year:'numeric'})}</strong><span>${esc(x.weight!=null?`${x.weight} lb · ${x.sets||'—'} × ${x.reps||'—'}`:[x.duration_min?`${x.duration_min} min`:'',x.steps?`${x.steps} steps`:''].filter(Boolean).join(' · ')||'No primary values')}</span></article>`).join('')}</div>`:'<p>No confirmed history yet.</p>'}</section>`;back.onclick=()=>renderExercise(returnIndex);};
    const renderExercise=index=>{currentIndex=index;const item=draft.items[index],profile=item.profile||activityProfile(item.exercise),arr=historyFor(item.exercise),last=arr.at(-1),ready=readinessFor(item,arr);screen.className='zeke-workout-screen is-exercise-view';back.hidden=false;back.onclick=renderSummary;title.textContent=item.exercise;const chartVals=arr.slice(-7).map(x=>Number(profile==='strength'?x.weight:x.duration_min)).filter(Number.isFinite);const min=chartVals.length?Math.min(...chartVals):0,max=chartVals.length?Math.max(...chartVals):1,range=Math.max(1,max-min);const points=chartVals.map((v,i)=>`${6+i*(88/Math.max(1,chartVals.length-1))},${46-((v-min)/range)*30}`).join(' ');const lastSummary=last?(profile==='strength'?(()=>{const reps=Array.isArray(last.set_reps)&&last.set_reps.length?last.set_reps:Array(Number(last.sets)||1).fill(last.reps);const same=reps.every(r=>String(r)===String(reps[0]));return same?`${last.weight??'—'} lb · ${reps.length} × ${reps[0]??'—'}`:`${last.weight??'—'} lb · ${reps.join(' / ')} reps`;})():[last.duration_min?`${last.duration_min} min`:'',last.steps?`${last.steps} steps`:''].filter(Boolean).join(' · ')):'No previous entry';const strengthRows=(item.sets||[{weight:'',reps:''},{weight:'',reps:''},{weight:'',reps:''}]).map((s,i)=>`<div class="zeke-set-row" data-set-row="${i}"><span>Set ${i+1}</span><label>Weight<input class="set-weight" type="number" min="0.1" step="0.1" inputmode="decimal" value="${escAttr(s.weight)}"></label><label>Reps<input class="set-reps" type="number" min="1" step="1" inputmode="numeric" value="${escAttr(s.reps)}"></label><button type="button" data-delete-set="${i}" aria-label="Delete set ${i+1}">−</button></div>`).join('');const entry=profile==='strength'?`<div id="strengthSets">${strengthRows}</div><button type="button" class="secondary" id="addSetBtn">+ Add Set</button>`:`<div class="zeke-cardio-grid"><label>Duration (minutes)<input id="exerciseDuration" type="number" min="0.1" step="0.1" value="${escAttr(item.duration_min)}"></label><label>${/stair|climb/i.test(item.exercise)?'Steps':'Distance (mi)'}<input id="exercisePrimaryCardio" type="number" min="0" step="0.01" value="${escAttr(/stair|climb/i.test(item.exercise)?item.steps:item.distance_mi)}"></label><label>Intensity minimum (optional)<input id="exerciseIntensityMin" type="number" min="0" step="0.1" value="${escAttr(item.intensity_min)}"></label><label>Intensity maximum (optional)<input id="exerciseIntensityMax" type="number" min="0" step="0.1" value="${escAttr(item.intensity_max)}"></label></div>`;screen.innerHTML=`<div class="mock-exercise-page"><section class="mock-coach-card"><div class="mock-card-title"><span class="mock-z-icon">Z</span><strong>COACH’S EYE</strong></div><div class="mock-coach-body"><div><h3>${esc(ready.category)}</h3><p>${esc(ready.text)}</p>${ready.apply?'<button type="button" class="secondary" id="applyProgressionBtn">Apply ZEKE Target</button>':''}<button type="button" class="text-action" id="workoutEvidenceBtn">Why / evidence</button></div><div class="mock-readiness"><div class="zeke-qualitative-gauge ${ready.position}" aria-label="${escAttr(ready.category)}"><span class="needle"></span></div><small>Readiness</small><strong>${esc(ready.category)}</strong></div></div></section><button type="button" class="mock-progress-card zeke-progress-button" id="viewHistoryBtn"><div class="mock-section-head"><strong>PROGRESSION</strong><span>View History ›</span></div><svg viewBox="0 0 100 56" preserveAspectRatio="none"><line x1="0" y1="48" x2="100" y2="48"/><polyline points="${points}"/></svg><p>${arr.length?`${arr.length} confirmed entr${arr.length===1?'y':'ies'} · recent trend shown above`:'No confirmed history yet'}</p></button><section class="mock-last-card"><div class="mock-section-head"><strong>LAST TIME ${last?`<small>(${fmtDate(last.event.timestamp,{month:'short',day:'numeric'})})</small>`:''}</strong></div><p class="zeke-last-summary">${esc(lastSummary)}</p><div class="mock-last-notes"><strong>Notes</strong><small>${esc(last?.notes||'No notes recorded.')}</small></div></section><section class="mock-today-card"><div class="mock-section-head"><strong>TODAY’S ENTRY</strong><span>Prefilled from last confirmed entry</span></div>${entry}<button type="button" class="mock-form-guide-trigger" id="formGuideBtn"><span><strong>View Form Guide</strong><small>Tap image for movement sequence</small></span><b>›</b></button><div class="activity-identity-summary"><span><strong>Variation</strong>${esc(item.variation_name||item.exercise)}</span><span><strong>Equipment</strong>${esc(item.equipment_type&&item.equipment_type!=='unknown'?item.equipment_type:'Not specified')}</span><span><strong>Load basis</strong>${esc(item.load_basis&&item.load_basis!=='unknown'?item.load_basis.replaceAll('_',' '):'Not specified')}</span></div><details class="zeke-optional-details"><summary>Add optional details</summary><div><label>Equipment / variation<select id="exerciseEquipment"><option value="unknown" ${!item.equipment_type||item.equipment_type==='unknown'?'selected':''}>Not specified</option><option value="selectorized machine" ${item.equipment_type==='selectorized machine'?'selected':''}>Machine</option><option value="dumbbell" ${item.equipment_type==='dumbbell'?'selected':''}>Dumbbell</option><option value="barbell" ${item.equipment_type==='barbell'?'selected':''}>Barbell</option><option value="Bowflex" ${item.equipment_type==='Bowflex'?'selected':''}>Bowflex</option><option value="resistance band" ${item.equipment_type==='resistance band'?'selected':''}>Resistance band</option><option value="cable" ${item.equipment_type==='cable'?'selected':''}>Cable</option></select></label><label>RPE<input id="exerciseRpe" type="number" min="1" max="10" step="0.5" value="${escAttr(item.rpe)}"></label><label>RIR<input id="exerciseRir" type="number" min="0" max="10" step="1" value="${escAttr(item.rir)}"></label><label>Pain<input id="exercisePain" type="number" min="0" max="10" step="1" value="${escAttr(item.pain)}"></label><label>Rest (seconds)<input id="exerciseRest" type="number" min="0" step="5" value="${escAttr(item.rest_sec)}"></label><label>Notes<textarea id="exerciseNotes">${esc(item.notes||'')}</textarea></label></div></details><p class="form-error" id="exerciseSaveError" hidden></p></section><div class="mock-save-row"><button type="button" class="secondary" id="cancelExerciseBtn">Cancel</button><button type="button" class="mock-save-exercise" id="saveExerciseBtn">Save</button><div id="exerciseSaveState" class="mock-save-status" aria-live="polite"></div></div></div>`;
      const syncFromForm=()=>{if(profile==='strength'){item.sets=$$('.zeke-set-row').map(row=>({weight:$('.set-weight',row)?.value||'',reps:$('.set-reps',row)?.value||''}));}else{item.duration_min=$('#exerciseDuration')?.value||'';if(/stair|climb/i.test(item.exercise))item.steps=$('#exercisePrimaryCardio')?.value||'';else item.distance_mi=$('#exercisePrimaryCardio')?.value||'';item.intensity_min=$('#exerciseIntensityMin')?.value||'';item.intensity_max=$('#exerciseIntensityMax')?.value||'';}item.equipment_type=$('#exerciseEquipment')?.value||item.equipment_type||'unknown';item.rpe=$('#exerciseRpe')?.value||'';item.rir=$('#exerciseRir')?.value||'';item.pain=$('#exercisePain')?.value||'';item.rest_sec=$('#exerciseRest')?.value||'';item.notes=$('#exerciseNotes')?.value||'';if(item.equipment_type&&item.equipment_type!=='unknown'){const derived=activityIdentity(item.exercise,{equipment:item.equipment_type},`${item.notes||''} ${item.equipment_type}`);item.exercise_family=derived.family;item.variation_name=derived.variation;item.load_basis=derived.load_basis;}markChanged(item);};screen.querySelectorAll('input,textarea').forEach(el=>el.addEventListener('input',syncFromForm));$('#addSetBtn')?.addEventListener('click',()=>{syncFromForm();const base=item.sets[0]||{weight:'',reps:''};item.sets.push({weight:base.weight,reps:base.reps});markChanged(item);renderExercise(index)});$$('[data-delete-set]').forEach(b=>b.onclick=()=>{syncFromForm();item.sets.splice(Number(b.dataset.deleteSet),1);if(!item.sets.length)item.sets.push({weight:'',reps:''});markChanged(item);renderExercise(index)});$('#applyProgressionBtn')?.addEventListener('click',()=>{if(!ready.apply)return;item.sets=Array.from({length:ready.apply.sets},()=>({weight:String(ready.apply.weight),reps:String(ready.apply.reps)}));markChanged(item);renderExercise(index)});$('#viewHistoryBtn').onclick=()=>{syncFromForm();renderHistory(item,index)};$('#workoutEvidenceBtn')?.addEventListener('click',()=>openCoachEvidence(item.variation_name||item.exercise));$('#formGuideBtn').onclick=()=>openFormGuide(item.variation_name||item.exercise);$('#cancelExerciseBtn').onclick=()=>{if(item.status==='in-progress'&&!confirm('Discard unsaved changes to this exercise?'))return;renderSummary()};$('#saveExerciseBtn').onclick=async()=>{syncFromForm();const btn=$('#saveExerciseBtn'),status=$('#exerciseSaveState'),error=$('#exerciseSaveError');error.hidden=true;const effectiveDate=$('#gymWorkoutDate').value||date;if(profile==='strength'){const valid=item.sets.filter(s=>s.weight!==''||s.reps!=='');if(!valid.length||valid.some(s=>!(Number(s.weight)>0)||!(Number(s.reps)>0))){error.hidden=false;error.textContent='Enter a valid weight and reps for every set, or delete incomplete sets.';return;}item.sets=valid;}else if(!(Number(item.duration_min)>0)){error.hidden=false;error.textContent='Enter a valid duration.';return;}btn.disabled=true;btn.textContent='Saving to storage…';status.textContent='';try{const identity=item.equipment_type&&item.equipment_type!=='unknown'?activityIdentity(item.exercise,{equipment:item.equipment_type},`${item.notes||''} ${item.equipment_type}`):{family:item.exercise_family||normalizedActivityName(item.exercise),variation:item.variation_name||item.exercise,equipment:'unknown',load_basis:item.load_basis||'unknown'};const structured={exercise:item.exercise,canonical_activity_id:activityKey(item.exercise).replace(/ /g,'_'),activity_profile:profile,exercise_family:identity.family,variation_name:identity.variation,variation_id:activityKey(identity.variation).replace(/ /g,'_'),equipment_type:identity.equipment,load_basis:identity.load_basis,identity_schema_version:1,identity_confidence:item.equipment_type&&item.equipment_type!=='unknown'?'confirmed':'unspecified',workout_day:effectiveDate,weight:null,reps:null,sets:null,set_reps:null,set_weights:null,duration_min:null,steps:null,distance_mi:null,intensity_min:null,intensity_max:null,rpe:item.rpe===''?null:Number(item.rpe),rir:item.rir===''?null:Number(item.rir),pain:item.pain===''?null:Number(item.pain),rest_sec:item.rest_sec===''?null:Number(item.rest_sec),notes:item.notes||'',completed:true,interpretation_status:'confirmed',activity_schema_version:3};if(profile==='strength'){structured.sets=item.sets.length;structured.set_reps=item.sets.map(s=>Number(s.reps));structured.set_weights=item.sets.map(s=>Number(s.weight));structured.weight=item.sets.every(s=>Number(s.weight)===Number(item.sets[0].weight))?Number(item.sets[0].weight):null;structured.reps=item.sets.every(s=>Number(s.reps)===Number(item.sets[0].reps))?Number(item.sets[0].reps):null;}else{structured.duration_min=Number(item.duration_min);structured.steps=item.steps===''?null:Number(item.steps);structured.distance_mi=item.distance_mi===''?null:Number(item.distance_mi);structured.intensity_min=item.intensity_min===''?null:Number(item.intensity_min);structured.intensity_max=item.intensity_max===''?null:Number(item.intensity_max);}await ZekeData.addEvent({category:'workout',timestamp:`${effectiveDate}T12:00:00`,raw_text:item.notes||'',structured,provenance:{source:'mobile-workout-entry',entry_mode:'structured-form',prefill_source:last?'previous_confirmed_entry':'blank',storage_status:'confirmed'}});item.status='saved';persist();status.innerHTML='<strong>✓ Saved</strong>';await refreshData();setTimeout(renderSummary,900);}catch(err){btn.disabled=false;btn.textContent='Try Save Again';status.textContent='Save failed';error.hidden=false;error.textContent=`Exercise was not saved: ${err?.message||'Unknown error'}`;}};
    };
