(() => {
  ...
  function openWorkoutEntryModal(options={}){
    ...
    const openFormGuide=exercise=>{const g=guideLibrary[activityKey(exercise)];document.body.classList.add('zeke-sheet-open');document.body.insertAdjacentHTML('beforeend',`<div class="zeke-sheet-scrim" id="formGuideSheet"><section class="zeke-form-sheet mock-form-sheet" role="dialog" aria-modal="true"><div class="zeke-sheet-handle"></div><header><h2>${esc(exercise)} – Form Guide</h2><button type="button" id="closeFormGuide" aria-label="Close">×</button></header><div class="zeke-guide-content">${g?`<button type="button" class="zeke-guide-image" id="expandGuideSequence"><img src="${g.image}" alt="Adult actively performing ${escAttr(exercise)}"><span>Tap to view movement sequence</span></button><small>${esc(g.credit)}</small>`:`<div class="zeke-guide-unavailable"><strong>Verified movement image not yet available</strong><p>ZEKE will not substitute a generic gym photo that does not clearly show this exercise being performed.</p></div>`}<section><h3>Setup</h3><p>${esc(g?.setup||'Use the equipment instructions and choose a stable, comfortable setup.')}</p><h3>Movement</h3><p>${esc(g?.movement||'Move through a controlled, comfortable range without momentum.')}</p><h3>Common Mistakes</h3><p>${esc(g?.mistakes||'Avoid uncontrolled movement, poor alignment, and continuing through sharp or increasing pain.')}</p><h3>Tips</h3><p>${esc(g?.tips||'Use repeatable form and a load or intensity you can control.')}</p></section></div><button type="button" class="mock-got-it" id="gotItGuide">Got it</button></section></div>`);const close=()=>{$('#formGuideSheet')?.remove();document.body.classList.remove('zeke-sheet-open')};$('#closeFormGuide').onclick=close;$('#gotItGuide').onclick=close;$('#formGuideSheet').onclick=e=>{if(e.target.id==='formGuideSheet')close()};$('#expandGuideSequence')?.addEventListener('click',()=>{const btn=$('#expandGuideSequence');btn.classList.toggle('expanded');btn.querySelector('span').textContent=btn.classList.contains('expanded')?'Movement sequence: start → controlled effort → return':'Tap to view movement sequence';});};
