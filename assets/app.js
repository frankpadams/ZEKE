(() => {
  ...
  function bind() {
    ...
    $$('[data-activity-pattern]').forEach(el=>el.onclick=e=>{e.stopPropagation();state.patternFocus=el.dataset.activityPattern;try{sessionStorage.setItem('zeke-pattern-focus',state.patternFocus)}catch(_){};state.route='pattern-lab';location.hash='#/pattern-lab';render()});
