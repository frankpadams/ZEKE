(() => {
  ...
  const IMPROVEMENT_LOG_KEY='zeke-improvements-log-v2';
  function improvementLog(){try{return JSON.parse(localStorage.getItem(IMPROVEMENT_LOG_KEY)||'[]')}catch(_){return []}}
  function saveImprovementLog(rows){try{localStorage.setItem(IMPROVEMENT_LOG_KEY,JSON.stringify((rows||[]).slice(-500)))}catch(_){}}
  function recordImprovement({kind='ux_feedback',summary='',detail='',severity='normal',source='runtime',status='open',context={}}={}){
    const row={id:`improvement:${Date.now()}:${Math.random().toString(36).slice(2,8)}`,timestamp:new Date().toISOString(),version:BUILD.version,build:BUILD.build,kind,summary:String(summary||'ZEKE improvement').slice(0,300),detail:String(detail||'').slice(0,2500),severity,status,source,route:state.route,context};
    const rows=improvementLog();rows.push(row);saveImprovementLog(rows);
    if(window.ZekeData?.snapshot?.().status==='connected')ZekeData.saveFactor({id:row.id,type:'improvement_log',status:'open',priority:severity==='high'?'high':'normal',summary:row.summary,improvement:row,provenance:{source:'zeke-improvements-log',storage:'user-repository'}}).catch(()=>{});
    return row;
  }
  window.ZekeImprovements={list:improvementLog,record:recordImprovement};
  window.addEventListener('zeke:ai-mishap',e=>{const d=e.detail||{};recordImprovement({kind:'ai_mishap',severity:d.severity||'high',summary:d.summary||'AI consultation mishap',detail:d.detail||d.error||'',source:'ai-router',context:{provider:d.provider||'',model:d.model||'',task:d.task||'',stage:d.stage||'',validation:d.validation||''}})});
  ...
  function routeFromHash() {
    ...
    const map = {
      ...
      'calendar':'calendar','documents':'documents','questions':'questions','clarifications':'questions','life-events':'life-events','symptoms':'life-events','pattern-lab':'insights','insights':'insights','settings':'settings','data-integrity':'data-integrity','system/data-integrity':'data-integrity'
    };
  ...
  function go(route) {
    ...
    const hashes = {dashboard:'health/dashboard',health:'health',fitness:'fitness',medications:'medications',calendar:'calendar',documents:'documents',questions:'questions','life-events':'life-events',insights:'insights',settings:'settings','data-integrity':'data-integrity'};
  ...
  function sleepPeriodAnalysis(){
    const all=allMetricSeries('sleep_duration');if(!all.length)return null;
    const now=new Date(), startOfWeek=d=>{const x=new Date(d);x.setHours(0,0,0,0);x.setDate(x.getDate()-((x.getDay()+6)%7));return x}, week0=startOfWeek(now), week1=new Date(week0);week1.setDate(week1.getDate()-7);
    const meanBetween=(a,b)=>average(all.filter(p=>{const t=new Date(p.date);return t>=a&&t<b}).map(p=>p.value));
    const currentWeek=meanBetween(week0,new Date(now.getTime()+864e5)), previousWeek=meanBetween(week1,week0);
    const recent30=new Date(now);recent30.setDate(recent30.getDate()-30);const prior30=new Date(recent30);prior30.setDate(prior30.getDate()-30);
    const avg30=meanBetween(recent30,new Date(now.getTime()+864e5)), prev30=meanBetween(prior30,recent30);
    const recent90=new Date(now);recent90.setDate(recent90.getDate()-90);const avg90=meanBetween(recent90,new Date(now.getTime()+864e5));
    const delta=(a,b)=>a!=null&&b!=null?a-b:null;
    return {currentWeek,previousWeek,avg30,prev30,avg90,weekDelta:delta(currentWeek,previousWeek),monthDelta:delta(avg30,prev30),count:all.length};
  }
  function sleepTrendNarrative(){
    const a=sleepPeriodAnalysis();if(!a)return 'ZEKE has no verified sleep observations to summarize.';
    const fmt=v=>v==null?'not enough data':durationLabel(v), parts=[];
    if(a.currentWeek!=null)parts.push(`This week averages ${fmt(a.currentWeek)} per recorded night${a.previousWeek!=null?`, ${Math.abs(a.weekDelta)*60<5?'about the same as':a.weekDelta>0?'up from':'down from'} ${fmt(a.previousWeek)} last week`:''}.`);
    if(a.avg30!=null)parts.push(`The last 30 days average ${fmt(a.avg30)}${a.prev30!=null?` versus ${fmt(a.prev30)} in the previous 30 days`:''}.`);
    if(a.avg90!=null)parts.push(`The 90-day average is ${fmt(a.avg90)}.`);
    if(a.avg30!=null)parts.push(a.avg30>=7?'Your recent average is within the usual adult recommendation of at least 7 hours regularly.':'Your recent average is below the usual adult recommendation of at least 7 hours regularly; ZEKE treats that as context, not a diagnosis.');
    return parts.join(' ');
  }

  function metricNarrative(id, points) {
    const meta=METRICS[id]||{label:id};
    if(id==='sleep_duration') return sleepTrendNarrative();
  ...
  function metricRangeLabel(rangeId){return ({week:'7 days',month:'31 days',quarter:'3 months','6months':'6 months',year:'1 year',all:'All time'})[rangeId]||'3 months'}
  function previousMetricSeries(id,rangeId){
    const days=RANGE_DAYS[rangeId];if(!days)return [];const now=Date.now(),start=now-days*864e5,prior=start-days*864e5;
    return allMetricSeries(id).filter(p=>{const t=new Date(p.date).getTime();return t>=prior&&t<start});
  }
  function rangeTrendChartSVG(id,points){
    const meta=METRICS[id]||{label:id,unit:''};if(!points||points.length<2)return `<div class="chart-empty">Another verified observation is needed to draw a trend for this period.</div>`;
    const clean=points.filter(p=>Number.isFinite(Number(p.value))&&Number.isFinite(new Date(p.date).getTime()));if(clean.length<2)return `<div class="chart-empty">Not enough verified observations in this period.</div>`;
    const w=760,h=230,pad={l:48,r:16,t:18,b:38},vals=clean.map(x=>Number(x.value)),min=Math.min(...vals),max=Math.max(...vals),span=(max-min)||1,times=clean.map(x=>new Date(x.date).getTime()),t0=Math.min(...times),t1=Math.max(...times),tspan=(t1-t0)||1;
    const x=t=>pad.l+(w-pad.l-pad.r)*(t-t0)/tspan,y=v=>pad.t+(h-pad.t-pad.b)*(1-(v-min)/span);
    const pts=clean.map(q=>[x(new Date(q.date).getTime()),y(Number(q.value))]);const path=pts.map((q,i)=>`${i?'L':'M'}${q[0].toFixed(1)} ${q[1].toFixed(1)}`).join(' ');
    const ticks=[0,.25,.5,.75,1].map(f=>{const v=min+span*f;return `<g><line x1="${pad.l}" y1="${y(v)}" x2="${w-pad.r}" y2="${y(v)}"/><text x="${pad.l-8}" y="${y(v)+4}" text-anchor="end">${esc(v.toFixed(Math.abs(span)<5?1:0))}</text></g>`}).join('');
    return `<svg class="metric-analysis-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(meta.label)} trend for ${esc(metricRangeLabel(state.detailMetricRange))}"><g class="metric-grid">${ticks}</g><path class="metric-analysis-line" d="${path}"/>${pts.map((q,i)=>`<circle class="metric-analysis-point" cx="${q[0]}" cy="${q[1]}" r="4"><title>${esc(fmtDate(clean[i].date,{month:'short',day:'numeric',year:'numeric'}))}: ${esc(clean[i].value)} ${esc(clean[i].unit||meta.unit||'')}</title></circle>`).join('')}<text class="metric-axis-note" x="${pad.l}" y="${h-8}">${esc(fmtDate(clean[0].date,{month:'short',day:'numeric',year:'numeric'}))}</text><text class="metric-axis-note" text-anchor="end" x="${w-pad.r}" y="${h-8}">${esc(fmtDate(clean.at(-1).date,{month:'short',day:'numeric',year:'numeric'}))}</text></svg>`;
  }
  function relatedContextForRange(points){
    if(!points?.length)return [];const start=new Date(points[0].date).getTime(),end=new Date(points.at(-1).date).getTime();
    return dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e)&&!['measurement','lab','sleep'].includes(semanticCategory(e))&&(()=>{const t=new Date(e.timestamp||e.recorded_at).getTime();return Number.isFinite(t)&&t>=start-3*864e5&&t<=end+3*864e5})())).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0)).slice(0,8);
  }
  function openMetricDetail(id) {
    const meta=METRICS[id]||{label:id,unit:''}, latest=latestMetric(id);
    let overlay=$('#metricDetailOverlay');if(!overlay){overlay=document.createElement('div');overlay.id='metricDetailOverlay';overlay.className='metric-detail-overlay';document.body.appendChild(overlay);}
    const renderDetail=()=>{
      const range=state.detailMetricRange, points=id==='blood_pressure'?bloodPressureSeries(false,range).sys:metricSeries(id,range), all=id==='blood_pressure'?bloodPressureSeries(true).sys:allMetricSeries(id), shown=points.length?points:(range==='all'?all:[]), values=shown.map(p=>Number(p.value)).filter(Number.isFinite), avg=average(values), first=shown[0],last=shown.at(-1),change=first&&last?Number(last.value)-Number(first.value):null, prior=previousMetricSeries(id,range),priorAvg=average(prior.map(p=>p.value)),context=relatedContextForRange(shown);
      const stats=[['Latest',last?`${last.value} ${last.unit||meta.unit||''}`:'—'],['Average',avg!=null?`${avg.toFixed(Math.abs(avg)<10?1:0)} ${last?.unit||meta.unit||''}`:'—'],['Change',change!=null?`${change>0?'+':''}${change.toFixed(Math.abs(change)<10?1:0)} ${last?.unit||meta.unit||''}`:'—'],['Observations',String(shown.length)],['Previous period',priorAvg!=null&&avg!=null?`${avg-priorAvg>0?'+':''}${(avg-priorAvg).toFixed(Math.abs(avg-priorAvg)<10?1:0)} vs avg`:'Not enough data']];
      const rows=shown.slice().reverse().map(p=>`<tr><td>${esc(fmtDate(p.date,{month:'short',day:'numeric',year:'numeric'}))}</td><td>${esc(p.value)} ${esc(p.unit||meta.unit||'')}</td><td>${esc(p.source||'ZEKE')}</td></tr>`).join('');
      overlay.innerHTML=`<section class="metric-detail metric-detail-v49" role="dialog" aria-modal="true" aria-label="${esc(meta.label)} details"><button class="metric-detail-close" aria-label="Close">×</button><header class="metric-detail-header"><div><span class="tile-kicker">ANALYSIS</span><h2>${esc(meta.label)}</h2><p>Latest verified: <strong>${esc(latest?.value??'—')} ${esc(latest?.unit||meta.unit||'')}</strong>${latest?.date?` · ${esc(fmtDate(latest.date,{month:'short',day:'numeric',year:'numeric'}))}`:''}</p></div><div class="metric-detail-ranges">${DASHBOARD_RANGE_OPTIONS.map(([rid,label])=>`<button type="button" class="range ${range===rid?'active':''}" data-detail-metric-range="${rid}">${label}</button>`).join('')}</div></header><div class="metric-detail-period">Showing ${esc(metricRangeLabel(range))}${shown.length?` · ${esc(fmtDate(shown[0].date,{month:'short',day:'numeric',year:'numeric'}))}–${esc(fmtDate(shown.at(-1).date,{month:'short',day:'numeric',year:'numeric'}))}`:''}</div><div class="metric-stat-grid">${stats.map(([a,b])=>`<div><small>${esc(a)}</small><strong>${esc(b)}</strong></div>`).join('')}</div><section><h3>Trend</h3>${rangeTrendChartSVG(id,shown)}</section><section class="metric-interpretation"><h3>What ZEKE sees</h3><p>${esc(metricNarrative(id,shown))}</p></section><section><h3>Relevant context in this period</h3>${context.length?`<div class="metric-context-list">${context.map(e=>`<button type="button" data-route="${isWorkoutEvent(e)?'fitness':'health'}"><strong>${esc(humanEvent(e))}</strong><small>${esc(fmtDate(e.timestamp||e.recorded_at,{month:'short',day:'numeric',year:'numeric'}))}</small></button>`).join('')}</div>`:'<p class="safety-copy">No additional verified context is recorded near this period. ZEKE does not treat missing context as evidence that nothing happened.</p>'}</section><section><h3>Underlying verified observations</h3><div class="metric-detail-table-wrap"><table><thead><tr><th>Date</th><th>Value</th><th>Source</th></tr></thead><tbody>${rows||'<tr><td colspan="3">No verified observations in this period.</td></tr>'}</tbody></table></div></section></section>`;
      overlay.querySelector('.metric-detail-close')?.addEventListener('click',()=>overlay.classList.remove('show'));
      overlay.querySelectorAll('[data-detail-metric-range]').forEach(btn=>btn.addEventListener('click',()=>{state.detailMetricRange=btn.dataset.detailMetricRange;localStorage.setItem('zeke.metric.detail.range.v2',state.detailMetricRange);renderDetail()}));
      overlay.querySelectorAll('[data-route]').forEach(btn=>btn.addEventListener('click',()=>{overlay.classList.remove('show');go(btn.dataset.route)}));
    };
    renderDetail();overlay.classList.add('show');overlay.onclick=e=>{if(e.target===overlay)overlay.classList.remove('show')};
  }
  ...
  function metricCard(id,allowEmpty=false,rangeId=state.dashboardHealthRange) {
    ...
    const delta=metricDelta(id,rangeId); let points=id==='blood_pressure'?bloodPressureSeries(false,rangeId).sys:metricSeries(id,rangeId); if(points.length>60){const keep=[points[0]];const step=(points.length-1)/58;for(let i=1;i<59;i++)keep.push(points[Math.round(i*step)]);keep.push(points.at(-1));points=keep;}
  ...
  function metricRecentInsight(id){
    ...
    if(id==='sleep_duration'){const a=sleepPeriodAnalysis();if(!a)return null;const trend=a.monthDelta==null?'with no prior 30-day comparison yet':Math.abs(a.monthDelta)<(5/60)?'and is essentially unchanged from the prior 30 days':a.monthDelta>0?`and is up ${durationLabel(Math.abs(a.monthDelta))} versus the prior 30 days`:`and is down ${durationLabel(Math.abs(a.monthDelta))} versus the prior 30 days`;return {id,title:'Sleep trend across weeks and months',summary:`The last 30 days average ${durationLabel(a.avg30)} per recorded night ${trend}.`,context:`This week: ${a.currentWeek==null?'not enough data':durationLabel(a.currentWeek)} · previous week: ${a.previousWeek==null?'not enough data':durationLabel(a.previousWeek)} · 90-day average: ${a.avg90==null?'not enough data':durationLabel(a.avg90)}. ${a.avg30!=null&&a.avg30>=7?'Recent average is within':'Recent average is below'} the usual adult recommendation of at least 7 hours regularly.`,points:all.slice(-30),allCount:all.length};}
  ...
  function familyVariationChart(sessions=[],category='strength',family=''){
    ...
    const legend=series.map(([name,rows],si)=>`<button type="button" class="variation-legend-item variation-series-${si%6}" data-variation-focus="${esc(name)}"><i></i><b>${esc(name)}</b><small>${rows.length} ${rows.length===1?'point':'points'}</small></button>`).join('');
  ...
  function timelineSnapshotHTML(){
    const {today,start,end,days}=timelineWindow(),rows=timelineDomainRows(),key=d=>dayKey(d),visible=rows.map(r=>({...r,items:r.items.filter(x=>{const d=new Date(x.start);return d>=start&&d<=new Date(end.getTime()+864e5)})}));
    return `<section class="panel timeline-snapshot v49-timeline"><div class="section-head"><div><span class="tile-kicker">WHAT'S BEEN HAPPENING?</span><h2>Timeline Snapshot</h2><p>${esc(fmtDate(start,{month:'short',day:'numeric'}))}–${esc(fmtDate(end,{month:'short',day:'numeric',year:'numeric'}))} · domains by date</p></div><div class="v49-timeline-nav"><button type="button" data-timeline-shift="-7" aria-label="Previous week">‹</button><button type="button" data-timeline-today>Today</button><button type="button" data-timeline-shift="7" aria-label="Next week">›</button><button class="text-action" data-route="calendar">Open full timeline</button></div></div><div class="v49-timeline-scroller"><div class="v49-timeline-grid"><div class="v49-timeline-corner">Domain</div>${days.map(d=>`<div class="v49-timeline-date ${key(d)===key(today)?'today':''}"><strong>${d.toLocaleDateString(undefined,{weekday:'short'})}</strong><span>${d.toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span></div>`).join('')}${visible.map(r=>`<div class="v49-timeline-label">${esc(r.label)}</div>${days.map(d=>{const items=r.items.filter(x=>x.day===key(d));return `<div class="v49-timeline-cell ${key(d)===key(today)?'today':''}">${items.slice(0,3).map(x=>`<button type="button" class="v49-timeline-event kind-${esc(x.kind)}" title="${esc(x.label||x.kind)}" data-route="${['workout','exercise','activity','fitness'].includes(x.kind)?'fitness':'health'}"><span></span><b>${esc((x.label||x.kind).slice(0,28))}</b></button>`).join('')}${items.length>3?`<small>+${items.length-3}</small>`:''}</div>`}).join('')}`).join('')}</div></div><p class="v49-timeline-note">Scroll the date grid horizontally. Use ‹ / › to move a week at a time; Today returns to the current window.</p></section>`;
  }

  function v47DashboardMetricCard(id){
    const latest=latestMetric(id),meta=METRICS[id];if(!latest||!meta)return '';
    let pts=id==='blood_pressure'?bloodPressureSeries(false,state.dashboardHealthRange).sys:metricSeries(id,state.dashboardHealthRange);if(pts.length>24)pts=pts.slice(-24);
    const delta=metricDelta(id,state.dashboardHealthRange),number=id==='sleep_duration'?durationLabel(latest.value):esc(latest.value),unit=id==='sleep_duration'?'':esc(latest.unit||meta.unit||'');
    let change='Latest';if(delta!=null)change=Math.abs(delta)<1e-9?'Stable':`${delta>0?'↑':'↓'} ${Math.abs(delta).toFixed(Math.abs(delta)<1?1:0)}${unit?` ${unit}`:''}`;
    const icon=id==='sleep_duration'?'moon':id==='weight'?'scale':id==='resting_hr'?'heart':id==='a1c'?'activity':'activity';
    return `<button type="button" class="v47-metric-tile metric-${esc(id)}" data-open-metric-detail="${esc(id)}"><div class="v47-metric-label">${v47Icon(icon,18)}<span>${esc(meta.label)}</span></div><strong>${number}${unit?` <small>${unit}</small>`:''}</strong><span class="v47-metric-change">${esc(change)}</span>${miniSpark(pts,id)}</button>`;
  }
  function v47StatusHTML(){
    const tasks=reviewTasks(), injury=state.events.filter(e=>recordIsActive(e)&&['injury','symptom'].includes(semanticCategory(e))&&(e.structured?.ongoing||/ongoing|active/i.test(String(e.structured?.status||'')))).sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0))[0];
    const actions=[];
    if(tasks.length)actions.push({icon:'question',title:`${tasks.length} question${tasks.length===1?'':'s'} need an answer`,meta:'Review only the items that could change a decision',route:'questions'});
    if(injury)actions.push({icon:'heart',title:'Active injury / symptom context',meta:`${humanEvent(injury)} · ${fmtDate(injury.timestamp||injury.recorded_at,{month:'short',day:'numeric'})}`,route:'health'});
    const summary=actions.length?`${actions.length} current item${actions.length===1?'':'s'} may need your attention.`:'Nothing in your verified record requires attention right now.';
    return `<section class="v47-card v47-status"><div class="v47-card-title"><span class="v47-title-icon">${v47Icon('activity',25)}</span><h2>Today’s Status</h2><span class="v47-pill ${actions.length?'warn':'good'}">${actions.length?'Needs attention':'Clear'}</span></div><p>${esc(summary)}</p><div class="v49-status-actions">${actions.map(a=>`<button type="button" data-route="${a.route}">${v47Icon(a.icon,18)}<div><strong>${esc(a.title)}</strong><small>${esc(a.meta)}</small></div>${v47Icon('chevron',16)}</button>`).join('')||'<div class="v49-status-clear"><strong>No action needed.</strong><span>Recent measurements remain available in Health at a Glance and Recent Activity.</span></div>'}</div></section>`;
  }
  function v47NextHTML(){
    const q=reviewTasks().length,cal=state.calendar.slice(0,q?2:3),rows=cal.map(e=>({icon:'calendar',title:e.title,meta:`${fmtDate(e.start,{weekday:'short',month:'short',day:'numeric'})} · ${fmtTime(e.start)}`,route:'calendar'}));if(q)rows.push({icon:'question',title:`Questions for you (${q})`,meta:'Today',route:'questions'});
    return `<section class="v47-card v47-next"><div class="v47-card-title"><span class="v47-title-icon">${v47Icon('calendar',25)}</span><h2>Next Up</h2>${rows.length?`<span class="v47-count">${rows.length}</span>`:''}</div><div class="v47-next-list">${rows.map(r=>`<button type="button" data-route="${r.route}">${v47Icon(r.icon,17)}<div><strong>${esc(r.title)}</strong><small>${esc(r.meta)}</small></div></button>`).join('')||'<div class="v47-compact-empty">Nothing scheduled or waiting right now.</div>'}</div></section>`;
  }
  function v47QuickHTML(){
    return `<section class="v47-card v47-quick"><h2>Quick Actions</h2><div class="v47-quick-grid"><button type="button" id="v47StartWorkout">${v47Icon('dumbbell',22)}<span>Start<br>Workout</span></button><button type="button" data-health-log="measurement">${v47Icon('heart',22)}<span>Log<br>Health</span></button><button type="button" data-route="calendar">${v47Icon('calendar',22)}<span>View<br>Calendar</span></button><button type="button" id="v47TalkButton">${v47Icon('chat',22)}<span>Talk to<br>ZEKE</span></button></div></section>`;
  }
  function v47RecentHTML(){
    const rows=dedupeDisplayEvents(state.events.filter(e=>recordIsActive(e)&&(isWorkoutEvent(e)||['sleep','measurement','lab','medication','immunotherapy','vaccination','injury','symptom'].includes(semanticCategory(e))))).sort((a,b)=>new Date(b.timestamp||b.recorded_at)-new Date(a.timestamp||a.recorded_at)).slice(0,5);
    const meta=e=>{const cat=semanticCategory(e),w=isWorkoutEvent(e)?workoutStructured(e):null;if(w)return {icon:'dumbbell',title:`Workout: ${w.exercise||'Activity'}`,sub:[w.variation||w.equipment||'',fmtDate(e.timestamp,{month:'short',day:'numeric'})].filter(Boolean).join(' · '),status:'Recorded'};if(cat==='sleep')return {icon:'moon',title:'Sleep',sub:`${durationLabel(Number(e.structured?.value??e.structured?.duration_hours??0))} · ${fmtDate(e.timestamp,{month:'short',day:'numeric'})}`,status:'Logged'};if(cat==='medication')return {icon:'pill',title:e.structured?.medication_name||e.structured?.name||'Medication',sub:fmtDate(e.timestamp,{month:'short',day:'numeric'}),status:'Recorded'};return {icon:cat==='lab'?'activity':cat==='injury'||cat==='symptom'?'heart':'scale',title:humanEvent(e),sub:fmtDate(e.timestamp||e.recorded_at,{month:'short',day:'numeric'}),status:'Recorded'};};
    return `<section class="v47-card v47-recent"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('activity',24)}<h2>Recent Activity</h2></div><button class="text-action" data-route="health">View All</button></div><div class="v47-recent-list">${rows.map(e=>{const m=meta(e);return `<button type="button" class="v47-recent-row" data-route="${isWorkoutEvent(e)?'fitness':'health'}"><span class="v47-row-icon">${v47Icon(m.icon,19)}</span><div><strong>${esc(m.title)}</strong><small>${esc(m.sub)}</small></div><span class="v47-row-status">${esc(m.status)}</span></button>`}).join('')||'<div class="v47-compact-empty">No recent verified activity.</div>'}</div></section>`;
  }
  function v47InsightsHTML(){
    const items=[];const coach=coachInsightFor(state.coachFocus);if(coach?.name)items.push({icon:'dumbbell',title:coach.name,text:coach.suggestion||coach.rationale||'Open Fitness for the supporting history.',action:'fitness'});
    const discovery=state.discoveries.find(d=>!['superseded','dismissed','resolved','stale'].includes(String(d.status||'').toLowerCase())&&!/context is available|trend is current|largest explanatory gap|missing nutrition|nutrition quality/i.test(`${d.title||''} ${d.summary||''} ${d.text||''}`));if(discovery)items.push({icon:'insight',title:discovery.title||'New observation',text:discovery.summary||discovery.text||'Open the evidence for details.',key:discovery.id||discovery.title});
    return `<section class="v47-card v47-insights"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('insight',24)}<h2>ZEKE Insights</h2></div><button class="text-action" data-route="insights">View All</button></div><div class="v47-insight-list">${items.slice(0,3).map((x,i)=>`<button type="button" class="v47-insight-row tone-${i}" ${x.key?`data-insight-key="${esc(x.key)}" data-insight-action="pattern"`:`data-route="${x.action||'insights'}"`}><span class="v47-row-icon">${v47Icon(x.icon,20)}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.text)}</small></div>${v47Icon('chevron',18)}</button>`).join('')||'<div class="v47-compact-empty"><strong>No new evidence-backed insight right now.</strong><span>ZEKE will use this space only when it has something materially different from the data already shown elsewhere.</span></div>'}</div></section>`;
  }
  ...
  function timelineDomainRows(){
    ...
    return [
      {key:'sleep',label:'Sleep & recovery',match:x=>['sleep','recovery'].includes(x.kind)},
      {key:'activity',label:'Exercise & activity',match:x=>['workout','exercise','activity','fitness'].includes(x.kind)},
      {key:'symptoms',label:'Injury / symptoms',match:x=>['illness','injury','symptom'].includes(x.kind)},
      {key:'measurements',label:'Measurements / labs',match:x=>['observation','measurement','lab'].includes(x.kind)},
      {key:'meds',label:'Medications / treatment',match:x=>['medication','vaccination','immunotherapy'].includes(x.kind)},
      {key:'context',label:'Context / calendar',match:x=>['context','calendar','life_event'].includes(x.kind)}
    ].map(r=>({...r,items:all.filter(r.match)}));
  }
  function timelineWindow(){const today=new Date();today.setHours(12,0,0,0);const center=new Date(today);center.setDate(center.getDate()+Number(state.timelineOffsetDays||0));const start=new Date(center);start.setDate(start.getDate()-6);const days=Array.from({length:14},(_,i)=>{const d=new Date(start);d.setDate(d.getDate()+i);return d});return {today,start,end:days.at(-1),days}}
  function v47TimelineHTML(){
    const {today,start,end,days}=timelineWindow(),rows=timelineDomainRows(),key=d=>dayKey(d),visible=rows.map(r=>({...r,items:r.items.filter(x=>{const d=new Date(x.start);return d>=start&&d<=new Date(end.getTime()+864e5)})}));
    return `<section class="v47-card v47-timeline v49-timeline"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('calendar',24)}<div><h2>Timeline Snapshot</h2><small>${esc(fmtDate(start,{month:'short',day:'numeric'}))}–${esc(fmtDate(end,{month:'short',day:'numeric',year:'numeric'}))}</small></div></div><div class="v49-timeline-nav"><button type="button" data-timeline-shift="-7" aria-label="Previous week">‹</button><button type="button" data-timeline-today>Today</button><button type="button" data-timeline-shift="7" aria-label="Next week">›</button><button class="text-action" data-route="calendar">Open timeline</button></div></div><div class="v49-timeline-scroller"><div class="v49-timeline-grid"><div class="v49-timeline-corner">Domain</div>${days.map(d=>`<div class="v49-timeline-date ${key(d)===key(today)?'today':''}"><strong>${d.toLocaleDateString(undefined,{weekday:'short'})}</strong><span>${d.toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span></div>`).join('')}${visible.map(r=>`<div class="v49-timeline-label">${esc(r.label)}</div>${days.map(d=>{const items=r.items.filter(x=>x.day===key(d));return `<div class="v49-timeline-cell ${key(d)===key(today)?'today':''}">${items.slice(0,3).map(x=>`<button type="button" class="v49-timeline-event kind-${esc(x.kind)}" title="${esc(x.label||x.kind)}" data-route="${['workout','exercise','activity','fitness'].includes(x.kind)?'fitness':'health'}"><span></span><b>${esc((x.label||x.kind).slice(0,28))}</b></button>`).join('')}${items.length>3?`<small>+${items.length-3}</small>`:''}</div>`}).join('')}`).join('')}</div></div><p class="v49-timeline-note">Scroll horizontally to move through the visible dates. Use ‹ / › to move a week at a time. Future columns show only genuinely scheduled context, never future observations.</p></section>`;
  }
  function numericGoalValue(v){const m=String(v??'').replace(/,/g,'').match(/-?\d+(?:\.\d+)?/);return m?Number(m[0]):null}
  function inferGoalSemantics(statement='',unit='',targetMetric=''){
    const t=`${statement} ${unit} ${targetMetric}`.toLowerCase();let direction='increase';if(/reduce|decrease|lower|lose|drop|down|under|below|body fat|bodyfat/.test(t))direction='decrease';if(/maintain|keep|stay between|range/.test(t))direction='maintain';if(/at least|above|over/.test(t))direction='at_least';if(/at most|under|below/.test(t))direction='at_most';
    let metric=targetMetric||'';if(!metric){if(/body ?fat/.test(t))metric='body_fat_pct';else if(/weight/.test(t)&&!/deadlift|bench|press|row|curl|extension|squat/.test(t))metric='weight';else if(/steps?/.test(t))metric='steps';else if(/a1c/.test(t))metric='a1c';else if(/resting.*heart|resting.*hr/.test(t))metric='resting_hr';else if(/sleep/.test(t))metric='sleep_duration';}
    return {direction,metric};
  }
  function goalCurrentValue(g){const sem=inferGoalSemantics(g.goal_statement||g.summary||'',g.unit||'',g.target_metric||'');const metric=g.target_metric||sem.metric;if(metric&&METRICS[metric])return latestMetric(metric)?.value??null;const name=String(g.goal_statement||g.summary||'').match(/(?:deadlift|bench(?: press)?|squat|row|curl|leg extension|lat pulldown)/i)?.[0];if(name){const key=activityKey(name),hist=workoutEvents().map(e=>{const w=workoutStructured(e);return {...w,date:e.timestamp||e.recorded_at}}).filter(x=>activityKey(x.exercise||'')===key&&Number(x.weight)>0).sort((a,b)=>new Date(a.date)-new Date(b.date));return hist.at(-1)?.weight??null}return null}
  function goalProgress(g){const sem=inferGoalSemantics(g.goal_statement||g.summary||'',g.unit||'',g.target_metric||''),direction=g.progress_direction||sem.direction,baseline=numericGoalValue(g.baseline),target=numericGoalValue(g.target),current=numericGoalValue(goalCurrentValue(g));if(baseline==null||target==null||current==null)return {pct:null,baseline,target,current,direction};let pct=null;if(direction==='decrease'||direction==='at_most')pct=(baseline-current)/(baseline-target)*100;else if(direction==='increase'||direction==='at_least')pct=(current-baseline)/(target-baseline)*100;else if(direction==='maintain')pct=Math.abs(current-target)<1e-9?100:0;return {pct:Number.isFinite(pct)?Math.max(0,Math.min(100,pct)):null,baseline,target,current,direction}}
  function goalFriendlyTitle(g){const sem=inferGoalSemantics(g.goal_statement||g.summary||'',g.unit||'',g.target_metric||'');if((g.target_metric||sem.metric)==='body_fat_pct'&&g.target)return `Reduce body fat to ${g.target}${g.unit||'%'}`;return g.goal_statement||g.summary||'Goal'}
  function v47GoalsHTML(){
    const goals=activeGoals().slice(0,3);
    return `<section class="v47-card v47-goals"><div class="v47-section-head"><div class="v47-card-title">${v47Icon('target',24)}<h2>Goals</h2></div><button class="text-action" data-route="fitness">Review</button></div>${goals.length?`<div class="v47-goal-list">${goals.map(g=>{const p=goalProgress(g),unit=g.unit||'',started=g.baseline_date||g.created_at||g.updated_at;const line=p.baseline!=null&&p.current!=null&&p.target!=null?`Start ${p.baseline}${unit} → Current ${p.current}${unit} → Target ${p.target}${unit}`:(g.target?`Target: ${g.target}${unit}`:'Open to define progress');const status=p.pct==null?'Progress needs a baseline':p.pct===0?'Goal just started · 0% progress':`${p.pct.toFixed(0)}% of the required change`;return `<button type="button" data-open-goal-detail="${esc(g.id)}"><strong>${esc(goalFriendlyTitle(g))}</strong><small>${esc(line)}</small><span class="v49-goal-meta">${started?`Started ${esc(fmtDate(started,{month:'short',day:'numeric',year:'numeric'}))} · `:''}${esc(status)}</span><span class="v47-progress" aria-label="${esc(status)}"><i style="width:${p.pct==null?0:p.pct}%"></i></span></button>`}).join('')}</div>`:'<div class="v47-goals-empty">No active goals recorded.<button type="button" class="text-action" id="addGoalBtn">Add a goal</button></div>'}</section>`;
  }
  function v47QuickTipHTML(){
    const streak=loggingStreak();if(!streak.streak)return '';
    return `<section class="v47-tip">${v47Icon('activity',21)}<strong>ZEKE Quick Tip:</strong><span>You have a ${streak.streak}-day recording streak. Consistency makes your trends and recommendations more useful.</span><button type="button" class="text-action" data-route="insights">View insights ${v47Icon('chevron',16)}</button></section>`;
  }
  function v47MobileDashboardHTML(){
    const repairs=repairCandidates();
    return `<div class="v47-mobile-dashboard" aria-label="ZEKE mobile dashboard">${coverageHTML()}<div class="dashboard-v47">
      ...
      <section class="dashboard-band dashboard-overview-band"><div class="dashboard-recent-summary">${truthfulRecentActivityHTML()}</div><div class="dashboard-glance-summary">${healthGlanceHTML(4)}</div></section>
      <section class="dashboard-band dashboard-guidance-band"><div>${coachHTML()}</div><div>${weeklyPlanHTML()}</div></section>
      <section class="dashboard-band dashboard-timeline-band">${timelineSnapshotHTML()}</section>
    </div></div>`;
  }
  function dashboardHTML() {
    return `<div class="v47-desktop-dashboard"><div class="v47-dashboard" aria-label="ZEKE dashboard"><div class="v47-grid v47-top-row">${v47StatusHTML()}${v47NextHTML()}${v47QuickHTML()}</div><div class="v47-grid v47-main-row">${v47RecentHTML()}${v47InsightsHTML()}${v47HealthHTML()}</div><div class="v47-grid v47-bottom-row">${v47TimelineHTML()}${v47GoalsHTML()}</div>${v47QuickTipHTML()}</div></div>${v47MobileDashboardHTML()}`;
  }
  ...
  function goalMetricHistory(g,days=365){
    const sem=inferGoalSemantics(g.goal_statement||g.summary||'',g.unit||'',g.target_metric||''),metric=g.target_metric||sem.metric;if(!metric||!METRICS[metric])return [];
    const cutoff=Date.now()-days*864e5;return metricEvents(metric).map(e=>({date:e.timestamp||e.recorded_at,value:Number(metricValue(e))})).filter(x=>Number.isFinite(x.value)&&new Date(x.date).getTime()>=cutoff).sort((a,b)=>new Date(a.date)-new Date(b.date));
  }
  function goalTrendSVG(g){const arr=goalMetricHistory(g,365),p=goalProgress(g);if(arr.length<2)return '<div class="empty-inline">Not enough measurements yet to draw a goal trajectory.</div>';const target=Number(p.target),vals=arr.map(x=>x.value).concat(Number.isFinite(target)?[target]:[]),min=Math.min(...vals),max=Math.max(...vals),span=Math.max(max-min,1),w=720,h=190,pad=28;const x=i=>pad+(i/(arr.length-1))*(w-pad*2),y=v=>h-pad-((v-min)/span)*(h-pad*2),pts=arr.map((d,i)=>`${x(i).toFixed(1)},${y(d.value).toFixed(1)}`).join(' '),targetY=Number.isFinite(target)?y(target):null;return `<svg class="v49-goal-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="Goal progress over time"><line x1="${pad}" y1="${h-pad}" x2="${w-pad}" y2="${h-pad}" class="axis"/>${targetY!=null?`<line x1="${pad}" y1="${targetY}" x2="${w-pad}" y2="${targetY}" class="target"/><text x="${w-pad}" y="${Math.max(12,targetY-5)}" text-anchor="end">Target ${esc(String(target))}${esc(g.unit||'')}</text>`:''}<polyline points="${pts}" class="trend"/>${arr.map((d,i)=>`<circle cx="${x(i)}" cy="${y(d.value)}" r="4"><title>${esc(fmtDate(d.date,{month:'short',day:'numeric',year:'numeric'}))}: ${esc(String(d.value))}${esc(g.unit||'')}</title></circle>`).join('')}<text x="${pad}" y="${h-5}">${esc(fmtDate(arr[0].date,{month:'short',day:'numeric'}))}</text><text x="${w-pad}" y="${h-5}" text-anchor="end">${esc(fmtDate(arr.at(-1).date,{month:'short',day:'numeric'}))}</text></svg>`}
  function openGoalDetail(goalId){const g=state.factors.find(f=>f.id===goalId&&f.type==='goal');if(!g)return;$('#goalDetailModal')?.remove();const p=goalProgress(g),u=g.unit||'',started=g.baseline_date||g.created_at||g.updated_at,remaining=p.current!=null&&p.target!=null?Math.abs(p.current-p.target):null,delta=p.current!=null&&p.baseline!=null?p.current-p.baseline:null,directionLabel={decrease:'lower is progress',increase:'higher is progress',at_most:'at or below target',at_least:'at or above target',maintain:'stay near target'}[p.direction]||p.direction;document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="goalDetailModal"><div class="direct-entry-card wide-card"><div class="section-head"><div><span class="tile-kicker">GOAL</span><h2>${esc(goalFriendlyTitle(g))}</h2><p>${started?`Started ${esc(fmtDate(started,{month:'short',day:'numeric',year:'numeric'}))} · `:''}${esc(directionLabel)}</p></div><button class="icon-btn" id="closeGoalDetail" aria-label="Close">×</button></div><div class="v49-goal-summary"><div><span>Baseline</span><strong>${p.baseline??'—'}${esc(u)}</strong></div><div><span>Current</span><strong>${p.current??'—'}${esc(u)}</strong></div><div><span>Target</span><strong>${p.target??'—'}${esc(u)}</strong></div><div><span>Progress</span><strong>${p.pct==null?'—':`${p.pct.toFixed(0)}%`}</strong></div></div><section><h3>Trajectory</h3>${goalTrendSVG(g)}</section><section class="v49-goal-interpret"><h3>What this means</h3><p>${p.pct==null?'ZEKE needs a baseline, target, and current value before it can calculate progress.':p.pct===0?'This goal has just started. The current value is still at the saved baseline, so progress is 0%.':`You have completed ${p.pct.toFixed(0)}% of the required change from baseline toward the target.`}${delta!=null?` Change since baseline: ${delta>0?'+':''}${delta.toFixed(1)}${esc(u)}.`:''}${remaining!=null?` Remaining to target: ${remaining.toFixed(1)}${esc(u)}.`:''}</p></section><div class="direct-entry-actions"><button class="secondary" id="editGoalFromDetail">Edit goal</button><button class="primary" id="closeGoalDetail2">Done</button></div></div></div>`);const close=()=>$('#goalDetailModal')?.remove();$('#closeGoalDetail').onclick=close;$('#closeGoalDetail2').onclick=close;$('#editGoalFromDetail').onclick=()=>{close();openGoalModal(goalId)}}
  function openGoalModal(goalId=''){
    $('#goalModal')?.remove();const existing=state.factors.find(f=>f.id===goalId&&f.type==='goal')||{},aiAvailable=(state.ai?.providers||[]).some(p=>p.connected||p.hasSessionKey);let reviewText=existing.ai_review||'';const inferred=inferGoalSemantics(existing.goal_statement||existing.summary||'',existing.unit||'',existing.target_metric||'');
    const metricOptions=[['','Automatic / activity-based'],['body_fat_pct','Body fat %'],['weight','Weight'],['steps','Daily steps'],['a1c','A1c'],['resting_hr','Resting heart rate'],['sleep_duration','Sleep duration']];
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="goalModal"><div class="direct-entry-card"><div class="section-head"><div><h2>${existing.id?'Edit goal':'Set a goal'}</h2><p>ZEKE stores what progress means, not just the target number.</p></div><button class="icon-btn" id="closeGoalModal" aria-label="Close">×</button></div><form id="goalForm" class="direct-entry-form"><label>Domain<select id="goalDomain"><option value="fitness" ${existing.domain!=='health'?'selected':''}>Fitness</option><option value="health" ${existing.domain==='health'?'selected':''}>Health</option></select></label><label>Tracked metric<select id="goalMetric">${metricOptions.map(([v,l])=>`<option value="${v}" ${(existing.target_metric||inferred.metric)===v?'selected':''}>${l}</option>`).join('')}</select></label><label class="wide">What would you like to accomplish?<input id="goalStatement" value="${esc(existing.goal_statement||existing.summary||'')}" required placeholder="e.g., reduce body fat to 25%"></label><label>Progress means<select id="goalDirection"><option value="decrease" ${(existing.progress_direction||inferred.direction)==='decrease'?'selected':''}>Value goes down</option><option value="increase" ${(existing.progress_direction||inferred.direction)==='increase'?'selected':''}>Value goes up</option><option value="at_most" ${(existing.progress_direction||inferred.direction)==='at_most'?'selected':''}>Stay at or below target</option><option value="at_least" ${(existing.progress_direction||inferred.direction)==='at_least'?'selected':''}>Stay at or above target</option><option value="maintain" ${(existing.progress_direction||inferred.direction)==='maintain'?'selected':''}>Maintain target / range</option></select></label><label>Current baseline<input id="goalBaseline" value="${esc(existing.baseline||'')}" placeholder="ZEKE can use current verified value"></label><label>Target<input id="goalTarget" value="${esc(existing.target||'')}" placeholder="e.g., 25"></label><label>Unit<input id="goalUnit" value="${esc(existing.unit||'')}" placeholder="%, lb, steps, sessions/week"></label><label>Target / review date (optional)<input id="goalDate" type="date" value="${esc(existing.target_date||'')}"></label><label class="wide">Constraints or context (optional)<textarea id="goalNotes" rows="2">${esc(existing.notes||'')}</textarea></label><section class="goal-review wide" id="goalReview" aria-live="polite"><strong>Goal semantics</strong><p>ZEKE will measure progress from the saved baseline in the selected direction. You can correct its interpretation before saving.</p></section><div class="direct-entry-actions wide"><button type="button" class="secondary" id="useCurrentGoalBaseline">Use current verified value</button><button type="button" class="secondary" id="cancelGoalModal">Cancel</button><button type="submit" class="primary">Save goal</button></div></form></div></div>`);
    const close=()=>$('#goalModal')?.remove(),read=()=>({domain:$('#goalDomain').value,statement:$('#goalStatement').value.trim(),target_metric:$('#goalMetric').value,progress_direction:$('#goalDirection').value,baseline:$('#goalBaseline').value.trim(),target:$('#goalTarget').value.trim(),unit:$('#goalUnit').value.trim(),target_date:$('#goalDate').value||'',notes:$('#goalNotes').value.trim()});
    $('#closeGoalModal').onclick=close;$('#cancelGoalModal').onclick=close;
    $('#goalStatement').addEventListener('blur',()=>{const g=read(),sem=inferGoalSemantics(g.statement,g.unit,g.target_metric);if(!g.target_metric&&sem.metric){$('#goalMetric').value=sem.metric}if(!existing.progress_direction)$('#goalDirection').value=sem.direction;});
    $('#useCurrentGoalBaseline').onclick=()=>{const g=read(),sem=inferGoalSemantics(g.statement,g.unit,g.target_metric),metric=g.target_metric||sem.metric;let value=null;if(metric&&METRICS[metric])value=latestMetric(metric)?.value??null;else value=goalCurrentValue({...existing,goal_statement:g.statement,target_metric:metric});if(value==null){$('#goalReview').innerHTML='<strong>No current verified value found</strong><p>Enter the baseline manually or save the goal without numeric progress until a baseline exists.</p>';return;}$('#goalBaseline').value=value;$('#goalReview').innerHTML=`<strong>Baseline captured</strong><p>${esc(String(value))}${g.unit?` ${esc(g.unit)}`:''} will be treated as 0% progress on the date the goal is saved.</p>`;};
    $('#goalForm').onsubmit=async event=>{event.preventDefault();const goal=read();if(!goal.statement)return;const sem=inferGoalSemantics(goal.statement,goal.unit,goal.target_metric);const metric=goal.target_metric||sem.metric;let baseline=goal.baseline;if(!baseline&&metric&&METRICS[metric]){const cur=latestMetric(metric);if(cur)baseline=String(cur.value)}const now=new Date().toISOString();await ZekeData.saveFactor({...existing,type:'goal',status:'active',domain:goal.domain,summary:goal.statement,goal_statement:goal.statement,target_metric:metric,progress_direction:goal.progress_direction||sem.direction,baseline,baseline_date:existing.baseline_date||(baseline?now:null),target:goal.target,unit:goal.unit,target_date:goal.target_date,notes:goal.notes,ai_review:reviewText||null,provenance:{...(existing.provenance||{}),source:'direct-goal-entry',goal_semantics_version:2}});close();await refreshData();render();showToast(existing.id?'Goal updated with progress semantics.':'Goal saved with a baseline and progress direction.');};
  }
  ...
  function exerciseBodyAreas(name=''){
    const k=window.ZekeKnowledgeBase?.get?.(name);if(!k)return {primary:[],secondary:[]};
    const normalize=x=>String(x||'').trim().replace(/\b\w/g,m=>m.toUpperCase());
    return {primary:(k.primary||[]).map(normalize),secondary:(k.secondary||[]).map(normalize)};
  }
  function bodyAreaLinksHTML(name=''){
    const body=exerciseBodyAreas(name),parts=[...body.primary.map(x=>[x,'primary']),...body.secondary.filter(x=>!body.primary.includes(x)).map(x=>[x,'secondary'])];
    if(!parts.length)return '';
    return `<div class="exercise-body-links" aria-label="Body areas worked">${parts.map(([part,role])=>`<button type="button" class="body-area-link ${role}" data-body-area="${esc(part)}" data-body-source="${esc(name)}"><span>${esc(part)}</span><small>${role}</small></button>`).join('')}</div>`;
  }
  function bodyAreaInjuryMatches(part=''){
    const needle=String(part||'').toLowerCase();if(!needle)return [];
    return state.events.filter(e=>recordIsActive(e)&&['injury','symptom','condition'].includes(semanticCategory(e))).filter(e=>{const st=e.structured||{};return `${st.body_area||''} ${st.location||''} ${st.structure||''} ${e.raw_text||''}`.toLowerCase().includes(needle)}).sort((a,b)=>new Date(b.timestamp||b.recorded_at)-new Date(a.timestamp||a.recorded_at));
  }
  function openBodyAreaView(part=''){
    $('#bodyAreaOverlay')?.remove();const needle=String(part||'').toLowerCase(),catalog=window.ZekeKnowledgeBase?.catalog||[];
    const primary=catalog.filter(k=>(k.primary||[]).some(x=>String(x).toLowerCase().includes(needle))).slice(0,16),secondary=catalog.filter(k=>!(primary.includes(k))&&(k.secondary||[]).some(x=>String(x).toLowerCase().includes(needle))).slice(0,16),injuries=bodyAreaInjuryMatches(part);
    document.body.insertAdjacentHTML('beforeend',`<div class="metric-detail-overlay show" id="bodyAreaOverlay"><section class="metric-detail body-area-detail" role="dialog" aria-modal="true" aria-label="${esc(part)} body area"><button class="metric-detail-close" id="closeBodyArea" aria-label="Close">×</button><span class="tile-kicker">BODY AREA</span><h2>${esc(part)}</h2><p>Exercises, training context, and recorded injury or symptom context connected through ZEKE's body-structure map.</p>${injuries.length?`<div class="body-context-alert"><strong>Current / recent related context</strong>${injuries.slice(0,4).map(e=>`<button type="button" class="body-context-row" data-edit-event="${esc(e.id)}"><span>◇</span><div><b>${esc(humanEvent(e))}</b><small>${esc(fmtDate(e.timestamp||e.recorded_at,{month:'short',day:'numeric',year:'numeric'}))}</small></div></button>`).join('')}</div>`:'<div class="body-context-clear"><span>✓</span><p>No current or recent injury/symptom record is directly linked to this body-area label.</p></div>'}<div class="body-exercise-columns"><div><h3>Primary involvement</h3>${primary.map(k=>`<button type="button" class="body-exercise-row" data-open-body-exercise="${esc(k.name)}"><b>${esc(k.name)}</b><small>${esc(k.equipment||'')}</small></button>`).join('')||'<p class="muted">No mapped primary exercises yet.</p>'}</div><div><h3>Secondary involvement</h3>${secondary.map(k=>`<button type="button" class="body-exercise-row" data-open-body-exercise="${esc(k.name)}"><b>${esc(k.name)}</b><small>${esc(k.equipment||'')}</small></button>`).join('')||'<p class="muted">No mapped secondary exercises yet.</p>'}</div></div><p class="safety-copy">Anatomical overlap provides context; it is not an automatic prohibition or medical clearance.</p></section></div>`);
    const close=()=>$('#bodyAreaOverlay')?.remove();$('#closeBodyArea').onclick=close;$('#bodyAreaOverlay').onclick=e=>{if(e.target.id==='bodyAreaOverlay')close()};
    $$('[data-open-body-exercise]').forEach(b=>b.onclick=()=>{const name=b.dataset.openBodyExercise;close();state.expandedActivity=name;state.activityBodyArea='';state.activityTab='all';state.activitySearch='';go('fitness');render();setTimeout(()=>document.querySelector(`[data-activity-name="${CSS.escape(name)}"]`)?.scrollIntoView({behavior:'smooth',block:'start'}),0)});
    $$('[data-edit-event]',$('#bodyAreaOverlay')).forEach(b=>b.onclick=()=>{const id=b.dataset.editEvent;close();editEvent(id)});
  }
  function variationSummaryHTML(arr=[],family=''){
    const by=new Map();for(const row of arr.filter(x=>!x.placeholder)){const v=String(row.variation_name||family||'Unspecified variation').trim();const prev=by.get(v);if(!prev||new Date(row.date||0)>new Date(prev.date||0))by.set(v,row)}
    const rows=[...by.entries()].sort((a,b)=>new Date(b[1].date||0)-new Date(a[1].date||0));if(!rows.length)return '<div class="empty-inline compact-empty-state">No recorded variations yet.</div>';
    return `<div class="variation-summary-list">${rows.map(([name,r])=>`<button type="button" class="variation-summary-row" data-variation-focus="${esc(name)}"><div><strong>${esc(name)}</strong><small>${esc(fmtDate(r.date,{month:'short',day:'numeric',year:'numeric'}))}</small></div><span>${[r.weight!=null?`${r.weight} lb`:'',r.reps!=null?`${r.reps} reps`:'',r.sets!=null?`${r.sets} sets`:''].filter(Boolean).join(' · ')||'Recorded'}</span></button>`).join('')}</div>`;
  }
  function fitnessPageHTML() {
    ...
    const allBodyAreas=[...new Set(entries.flatMap(x=>{const b=exerciseBodyAreas(x.name);return [...b.primary,...b.secondary]}))].sort();
    if(state.activityBodyArea)entries=entries.filter(x=>{const b=exerciseBodyAreas(x.name);return [...b.primary,...b.secondary].some(p=>p.toLowerCase()===String(state.activityBodyArea).toLowerCase())});
  ...
  function calendarPageHTML() {
    const scale=state.timelineScale||'week',daysByScale={day:1,week:14,month:35,year:365},windowDays=daysByScale[scale]||14,all=window.ZekeLongitudinal?.timeline?.(state.events,state.calendar)||[],today=new Date(),from=new Date(today);from.setHours(0,0,0,0);if(scale!=='day')from.setDate(from.getDate()-Math.floor(windowDays*.72));const to=new Date(from);to.setDate(to.getDate()+windowDays);
    const visible=all.filter(x=>{const t=new Date(x.start);return t>=from&&t<=to}),kindLabel=k=>({workout:'Workout',exercise:'Exercise',activity:'Activity',sleep:'Sleep',medication:'Medication',injury:'Injury',illness:'Illness',symptom:'Symptom',observation:'Measurement',lab:'Lab',vaccination:'Vaccination',immunotherapy:'Immunotherapy',calendar:'Calendar'}[k]||String(k||'Event').replaceAll('_',' '));
    const eventRows=visible.sort((a,b)=>new Date(b.start)-new Date(a.start)).slice(0,scale==='year'?80:160).map(x=>`<button type="button" class="timeline-explorer-event kind-${esc(x.kind)}" data-timeline-event="${esc(x.source_id||x.id||'')}"><time>${esc(fmtDate(x.start,{month:'short',day:'numeric',year:scale==='year'?'numeric':undefined}))}</time><span><small>${esc(kindLabel(x.kind))}</small><strong>${esc(x.label||kindLabel(x.kind))}</strong></span><b>›</b></button>`).join('');
    ...
    const candidateRows=remaining.slice(0,40).map(e=>{const match=calendarHealthMatch(e);return `<article class="calendar-review-item"><div class="calendar-review-copy"><time>${esc(fmtDate(e.start,{month:'short',day:'numeric',year:'numeric'}))}</time><strong>${esc(e.title)}</strong>${match?'<span class="status-badge success">Possible ZEKE match already exists</span>':''}</div><div class="calendar-review-actions"><button class="primary compact" data-calendar-relevance="relevant" data-calendar-id="${esc(e.id)}">Relevant</button><button class="secondary compact" data-calendar-relevance="not_relevant" data-calendar-id="${esc(e.id)}">Not relevant</button><button class="text-action" data-calendar-relevance="unsure" data-calendar-id="${esc(e.id)}">Unsure</button></div></article>`}).join('');
    return `<div class="page-head"><div><span class="tile-kicker">LONGITUDINAL EXPLORER</span><h1>Timeline</h1><p>Health, fitness, recovery, medication, and confirmed context in one record. Scheduled calendar items remain context, not proof that something happened.</p></div></div><section class="panel timeline-explorer"><div class="timeline-scale-control" aria-label="Timeline scale">${[['day','Day'],['week','Week'],['month','Month'],['year','Year']].map(([id,label])=>`<button type="button" class="range ${scale===id?'active':''}" data-timeline-scale="${id}" aria-pressed="${scale===id}">${label}</button>`).join('')}</div><div class="timeline-explorer-summary"><strong>${visible.length} recorded/context item${visible.length===1?'':'s'}</strong><span>${esc(fmtDate(from,{month:'short',day:'numeric',year:'numeric'}))} – ${esc(fmtDate(to,{month:'short',day:'numeric',year:'numeric'}))}</span></div><div class="timeline-explorer-list">${eventRows||'<div class="empty-inline">No recorded events in this window.</div>'}</div></section><section class="panel mobile-calendar-review"><div class="section-head"><div><span class="tile-kicker">CALENDAR CONTEXT</span><h2>Review potentially relevant events</h2><p>Mark relevance first. ZEKE only treats an event as having happened after confirmation or matching evidence.</p></div><button class="secondary" id="loadCalendarReview">${state.calendarReviewLoaded?'Refresh past year':'Scan past year'}</button></div>${state.calendarReviewLoaded?`<div class="calendar-review-summary"><span><b>${candidates.length}</b> potential</span><span><b>${reviewed.length}</b> screened</span><span><b>${remaining.length}</b> left</span></div>${candidateRows||'<div class="empty-inline success-empty">You have screened every potentially relevant event in this calendar window.</div>'}`:'<div class="empty-inline">Nothing is added to your health record during this first-pass scan.</div>'}</section>`;
  }
  ...
  async function downloadSupportWorkbook(options={}){
    ...
    const improvementRows=improvementLog().filter(inRange).map(r=>supportRedact(r,mode));
    ...
    addSupportSheet(wb,'Executive Summary',[
      ...
      {item:'Runtime errors',value:runtimeRows.length},{item:'Open improvements',value:improvementRows.filter(x=>x.status!=='resolved'&&x.status!=='waived').length},{item:'Potential health events',value:potential.length},{item:'Canonical corrections',value:corrections.length}
    ]);
    ...
    addSupportSheet(wb,'Improvements Log',improvementRows);
  ...
  function settingsPageHTML() {
    return `<div class="page-head"><div><h1>Settings</h1><p>Connections and preferences. ZEKE's router and provider managers handle the technical choices.</p></div></div>
      ...
      ${dataVisibilityHTML()}
      <section class="panel settings-section improvements-log-section"><div class="section-head"><div><h2>Improvements Log</h2><p>Persistent product feedback that travels forward across releases. Open items must be reviewed before a new release is treated as ready.</p></div><span class="badge">${improvementLog().filter(x=>x.status!=='waived'&&x.status!=='resolved').length} open</span></div><div class="support-export-grid"><label class="wide">What should ZEKE improve?<textarea id="improvementSummary" rows="2" placeholder="Describe what did not work, what you expected, or what should be better."></textarea></label><label>Area<select id="improvementKind"><option value="ux_feedback">UX / workflow</option><option value="bug">Bug / error</option><option value="ai_mishap">AI mishap</option><option value="data_integrity">Data integrity</option><option value="sync">Sync / connection</option></select></label><label>Priority<select id="improvementSeverity"><option value="normal">Normal</option><option value="high">High</option></select></label></div><div class="card-actions"><button class="primary" id="addImprovementLog">Add to Improvements Log</button><button class="secondary" id="exportImprovementLog">Export log JSON</button></div><div class="improvement-log-preview">${improvementLog().slice(-6).reverse().map(x=>`<article><div><strong>${esc(x.summary)}</strong><small>${esc(fmtDate(x.timestamp,{month:'short',day:'numeric',year:'numeric'}))} · ${esc(x.kind.replaceAll('_',' '))} · v${esc(x.version)}</small></div><div class="improvement-entry-actions"><span>${esc(x.status||'open')}</span>${(x.status||'open')==='open'?`<button type="button" class="text-action" data-improvement-status="resolved" data-improvement-id="${esc(x.id)}">Resolve</button><button type="button" class="text-action" data-improvement-status="waived" data-improvement-id="${esc(x.id)}">Waive</button>`:''}</div></article>`).join('')||'<div class="empty-inline">No locally retained improvement entries yet.</div>'}</div></section>
      <section class="panel settings-section diagnostics-export-section"><div class="section-head"><div><h2>Diagnostics & Exports</h2><p>Create one multi-tab workbook that connects technical failures to the user workflows in which they occurred.</p></div><span class="badge">${runtimeDiagnostics().length+(window.ZekeWorkflowEngine?.metrics()?.unresolved_interactions||0)} retained items</span></div><div class="support-export-grid"><label>Privacy level<select id="supportPrivacyMode"><option value="full" ${state.supportExportOptions.mode==='full'?'selected':''}>Full developer report</option><option value="technical" ${state.supportExportOptions.mode==='technical'?'selected':''}>Technical only</option><option value="anonymized" ${state.supportExportOptions.mode==='anonymized'?'selected':''}>Anonymized</option></select></label><label>From date<input id="supportFromDate" type="date" value="${esc(state.supportExportOptions.from||'')}"></label><label>Through date<input id="supportToDate" type="date" value="${esc(state.supportExportOptions.to||'')}"></label><label class="support-clear-option"><input id="clearAfterSupportExport" type="checkbox" ${state.supportExportOptions.clearAfter?'checked':''}> Clear retained diagnostic logs after a successful export</label></div><div class="card-actions"><button class="primary" id="downloadSupportReport">Download Support & Improvement Report</button><button class="secondary" id="exportRuntimeDiagnostics">Runtime JSON only</button><button class="text-action danger" id="clearRuntimeDiagnostics">Clear retained logs</button></div><p class="safety-copy">Workbook tabs include Executive Summary, Technical Errors, Unresolved Interactions, AI Consultation History, User Corrections, UX Feedback, Improvements Log, Potential Health Events, Audit History, Conversation Metrics, Workflow History, and Developer Notes. API keys, access tokens, passwords, and saved credentials are never included.</p><div class="status-line">${esc(state.supportExportStatus||'')}</div></section>
  ...
  function globalTalkHTML(){ return `<button class="global-talk-button" id="globalTalkButton" aria-label="Talk to ZEKE"><img src="./assets/branding/zeke-mark-provisional.png" alt=""><span>Talk to ZEKE</span></button><div class="global-talk-overlay" id="globalTalkOverlay"><div class="global-talk-backdrop" id="globalTalkBackdrop"></div><div class="global-talk-panel"><button type="button" class="global-talk-close" id="globalTalkClose" aria-label="Close Talk to ZEKE">×</button>${conversationHTML()}</div></div>`;}
  ...
  function v47Icon(name,size=20){
    const paths={
      ...
      question:'<circle cx="12" cy="12" r="9"/><path d="M9.8 9a2.4 2.4 0 1 1 3.8 1.9c-1 .7-1.6 1.2-1.6 2.6M12 17.5h.01"/>',
      settings:'<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1a1.7 1.7 0 0 0 1.9.3A1.7 1.7 0 0 0 10 3V2.8h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.2v4H21a1.7 1.7 0 0 0-1.6 1Z"/>',
      plus:'<path d="M12 5v14M5 12h14"/>',
      chat:'<path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"/><path d="M8 9h8M8 13h5"/>',
      activity:'<path d="M3 12h4l2-7 4 14 2-7h6"/>',
      insight:'<path d="M9 18h6M10 22h4"/><path d="M8.5 15.5A6 6 0 1 1 15.5 15.5C14.5 16.3 14 17 14 18h-4c0-1-.5-1.7-1.5-2.5Z"/>',
      scale:'<circle cx="12" cy="12" r="9"/><path d="M8 10a4 4 0 0 1 8 0M12 10l2-2"/>',
      moon:'<path d="M21 12.8A8.5 8.5 0 1 1 11.2 3 6.5 6.5 0 0 0 21 12.8Z"/>',
      target:'<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1.5"/><path d="M15 9l6-6M17 3h4v4"/>',
      pill:'<path d="m8.5 4.5 11 11a4.2 4.2 0 0 1-6 6l-11-11a4.2 4.2 0 0 1 6-6Z"/><path d="m7 15 8-8"/>',
    ...
    return `<span class="v47-icon" aria-hidden="true"><svg class="v47-icon-svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">${body}</svg></span>`;
  }
  function navHTML() {
    const items=[['dashboard','home','Home'],['health','heart','Health'],['fitness','dumbbell','Fitness'],['calendar','calendar','Timeline'],['insights','compass','Discover'],['documents','file','Documents'],['questions','question','Questions']];
    const navItems=items.map(([id,icon,label])=>`<button class="nav-item ${state.route===id?'active':''}" data-route="${id}" title="${esc(label)}">${v47Icon(icon,21)}<b>${esc(label)}</b></button>`).join('');
    const questions=reviewTasks().length;
    return `<aside class="sidebar v47-sidebar" id="sidebar"><div class="brand"><button class="brand-home" data-route="dashboard" title="Return to Dashboard"><img class="brand-logo" src="./assets/branding/zeke-mark-provisional.png" alt="Project ZEKE"></button><div><strong>PROJECT ZEKE</strong><span>One thread. Everything connected.</span></div><button class="sidebar-close" id="sidebarClose" aria-label="Close navigation">×</button></div><nav>${navItems}<button class="nav-item" id="globalLogNav" title="Log information">${v47Icon('plus',21)}<b>Log</b></button><button class="nav-item" id="openTalkNav" title="Talk to ZEKE">${v47Icon('chat',21)}<b>Talk to ZEKE</b></button></nav><button class="questions-for-you-card ${questions?'has-questions':''}" data-route="questions">${v47Icon('question',18)}<div><strong>${questions} Question${questions===1?'':'s'} for You</strong><small>${questions?'To confirm recent entries and improve insights.':'Nothing is waiting right now.'}</small></div><b>View</b></button><div class="sidebar-spacer"></div><button class="nav-item settings-nav ${state.route==='settings'?'active':''}" data-route="settings" title="Settings">${v47Icon('settings',21)}<b>Settings</b></button><div class="privacy-note">Private by design. Your records stay with your chosen storage provider.</div><div class="build-label">v${esc(BUILD.version)} · ${esc(BUILD.build)}</div></aside><div class="sidebar-scrim" id="sidebarScrim"></div><nav class="mobile-bottom-nav" aria-label="Primary navigation"><button class="mobile-nav-item ${state.route==='dashboard'?'active':''}" data-route="dashboard">${v47Icon('home',19)}<b>Home</b></button><button class="mobile-nav-item ${state.route==='health'?'active':''}" data-route="health">${v47Icon('heart',19)}<b>Health</b></button><button class="mobile-nav-item mobile-zeke-entry" id="mobileLogButton" aria-label="Log information">${v47Icon('plus',20)}<b>Log</b></button><button class="mobile-nav-item ${state.route==='fitness'?'active':''}" data-route="fitness">${v47Icon('dumbbell',19)}<b>Fitness</b></button><button class="mobile-nav-item" id="mobileMoreButton"><span>•••</span><b>More</b></button></nav>`;
  }
  ...
  function topbarHTML() {
    const questions=reviewTasks().length;
    return `<header class="topbar v47-topbar"><button class="topbar-brand brand-home" data-route="dashboard" title="Return to Dashboard"><img src="./assets/branding/zeke-mark-provisional.png" alt="ZEKE"><div><strong>ZEKE</strong><span>v${esc(BUILD.version)}</span></div></button><div class="topbar-greeting"><h1>${esc(greetingText())}.</h1><p>You’ve got this. <span class="v47-top-date">${new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'})}</span></p></div><div class="v47-live"><i></i><span>Live / Connected</span></div><div class="top-actions"><button class="v47-head-icon" id="quickLogBtn" title="Log information" aria-label="Log information">${v47Icon('plus',20)}</button><button class="v47-head-icon" id="searchBtn" title="Search ZEKE" aria-label="Search ZEKE">${v47Icon('search',20)}</button><button class="v47-head-icon" id="topTalkBtn" title="Talk to ZEKE" aria-label="Talk to ZEKE">${v47Icon('chat',20)}</button><button class="v47-head-icon v47-question-head" data-route="questions" title="Questions for You" aria-label="Questions for You">${v47Icon('question',20)}${questions?`<span>${questions}</span>`:''}</button><button class="v47-head-icon" id="statusBtn" title="ZEKE status" aria-label="ZEKE status">${v47Icon('activity',20)}</button></div></header>`;
  }
  ...
  function connectedAppHTML() {
    ...
    return `<div class="app-shell v47-shell">${navHTML()}<main class="main-shell">${topbarHTML()}<div class="content-shell">${content}</div></main>${globalTalkHTML()}${customizeDrawerHTML()}${quickLogHTML()}<div class="toast" id="toast"></div><input type="file" id="conversationFile" hidden></div>`;
  }
  ...
  function openPeriodRecordEditModal(id){
    const event=state.events.find(x=>x.id===id);if(!event)return;
    const s=event.structured||{},category=semanticCategory(event),labels={injury:'Injury',illness:'Illness',symptom:'Symptom',life_event:'Life event'};
    const label=labels[category]||'Health period',start=s.start_date||String(event.timestamp||event.recorded_at||'').slice(0,10)||activeDay(),end=s.end_date||'',ongoing=Boolean(s.ongoing),approximate=Boolean(s.approximate_date);
    const name=s.name||s.condition||s.symptom||s.injury||s.concept_label||event.raw_text||label;
    $('#periodRecordEditModal')?.remove();
    document.body.insertAdjacentHTML('beforeend',`<div class="direct-entry-overlay" id="periodRecordEditModal"><section class="direct-entry-card period-record-edit-card"><div class="section-head"><div><span class="tile-kicker">LONGITUDINAL PERIOD</span><h2>Edit ${esc(label.toLowerCase())}</h2><p>Keep the start, end, and ongoing state explicit. ZEKE preserves the previous version in correction history.</p></div><button class="icon-btn" id="closePeriodRecordEdit" aria-label="Close">×</button></div><form id="periodRecordEditForm" class="direct-entry-form"><label class="wide">What happened<input id="periodEditName" value="${escAttr(name)}" required></label><label>Start date<input id="periodEditStart" type="date" value="${esc(start)}" required></label><label>End date<input id="periodEditEnd" type="date" value="${esc(end)}" ${ongoing?'disabled':''}></label><label class="checkbox-line"><input id="periodEditOngoing" type="checkbox" ${ongoing?'checked':''}> Ongoing</label><label class="checkbox-line"><input id="periodEditApproximate" type="checkbox" ${approximate?'checked':''}> Date is approximate</label><label class="wide">Notes<textarea id="periodEditNotes" rows="3">${esc(s.notes||event.raw_text||'')}</textarea></label><div class="record-provenance wide"><strong>Source</strong><span>${esc(event.provenance?.sheet||event.provenance?.file||event.provenance?.source||'ZEKE')}</span><small>Record ID ${esc(event.id)}</small></div><p class="form-error wide" id="periodEditError" hidden></p><div class="direct-entry-actions wide"><button type="button" class="secondary" id="cancelPeriodRecordEdit">Cancel</button><button type="submit" class="primary">Save period correction</button></div></form></section></div>`);
    const close=()=>$('#periodRecordEditModal')?.remove(),ongoingBox=$('#periodEditOngoing'),endInput=$('#periodEditEnd');
    const syncOngoing=()=>{endInput.disabled=ongoingBox.checked;if(ongoingBox.checked)endInput.value=''};ongoingBox.onchange=syncOngoing;syncOngoing();
    $('#closePeriodRecordEdit').onclick=close;$('#cancelPeriodRecordEdit').onclick=close;$('#periodRecordEditModal').onclick=e=>{if(e.target.id==='periodRecordEditModal')close()};
    $('#periodRecordEditForm').onsubmit=async ev=>{ev.preventDefault();const startDate=$('#periodEditStart').value,endDate=ongoingBox.checked?'':endInput.value;if(endDate&&endDate<startDate){const box=$('#periodEditError');box.hidden=false;box.textContent='End date cannot be before the start date.';return;}const nextName=$('#periodEditName').value.trim(),notes=$('#periodEditNotes').value.trim(),structured={...s,start_date:startDate,end_date:endDate||null,ongoing:ongoingBox.checked,approximate_date:$('#periodEditApproximate').checked,notes,interpretation_status:'confirmed'};if('name' in structured||!structured.concept_label)structured.name=nextName;if('injury' in structured)structured.injury=nextName;if('symptom' in structured)structured.symptom=nextName;if('condition' in structured)structured.condition=nextName;if('concept_label' in structured)structured.concept_label=nextName;await ZekeData.updateEvent(id,{timestamp:`${startDate}T12:00:00`,raw_text:notes||nextName,structured,correction_note:'Longitudinal period corrected by user through period editor'});close();await refreshData();render();showToast(`${label} period updated.`)};
  }
  ...
  async function editEvent(id) {
    ...
    const category=semanticCategory(e);
    if(category==='medication'){openMedicationOccurrenceEditModal(id);return;}
    if(['injury','illness','symptom','life_event'].includes(category)&&(e.structured?.start_date||e.structured?.end_date||e.structured?.ongoing||e.structured?.approximate_date)){openPeriodRecordEditModal(id);return;}
  ...
  function openWorkoutEntryModal(options={}){
    ...
    const progressionContextFor=(item,arr)=>{
    ...
    const renderExercise=index=>{
      currentIndex=index;const item=draft.items[index],profile=item.profile||activityProfile(item.exercise),family=item.exercise_family||activityIdentity(item.exercise,{},'').family||item.exercise,arr=item.variation_name?historyFor(item.variation_name):[],last=arr.at(-1),ready=progressionContextFor(item,arr),variations=knownVariationsForFamily(family,item.exercise);
  ...
  function bind() {
    ...
    $('#v47StartWorkout')?.addEventListener('click',()=>{go('fitness');setTimeout(()=>$('#fitnessBuildBtn')?.click(),80)});
    $('#v47TalkButton')?.addEventListener('click',()=>$('#globalTalkButton')?.click());
    ...
    $$('[data-open-goal-detail]').forEach(el=>el.onclick=()=>openGoalDetail(el.dataset.openGoalDetail));
    ...
    $('#globalTalkButton')?.addEventListener('click',()=>{document.body.classList.add('global-talk-open');setTimeout(()=>$('#talkInput')?.focus(),0)});
    const closeGlobalTalk=()=>{document.body.classList.remove('global-talk-open','global-talk-expanded');document.body.classList.remove('conversation-expanded')};
    $('#globalTalkBackdrop')?.addEventListener('click',closeGlobalTalk);
    $('#globalTalkClose')?.addEventListener('click',closeGlobalTalk);
    ...
    $$('[data-timeline-shift]').forEach(el=>el.onclick=()=>{state.timelineOffsetDays=Number(state.timelineOffsetDays||0)+Number(el.dataset.timelineShift||0);localStorage.setItem('zeke.timeline.offsetDays.v2',String(state.timelineOffsetDays));render()});
    $$('[data-timeline-today]').forEach(el=>el.onclick=()=>{state.timelineOffsetDays=0;localStorage.setItem('zeke.timeline.offsetDays.v2','0');render()});
    ...
    $('#expandConversation')?.addEventListener('click',()=>{const inGlobal=document.body.classList.contains('global-talk-open');const cls=inGlobal?'global-talk-expanded':'conversation-expanded';const expanded=document.body.classList.toggle(cls);if(inGlobal)document.body.classList.remove('conversation-expanded');const btn=$('#expandConversation');if(btn){btn.textContent=expanded?'Collapse':'Expand';btn.setAttribute('aria-expanded',String(expanded));}});
    ...
    $$('[data-direct-question-choice]').forEach(el=>el.onclick=async()=>{
      const q=state.factors.find(f=>f.id===state.activeReviewId);if(!q)return;
      const actions=el.closest('.review-decision-actions');
      actions?.querySelectorAll('button').forEach(b=>b.disabled=true);
      el.classList.add('is-selected','is-working');
      const original=el.textContent;el.textContent='Saving…';
      let feedback=document.querySelector('#reviewSaveFeedback');
      document.querySelectorAll('.review-live-feedback').forEach(node=>{if(node!==feedback)node.remove()});
      if(!feedback){feedback=document.createElement('div');feedback.id='reviewSaveFeedback';feedback.className='review-live-feedback';feedback.setAttribute('role','status');feedback.setAttribute('aria-live','polite');actions?.insertAdjacentElement('afterend',feedback)}
      feedback.classList.remove('is-error','is-success');feedback.textContent='ZEKE is applying your decision…';
      const wf=beginWorkflow(q.question||'Resolve duplicate review',{goal:'Resolve possible duplicate record',target:{question_id:q.id,question_key:q.question_key}});
      state.pending={type:'question',question:q,workflowId:wf?.id||state.workflowId};
      try{
        await handleQuestionChoice(el.dataset.directQuestionChoice);
      }catch(err){
        actions?.querySelectorAll('button').forEach(b=>b.disabled=false);
        el.classList.remove('is-working','is-selected');el.textContent=original;
        const needsReconnect=err?.code==='reauth_required'||/authorization expired|reconnect/i.test(String(err?.message||''));
        feedback.classList.add('is-error');
        feedback.textContent=needsReconnect?'That did not save. Nothing was changed. Reconnect ZEKE storage, then try again.':'That did not save. Nothing was changed; please try again.';
        showToast(feedback.textContent,'error');
      }
    });
    $('#answerReviewNow')?.addEventListener('click',()=>{const q=state.factors.find(f=>f.id===state.activeReviewId),box=$('#reviewInlineAnswer'),choices=$('#reviewInlineChoices');if(!q||!box)return;box.hidden=false;if(choices){const opts=pendingQuestionChoices(q).filter(x=>!['question-answer','question-other','question-why'].includes(x.value));choices.innerHTML=opts.map(x=>`<button type="button" class="secondary compact" data-review-inline-choice="${esc(x.value)}">${esc(x.label)}</button>`).join('');choices.querySelectorAll('[data-review-inline-choice]').forEach(btn=>btn.addEventListener('click',async()=>{const wf=beginWorkflow(q.question||'Resolve review item',{goal:`Resolve: ${reviewFriendlyTitle(q)}`,target:{question_id:q.id,question_key:q.question_key}});state.pending={type:'question',question:q,workflowId:wf?.id||state.workflowId};await handleQuestionChoice(btn.dataset.reviewInlineChoice);if(state.pending==null){state.activeReviewId='';try{sessionStorage.removeItem('zeke-active-review')}catch(_){}go('questions');}render();}));}setTimeout(()=>$('#reviewInlineAnswerText')?.focus(),0);});
    $('#cancelReviewInlineAnswer')?.addEventListener('click',()=>{const box=$('#reviewInlineAnswer');if(box)box.hidden=true;});
    $('#submitReviewInlineAnswer')?.addEventListener('click',async()=>{const q=state.factors.find(f=>f.id===state.activeReviewId),text=$('#reviewInlineAnswerText')?.value.trim();if(!q||!text){showToast('Enter an answer first.','error');return;}const wf=beginWorkflow(q.question||'Resolve review item',{goal:`Resolve: ${reviewFriendlyTitle(q)}`,target:{question_id:q.id,question_key:q.question_key}});state.pending={type:'question-awaiting',question:q,workflowId:wf?.id||state.workflowId};const applied=await applyQuestionAnswer(q,text);if(applied.applied){await ZekeData.resolveFactor(q.id,'resolved',text);state.pending=null;state.activeReviewId='';try{sessionStorage.removeItem('zeke-active-review')}catch(_){}closeWorkflow('completed','The open question was resolved from Questions for You.',{save_status:'saved'});await refreshData();go('questions');render();showToast('Answer saved.');}else{await ZekeData.saveFactor({...q,status:'open',answer_attempt:text,last_attempt_at:new Date().toISOString()});state.pending={type:'question-awaiting',question:{...q,status:'open',answer_attempt:text},workflowId:state.workflowId};showToast(applied.message||'That answer needs more detail.','error');}});
    ...
    $$('[data-timeline-scale]').forEach(b=>b.onclick=()=>{state.timelineScale=b.dataset.timelineScale||'week';localStorage.setItem('zeke.timeline.scale.v1',state.timelineScale);render()});
    $$('[data-timeline-event]').forEach(b=>b.onclick=()=>{const id=b.dataset.timelineEvent;if(id&&state.events.some(e=>e.id===id))editEvent(id);});
    $$('[data-body-browse]').forEach(b=>b.onclick=()=>{state.activityBodyArea=b.dataset.bodyBrowse||'';state.expandedActivity='';render()});
    $$('[data-body-area]').forEach(b=>b.onclick=e=>{e.stopPropagation();openBodyAreaView(b.dataset.bodyArea)});
    $$('[data-variation-focus]').forEach(b=>b.onclick=e=>{e.stopPropagation();const card=b.closest('.fitness-progress-card'),chart=card?.querySelector('.family-variation-chart'),name=b.dataset.variationFocus;if(!chart)return;const same=chart.dataset.focusVariation===name;chart.dataset.focusVariation=same?'':name;chart.querySelectorAll('[data-variation-series]').forEach(g=>g.classList.toggle('is-muted',!same&&g.dataset.variationSeries!==name));card?.querySelectorAll('[data-variation-focus]').forEach(x=>x.classList.toggle('is-active',!same&&x.dataset.variationFocus===name));});
    ...
    $('#addImprovementLog')?.addEventListener('click',async()=>{const summary=$('#improvementSummary')?.value.trim();if(!summary){showToast('Describe the improvement first.','error');return;}recordImprovement({kind:$('#improvementKind')?.value||'ux_feedback',severity:$('#improvementSeverity')?.value||'normal',summary,source:'user-report',context:{path:location.pathname,screen:state.route}});render();showToast('Added to the persistent Improvements Log.');});
    $('#exportImprovementLog')?.addEventListener('click',()=>downloadJSON({export_type:'ZEKE Improvements Log',build:BUILD,exported_at:new Date().toISOString(),entries:improvementLog()},`zeke-improvements-log-${localDay()}.json`));
    $$('[data-improvement-status]').forEach(btn=>btn.addEventListener('click',()=>{const rows=improvementLog(),row=rows.find(x=>x.id===btn.dataset.improvementId);if(!row)return;row.status=btn.dataset.improvementStatus;row.status_changed_at=new Date().toISOString();saveImprovementLog(rows);if(window.ZekeData?.snapshot?.().status==='connected')ZekeData.saveFactor({id:row.id,type:'improvement_log',status:row.status,priority:row.severity==='high'?'high':'normal',summary:row.summary,improvement:row,provenance:{source:'zeke-improvements-log',storage:'user-repository'}}).catch(()=>{});render();showToast(`Improvement ${row.status}.`);}));
