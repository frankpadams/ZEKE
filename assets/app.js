(() => {
  ...
  const ACTIVITY_FIELD_DEFINITIONS={
    ...
    band_resistance:{label:'Band resistance / color',type:'text',key:'band_resistance',placeholder:'e.g., yellow, light, level 2'},
  ...
  const PROFILE_FIELD_DEFAULTS={
    strength:{fields:['weight','weight_unit','reps','sets','rpe','rest_sec','technique_notes','injury_context'],required:[]},
    ...
    rehab:{fields:['sets','reps','band_resistance','pain_before','pain_during','pain_after','difficulty','rom_change','injury_context'],required:[]},
  ...
  function activitySchema(name,profile){
    ...
    const n=String(name||'').toLowerCase();
    if(/cheerleaders?|cheer leader/.test(n))return {fields:['sets','reps','band_resistance','pain_before','pain_during','pain_after','difficulty','rom_change','injury_context'],required:[],custom:false};
  ...
  function openActivityEntryModal(name, profileOverride=''){
    ...
    $('#directExerciseForm')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;if(!form.reportValidity())return;const date=$('#directExerciseDate').value,profile=$('#directActivityProfile').value,notes=$('#directExerciseNotes').value.trim(),fieldValues=readActivityFields();const structured={exercise:name,canonical_activity_id:window.ZekeKnowledgeBase?.get?.(name)?.id||activityKey(name).replace(/ /g,'_'),equipment:window.ZekeKnowledgeBase?.get?.(name)?.equipment||null,movement_pattern:window.ZekeKnowledgeBase?.get?.(name)?.movement||null,activity_profile:profile,activity_schema_fields:activitySchema(name,profile).fields,workout_id:`workout-${date}`,...fieldValues,completed:true,notes,interpretation_status:'confirmed'};const meaningful=Object.entries(fieldValues).some(([,v])=>v!=null&&v!=='')||notes;if(!meaningful){const err=$('#directExerciseError');err.hidden=false;err.textContent='Enter at least one relevant detail or a note.';return;}await ZekeData.addEvent({category:'workout',timestamp:`${date}T12:00:00`,raw_text:notes,structured,provenance:{source:'direct-activity-entry',entry_mode:'schema-driven-form',activity_schema_version:'2'}});close();await refreshData();render();showToast(`${name} logged for ${fmtDate(date+'T12:00:00',{month:'short',day:'numeric'})}.`)});
  }
