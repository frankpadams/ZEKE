...
const UI_KEY='ZEKE_EXPERIENCE_UI_V060';
const CONVO_KEY='ZEKE_CONVERSATION_V060';
const RANGE_OPTIONS=[['1W',7],['1M',30],['3M',90],['6M',180],['1Y',365],['All',0]];
const RESEARCH={
...
function loadUI(){try{return{range:90,hidden:{nutrition:true},...JSON.parse(localStorage.getItem(UI_KEY)||'{}')}}catch{return{range:90,hidden:{nutrition:true}}}}
...
async function renderDashboard(){
  ...
  const ui=loadUI(),events=await api.listEvents(),factors=await api.listFactors();const days=Number(ui.range)||0;
  ...
  const weeks=groupWeeks(events,days||365);const coaches=coachInsights(events);const ideas=researchIdeas(events,factors);
  const wrap=document.createElement('div');wrap.id='zeke-experience-dashboard';wrap.className='zx-dashboard';
  wrap.innerHTML=`
    <section class="zx-hero-row"><div><span class="zx-eyebrow">YOUR HEALTH BRIEFING</span><h2>Here’s what matters right now.</h2><p>Real data first. Interpretation second. Evidence and uncertainty available when you want the detail.</p></div>${rangeSelector(ui)}</section>
    <section class="zx-conversation" id="zx-conversation"><div class="zx-convo-head"><div class="zx-avatar">Z</div><div><h3>Talk to ZEKE</h3><p>Ask a question or tell me what happened. I’ll keep the conversation and response together.</p></div><button class="zx-ai-status" type="button">AI Router</button></div><div class="zx-thread"></div><div class="zx-compose"><textarea placeholder="Ask ZEKE something, or tell ZEKE what happened…"></textarea><button class="zx-send" type="button">Send</button></div><div class="zx-convo-hints"><button>Summarize my recent workouts</button><button>What changed this quarter?</button><button>Am I progressing too fast?</button></div></section>
    <section class="zx-metrics">${metricTile('Weight',weight,{unit:'lb'})}${metricTile('Resting heart rate',rhr,{unit:'bpm'})}${metricTile('Steps',steps,{unit:'steps'})}${metricTile('A1c',a1c,{unit:'%'})}</section>
    <section class="zx-grid-two">
      ${card('Weight trend',weight.length?`${weight.length} observations in selected range`:'No observations in selected range',lineSvg(weight,{unit:'lb'}),'<button class="zx-info" data-explain="weight">?</button>')}
      ${card('Resting heart rate',rhr.length?`${rhr.length} observations in selected range`:'No observations in selected range',lineSvg(rhr,{unit:'bpm'}),'<button class="zx-info" data-explain="rhr">?</button>')}
      ${card('Workout consistency','Sessions per week—not decorative activity bars',barsSvg(weeks,'sessions'),'<a class="zx-link" href="#/health/workouts/analytics">Open analytics</a>')}
      ${card('Recent labs','Values first; context and reference logic expand from here',labTiles(events,days))}
    </section>
    <section class="zx-grid-two zx-insight-grid">
      ${card('Coach’s eye','Evidence-aware prompts based on your logged load, reps, RPE, pain, and progression',coaches.length?`<div class="zx-coach-list">${coaches.map((c,i)=>`<button class="zx-coach ${c.kind}" data-coach="${i}"><span>${c.kind==='caution'?'⚠':'↗'}</span><div><strong>${esc(c.title)}</strong><p>${esc(c.body)}</p></div><em>Why?</em></button>`).join('')}</div>`:'<div class="zx-empty-chart">Log more workout details to unlock progression coaching.</div>')}
      ${card('I’ve been thinking…','Quiet, conversational prompts—not a suggestion queue',`<div class="zx-thoughts">${ideas.map((x,i)=>`<button data-idea="${i}"><span>💬</span><div><strong>${esc(x.title)}</strong><p>${esc(x.body)}</p></div></button>`).join('')}</div>`,'<button class="zx-family" type="button">Family history</button>')}
    </section>`;
...
function hideLegacyDashboard(){
  const selectors=['.tell-zeke-card','.widgets-grid','.right-rail > .ai-card','.right-rail > .response-card','.right-rail > .summary-card'];selectors.forEach(s=>$$(s).forEach(el=>el.classList.add('zx-legacy-hidden')));
  $$('.right-rail .summary-list .progress').forEach(el=>el.style.display='none');
}
...
function bindDashboard(root,state){
  ...
  $('.zx-family',root).onclick=()=>openFamilyModal(state.factors);
...
function openFamilyModal(existing){const fam=existing.filter(x=>x.type==='family_history');const back=modalShell('Family health history',`<p>Add only what you want ZEKE to use as context. A relative’s history should influence questions and discovery context—not become a diagnosis about you.</p><form class="zx-family-form"><div class="zx-form-grid"><label>Relationship<input name="relation" placeholder="e.g., sister, father, maternal uncle" required></label><label>Condition or event<input name="condition" placeholder="e.g., heart attack, depression, melanoma" required></label><label>Age at diagnosis/event<input name="age" type="number" min="0" max="120" placeholder="optional"></label><label>Status / outcome<input name="status" placeholder="e.g., living, deceased, ongoing treatment"></label><label class="full">Notes<textarea name="notes" placeholder="Only context you want ZEKE to retain"></textarea></label></div><button class="zx-primary" type="submit">Add family history</button></form><h3>Recorded family context</h3><div class="zx-family-list">${fam.length?fam.map(f=>`<div><strong>${esc(f.relation||'Relative')} · ${esc(f.condition||'History')}</strong><span>${f.age_at_event?`Age ${esc(f.age_at_event)} · `:''}${esc(f.status_note||'')}</span></div>`).join(''):'<p class="zx-muted">No family history entries yet.</p>'}</div>`);const form=$('.zx-family-form',back);form.onsubmit=async e=>{e.preventDefault();const fd=new FormData(form);try{await api.saveFactor({type:'family_history',relation:fd.get('relation'),condition:fd.get('condition'),age_at_event:fd.get('age')?Number(fd.get('age')):null,status_note:fd.get('status'),notes:fd.get('notes'),source:'user confirmed'});back.remove();const old=$('#zeke-experience-dashboard');if(old)old.remove();renderDashboard();}catch(err){openSimpleModal('Could not save family history',err.message);}};}
