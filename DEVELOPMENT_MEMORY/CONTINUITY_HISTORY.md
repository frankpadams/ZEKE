...
## 2026-08-24 — v0.47.0 recovery continuity, governance 2026.08.24.6

### Trigger
Multiple v0.46 UI packages passed structural/automated checks but rendered far below the approved desktop mockup, culminating in a catastrophic presentation failure with page-scale black SVG/icon shapes. The user directed a rollback-based recovery that must preserve all new functionality while seriously fixing the UI.

### Deep review performed
The current Constitution, Design Authority, Architecture, authority/lifecycle and governance rules, Project State/Gate, release scope, decision/continuity/iteration/error history, backlog/workflow, test guidance/report, package history, Project Identity/Health, standing continuity documents, recent Project ZEKE decisions, v0.45.1 baseline, and reconciled v0.46.0 functional donor were reviewed before the v0.47 recovery implementation.

### Continuity decision
v0.45.1 is the stable rollback source; reconciled v0.46.0 build 2026.08.24.2 donates verified post-v0.45 functionality. The failed later v0.46 presentation layers are excluded. No canonical user-record migration is required for this UI recovery.

### New release assurance
The exact approved desktop mockup is stored in-package as visual evidence. Chromium visual gates now cover multiple desktop/mobile widths, major routes, adversarial sparse/dense/long-text Dashboard states, and oversized SVG detection. These checks supplement rather than replace functional and governance regressions.
